...
NA_stats(def,"b_Term"))
* log(Offr) is a binary variable based on competition. It has a value of 0 for uncompeted contracts and a value of 1 for competition, regardless of number of offers. 'r  summary(def$log(Offr));
NA_stats(def,"log(Offr)")'
...
NA_stats(def,"n_Offr")'
* log(Offr) is the natural log of the number of offers received. Uncompeted contracts are classified as having received one offer. 'r summary(def$log(Offr));
NA_stats(def,"log(Offr)")'
...
levels(smp$EffComp)
smp$log(Offr)<-smp$EffComp
levels(smp$log(Offr))<-list("0"=c("No Comp.","1 Offer"),
                           "1"="2+ Offers")
summary(smp$log(Offr))
summary(smp$EffComp)
smp$log(Offr)<-as.numeric(as.character(smp$log(Offr)))
...
Comp_Cons_NCS6C <- glm (data=smp,
                 log(Offr) ~cl_def6_HHI_lag1+cl_def6_ratio_lag1
                 )
...
Comp_Cons_NCS6D <- glm (data=smp,
                 log(Offr) ~cl_def6_HHI_lag1+cl_def6_obl_lag1+cl_def6_ratio_lag1)
...
Comp_Cons_NCS6E <- glm (data=smp,
                 log(Offr) ~cl_def6_HHI_lag1+cl_def6_ratio_lag1+cl_def6_obl_lag1+
                  cl_US6_avg_sal_lag1 )
...
Comp_Cons_NCS5B <- glm (data=smp,
                 log(Offr) ~cl_def5_HHI_lag1+cl_def5_ratio_lag1 
                 )
...
Comp_Cons_NCS5C <- glm (data=smp,
                 log(Offr) ~cl_def5_HHI_lag1+cl_def5_ratio_lag1+cl_def5_obl_lag1)
...
Comp_Cons_NCS4B <- glm (data=smp,
                 log(Offr) ~cl_def4_HHI_lag1+cl_def4_ratio_lag1, 
                 )
...
Comp_Cons_NCS4C <- glm (data=smp,
                 log(Offr) ~cl_def4_HHI_lag1+cl_def4_ratio_lag1+cl_def4_obl_lag1)
...
Comp_Cons_NCS3B <- glm (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1, 
                 )
...
Comp_Cons_NCS3C <- glm (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1+cl_def3_obl_lag1)
glmer_examine(Comp_Cons_NCS3C)

Comp_Cons_NCS3C2 <- glm (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_obl_lag1)
...
Comp_Cons_NCS2B <- glm (data=smp,
                 log(Offr) ~cl_def2_HHI_lag1+cl_def2_ratio_lag1
                 )
...
Comp_Cons_NCS2C <- glm (data=smp,
                 log(Offr) ~cl_def2_HHI_lag1+cl_def2_ratio_lag1+cl_def2_obl_lag1)
glmer_examine(Comp_Cons_NCS2C,display=TRUE)

Comp_Cons_NCS2C2 <- glm (data=smp,
                 log(Offr) ~cl_def2_HHI_lag1+cl_def2_obl_lag1)
glmer_examine(Comp_Cons_NCS2C,display=TRUE)
...
corrdisp<-Hmisc::rcorr(
  as.matrix(cbind(subset(smp, select=c(cl_def3_HHI_lag1,
                                       cl_def3_ratio_lag1,
                                       cl_def6_HHI_lag1,
                                       cl_def6_obl_lag1,
                                       cl_def6_ratio_lag1,
...
if(!exists("Comp_Cons_15C")){
  ...
  # save(Comp_Cons_15C,file="Output//Comp_Cons_15C.rdata")
   ...
   Comp_Cons_15C_1m <- lmer (data=smp1m,
                         log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1+
                         # cl_US3_avg_sal_lag1+
                         cl_def6_HHI_lag1+cl_def6_ratio_lag1+cl_def6_obl_lag1+
                         cl_US6_avg_sal_lag1 + 
   ...
   save(Comp_Cons_15C,Comp_Cons_15C_1m,file="Output//Comp_Cons_15C.rdata")
}
glmer_examine(Comp_Cons_13C)
stargazer::stargazer(Comp_Cons_13A,Comp_Cons_13B,Comp_Cons_13C,
                     type="text",
                       digits=2)
stargazer::stargazer(Comp_Cons_15C,Comp_Cons_15C_1m,type="text",
                       digits=2)
...
stargazer::stargazer(Comp_Cons_13A,Comp_Cons_13B,Comp_Cons_13C,Comp_Cons_13D,Comp_Cons_13D_1m,
                     type="text",
                       digits=2)
summary_regression_compare(Comp_Cons_13C,Comp_Cons_13D)
summary_regression_compare(Comp_Cons_13C_1m,Comp_Cons_13D_1m)
...
###Model 15D: Removing salary:pricing
We don't have a good explaination for the  result, so we're taking out the interaction, even though it's significant.
```{r Model15D}
...
stargazer::stargazer( Comp_Cons_15A,Comp_Cons_15B,Comp_Cons_15D,
                     type="text",
                       digits=2)
...
While the AIC and BIC scores get worse, the resuls make more sense afterwards, notably detailed industry consolidation no longer predicts more offers. Leaving out pricing:salar.

###Model 15E: Ratio:Ceiling
Both base variables are significant but with an unexpected sign in the case of ratio. The expectation is that in instances with a high contract ceiling and a high ratio, that larger contracts in a defense intensive sector, the number of offers may go up as vendors attempt to capture winner take all contracts and may not have recourse to commercial markets if they lose.
...
Comp_Cons_15E <- lmer (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1+
                  ...
                  cl_US6_avg_sal_lag1 + 
                   ...
                   # b_UCA:cl_Days+
                   # b_UCA:cl_Ceil+
                   cl_def6_ratio_lag1:cl_Ceil+
                   (1 | NAICS3/NAICS) + (1 | Agency/Office)+
                   (1 | StartCY),
                        verbose=1)

glmer_examine(Comp_Cons_15E,display=TRUE)
get_icc(Comp_Cons_15E)
save(Comp_Cons_15E,file="Output//Comp_Cons_15E.rdata")

stargazer::stargazer(Comp_Cons_15D,Comp_Cons_15E,
                     type="text",
                       digits=2)


summary_regression_compare(Comp_Cons_13D,Comp_Cons_15E)


```
Contrary to expectation, leaving it out.
###Model 15G: Defense Obligations:Vehicle
Detailed industires with more obligations have a larger industrila base that can compete for indefinite delivery vehicles and thus the interaction is expected to estimate more offers.
```{r Model15G}

Comp_Cons_15G <- lmer (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1+
                  # cl_US3_avg_sal_lag1+
                   cl_def6_HHI_lag1+cl_def6_ratio_lag1+cl_def6_obl_lag1+
                  cl_US6_avg_sal_lag1 + 
                   ...
                   # b_UCA:cl_Days+
                   # b_UCA:cl_Ceil+
                   cl_def6_obl_lag1:Veh+
                   (1 | NAICS3/NAICS) + (1 | Agency/Office)+
                   (1 | StartCY),
                        verbose=1)

glmer_examine(Comp_Cons_15G,display=TRUE)
get_icc(Comp_Cons_15G)
save(Comp_Cons_15G,file="Output//Comp_Cons_15G.rdata")

stargazer::stargazer(Comp_Cons_15D,Comp_Cons_15G,
                     type="text",
                       digits=2)


summary_regression_compare(Comp_Cons_13D,Comp_Cons_15G)


```

###Model 15H: Defense Ratio:Pricing
Detailed industries with a larger proportion in the defense sector may also have a larger pool of companies that use government cost accounting standards, and thus that are able to bid on cost-based or incentive fee contracts. 

```{r Model15H}

Comp_Cons_15H <- lmer (data=smp,
                 log(Offr) ~cl_def3_HHI_lag1+cl_def3_ratio_lag1+
                  # cl_US3_avg_sal_lag1+
                   cl_def6_HHI_lag1+cl_def6_ratio_lag1+cl_def6_obl_lag1+
                  cl_US6_avg_sal_lag1 + 
                   ...
                   # b_UCA:cl_Days+
                   # b_UCA:cl_Ceil+
                   cl_def6_ratio_lag1:PricingFee+
                   (1 | NAICS3/NAICS) + (1 | Agency/Office)+
                   (1 | StartCY),
                        verbose=1)

glmer_examine(Comp_Cons_15H,display=TRUE)
get_icc(Comp_Cons_15H)
save(Comp_Cons_15H,file="Output//Comp_Cons_15H.rdata")

stargazer::stargazer(Comp_Cons_15D,Comp_Cons_15H,
                     type="text",
                       digits=2)


summary_regression_compare(Comp_Cons_13D,Comp_Cons_15H)


```
The expectation does not hold up, the interaction will not be added.


###Model 16A: Agency/Office+NAICS6 trimmed
This is the NAICS6 only version of the model. It uses 15C as a basis and then strips out the NAICS3 references, aside from being used to organize the NAICS6 level (note the : rather than the /)
```{r Model16A}
...
stargazer::stargazer(Comp_Cons_15C, Comp_Cons_16A,
                     type="text",
                       digits=2)
