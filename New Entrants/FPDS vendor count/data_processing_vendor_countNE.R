...
library(csis360)
  ...
  platform_only <- read_delim(
    ...
    na=c("NA","NULL"),delim="\t")
  top_level <- read_csv("K:/2018-01 NPS New Entrants/FPDS Vendor Count/Entity Count History Customer NE.csv")
  ...
  top_levelNE <- top_level %>%
    ...
    mutate(
      ...
      EntitySizeText = fct_recode(
        ...
        "Large+" =  "Large: Big 5 JV")) %>%
    group_by(
      fiscal_year, duns, EntitySizeText, EntitySizeText.detail,
      EntityCategory,
      AnyEntityUSplaceOfPerformance,
      AnyEntityForeignPlaceOfPerformance,
      IsEntityAbove2016constantOneMillionThreshold,
