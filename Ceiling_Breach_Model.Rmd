...
save(naics02_cbre,naics02_term,naics02_comp,
     ...
     file="Output//empty_models.rdata")

load(file="Output//empty_models.rdata")
...
### Quarter
```{r StartFY}
lubridate::month(smp$MinOfSignedDate)
lubridate::month(smp$MinOfSignedDate)  
  floor((12-1)/4)
StartFY_term <- glmer(data=smp,
                 b_Term ~ (1 | StartFY)
                  , family=binomial(link="logit"))
display(StartFY_term)
summary(StartFY_term)
get_icc(StartFY_term)

StartFY_cbre <- glmer(data=smp,
                 b_CBre ~ (1 | StartFY)
                  , family=binomial(link="logit"))
display(StartFY_cbre)
summary(StartFY_cbre)
get_icc(StartFY_cbre)

StartFY_comp <- glmer(data=smp,
                 b_Comp ~ (1 | StartFY)
                  , family=binomial(link="logit"))
