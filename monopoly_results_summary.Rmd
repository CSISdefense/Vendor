...
centered_log_description(log(def$HHI_lag1),"days")

NA_stats(def,"c_HHI_lag1.y")
...
save(file="250kSample.Rdata",smp)
...
smp$NAICS2<-substring(smp$NAICS6,1,2)
smp1m$NAICS6<-smp$NAICS
smp$NAICS5<-substring(smp$NAICS6,1,5)
smp$NAICS4<-substring(smp$NAICS6,1,4)
smp$NAICS3<-substring(smp$NAICS6,1,3)
smp1m$NAICS2<-substring(smp1m$NAICS6,1,2)
...
save("Term_Comp.rdata",Term_Comp_05A,Term_Comp_08A,Term_Comp_08B,Term_Comp_08C,Term_Comp_08D,Term_Comp_08E,Term_Comp_08F,Term_Comp_08G,Term_Comp_08H)
...
Term_CBre_05A <- glm (data=smp,
                   b_CBre ~n_Comp + 
                     ...
                     b_UCA:n_Comp
                   , family=binomial(link="logit"))
display(Term_CBre_05A)
smp$NAICS2<-substring(smp$NAICS,1,2)

Term_CBre_08B<- glmer(data=smp1m,
                    b_CBre ~n_Comp + 
                      cl_Ceil + cl_Days+ 
                      SIDV + MIDV + FSSGWAC + BPABOA +
                      n_Fixed + b_UCA +
                      ...
                      (1 | NAICS2)
                    , family=binomial(link="logit"))
display(Term_CBre_08B)
save(file="Term_CBre.RData",Term_CBre_08B)
library(stargazer)

?htmlreg
texreg::htmlreg(list(Term_Cons_08A,
                     CBre_Cons_08A),file="ConsModel.html",single.row = TRUE)


texreg::htmlreg(list(Term_Comp_08H,
                     Term_CBre_08B),file="Comp2Model.html",single.row = TRUE)

texreg::htmlreg(list(Term_CBre_08B,Term_Cons_08A),file="Hypothesis1.html")
# 
...
stargazer(Term_CBre_08B,Term_Cons_08A,type="html",out="Term_2.html")
```
...
save(file="Term_Cons.RData",Term_Cons_08A)
```
