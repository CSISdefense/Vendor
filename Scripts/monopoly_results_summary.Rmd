...
def_detail<-def_detail %>% dplyr::select(CSIScontractID,ChangeOrderCeilingGrowth)
smp<-left_join(smp,def_detail)
smp1m<-left_join(smp1m,def_detail)
def<-left_join(def,def_detail)

  
    smp1m$n_CBre<-(smp1m$ChangeOrderCeilingGrowth/
                        smp1m$UnmodifiedContractBaseAndAllOptionsValue_Then_Year)+1
    smp1m$n_CBre[
      is.na(smp1m$n_CBre) & smp1m$b_CBre==0]<-1

    #l_CBre
    smp1m$l_CBre<-na_non_positive_log(smp1m$n_CBre)
...
sum(def$Action_Obligation[def$b_Term==1],na.rm=TRUE)/sum(def$Action_Obligation,na.rm=TRUE)`
...
NAICS_summary<-def %>% group_by(NAICS,StartCY,def6_HHI_lag1,def6_obl_lag1,def6_ratio_lag1,US6_avg_sal_lag1) %>%
  dplyr::summarise(annual_action_obligation=sum(Action_Obligation),
...
# 
# Percent_Obli <- c()
# Total_Obli <- sum(def$Action_Obligation, na.rm = TRUE)
# memory.limit(56000)
# for (i in NAICS_Freq$NAICS_Code) {
#   Percent_Obligation <- round(sum(def$Action_Obligation[def$NAICS == i], na.rm = TRUE)/Total_Obli,5)
#   Percent_Obli <- c(Percent_Obli, Percent_Obligation)
#   print(i)
# }
...
for (i in NAICS2_Freq$NAICS_Code) {
  Percent_Obligation <- round(sum(def$Action_Obligation[def$NAICS2 == i], na.rm = TRUE)/Total_Obli,5)
  Percent_Obli <- c(Percent_Obli, Percent_Obligation)
}
...
for (i in Frequency[["NAICS3"]]) {
  Percent_Obligation <- round(sum(def[["Action_Obligation"]][def$NAICS3 == i], na.rm = TRUE)/sum(def[["Action_Obligation"]], na.rm = TRUE),5)
    Percent_Obli <- c(Percent_Obli, Percent_Obligation)
  }
...
Percent_Obli <- c()
Total_Obli <- sum(def_dup$Action_Obligation, na.rm = TRUE)
for (i in Office_Freq$OfficeID) {
  Percent_Obligation <- round(sum(def_dup$Action_Obligation[def_dup$Office == i], na.rm = TRUE)/Total_Obli,5)
  Percent_Obli <- c(Percent_Obli, Percent_Obligation)
}
...
sum(def$Action_Obligation[is.na(def$EffComp)],na.rm=TRUE)/sum(def$Action_Obligation,na.rm=TRUE)
...
# smp<-smp[smp$CSIScontractID %in% def$CSIScontractID,]
# smp1m<-smp1m[smp1m$CSIScontractID %in% def$CSIScontractID,]
...
# sum(def[!complete,]$Action_Obligation.Then.Year,na.rm=TRUE)/sum(def$Action_Obligation.Then.Year,na.rm=TRUE)
# 
...
#Make sure we're using then year dollars
...
#  "MaxBoostDate"
#  
#  smp<-smp %>% dplyr::select(-qLowCeiling)
#  smp1m<-smp1m %>% dplyr::select(-qLowCeiling)
#  
# 
# 
#         
# #Sample adjustments
# colnames(smp)[colnames(smp)=="Dur"]<-"qDuration"
# colnames(smp1m)[colnames(smp1m)=="Dur"]<-"qDuration"
...
# colnames(smp)[colnames(smp)=="ObligationWT"]<-"ObligationWT_Then_Year"
# colnames(smp1m)[colnames(smp1m)=="ObligationWT"]<-"ObligationWT_Then_Year"
# colnames(smp)[colnames(smp)=="UnmodifiedContractBaseAndAllOptionsValue"]<-"UnmodifiedContractBaseAndAllOptionsValue_Then_Year"
# colnames(smp1m)[colnames(smp1m)=="UnmodifiedContractBaseAndAllOptionsValue"]<-"UnmodifiedContractBaseAndAllOptionsValue_Then_Year"
