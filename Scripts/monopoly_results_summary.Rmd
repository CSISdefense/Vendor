...
# save(file="../Data/Clean/defense_contract_complete.RData",def)
...
CloseOut<-def %>% dplyr::filter(IsClosed==1)
...
CloseOut<-CloseOut %>% filter(IsClosed==1)

# summary(CloseOut)
summary(CloseOut$MaxClosedDate)
summary(CloseOut$ObligatedAfterMaxClosedDate)

sum(CloseOut$ObligatedAfterMaxClosedDate)

CloseOut$PostCloseFloor0<-CloseOut$ObligatedAfterMaxClosedDate
CloseOut$PostCloseFloor0[CloseOut$PostCloseFloor0<0]<-0

CloseOut$Term

(
CloseOutCeilObl<-ggplot(CloseOut,aes(x=PostCloseFloor0+1,color=qHighCeiling)) +
  ...
  scale_y_continuous(label=percent)+facet_wrap(~Term)
)
```

#### After Close Out Duration
```{r CloseOutDur}

summary(CloseOut$MaxClosedDate)
summary(CloseOut$oblig)

CloseOut$PreCloseLength<-CloseOut$MaxClosedDate-CloseOut$MinOfSignedDate
CloseOut$PostCloseCompleteLength<-CloseOut$MaxOfSignedDate-CloseOut$MaxClosedDate

summary(CloseOut$PreCloseLength)



(
CloseDur<-ggplot(CloseOut,aes(x=PreCloseLength))+ #,color=ClosePctCat)) +
  stat_ecdf(geom = "step")+
  ...
  labs(title="Duration and Close Out Completeness",
       ...
       x="Days from Start to Final Close Out",
...
(
CloseDur<-ggplot(CloseOut,aes(x=PostCloseCompleteLength,color=qHighCeiling))+ #,color=ClosePctCat)) +
  stat_ecdf(geom = "step")+
    coord_cartesian(ylim=c(0.7,1),xlim=c(0,2500))+#+
  # scale_x_log10(label=comma)+
  labs(title="Duration and Close Out Completeness",
       y="Cumulative Distribution % of\nCategory's Closed Out Contracts",
       x="Days Close Out to Final Signed Date",
       color="Close Out Completeness")+
  scale_y_continuous(label=percent)+facet_wrap(~Term)+geom_vline(xintercept = 365)
)
...
nrow(term %>% filter(ThroughTerm/3<ObligatedAfterMaxTerminatedDate))
...
term$TermPct<-term$PostTermFloor0/(term$PostTermFloor0+term$PreTermFloor1)
summary(term$TermPct)
# g<-build_plot(
...
term$NoPreTermObl[term$ObligatedBeforeMaxTerminatedDate<=0]<-"0 Pre-Term"
term$NoPreTermObl<-factor(term$NoPreTermObl)


term$TermPctCat<-Hmisc::cut2(term$TermPct,c(0.25,0.5))
summary(term$TermPctCat)
# levels(term$TermPctCat)[3]<-"[50%+]"
term$TermPctCat<-as.character(term$TermPctCat)
term$TermPctCat[term$TermPct==0]<-"[0]"
term$TermPctCat[term$TermPct==0 & term$ObligatedOnMaxTerminatedDate>0]<-"[0]+Last Day Spend"
# term$TermPctCat[term$TermPct==0 & term$ObligatedOnMaxTerminatedDate>0 & term$MaxTerminatedDate<=term$MinOfSignedDate]<-"[0]+One Day Spend"
term$TermPctCat<-factor(term$TermPctCat)
summary(term$TermPctCat)




levels(term$TermPctCat)<-list(
...
term$TermPartial<-term$TermPctCat
levels(term$TermPartial)<-list(
  "Complete Termination"=c("[0]+No Last Day Spend","[0]+Last Day Spend","(0%,25%)"),
  "Partial Termination"=c("[25%,50%)","[50%+]")
)
summary(term$TermPartial)

term$TermPctCat3<-term$TermPctCat
levels(term$TermPctCat3)<-list(
...
(
TermPctHist<-ggplot(term %>% filter(TermPct>0),aes(x=TermPct,fill=TermPctCat )) +
...
(
TermPctHistDoC<-ggplot(term %>% filter(TermPct>0),aes(x=TermPct,fill=TermPctCat )) +
  geom_histogram(bins=100)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  geom_vline(xintercept = 1)+geom_vline(xintercept = 0.1)+
facet_grid(NoPreTermObl~isDefaultOrCause,scales="free_y", space="free_y")+
  labs(title="Distribution of Contracts with Obligations After Last termination",
       y="Contract Count",
       x="Percent of Obligations After Day of Termination",
       fill="Termination Completion"
...
(
TermPct<-ggplot(term,aes(x=TermPct)) +
  stat_ecdf(geom = "step")+
...
(
TermPctDoC<-ggplot(term,aes(x=TermPct,color=isDefaultOrCause)) +
  stat_ecdf(geom = "step")+
    coord_cartesian(ylim=c(0.9,1),xlim=c(0,1))+
  labs(title="Cumulative Distribution of Terminated Contracts",
       y="% of Terminated Contracts",
...
(
TermCeilRatio<-ggplot(term,aes(x=TermPct,color=qHighCeiling)) +
  stat_ecdf(geom = "step")+
...
(
TermCeilOblDoC<-ggplot(term,aes(x=PostTermFloor0+1,color=qHighCeiling)) +
  stat_ecdf(geom = "step")+stat_ecdf(geom = "step",aes(x=PostTermFloor0+1))+
    coord_cartesian(ylim=c(0.65,1))+#+,xlim=c(0,1e8)
  scale_x_log10(label=comma)+
  labs(title="Cumulative Distribution of Terminated Contracts",
       ...
       x="Obligations After Termination",
       color="Initial Ceiling")+
  scale_y_continuous(label=percent)+facet_wrap(~isDefaultOrCause)
)
...
term$PostTermCompleteLength<-term$MaxTerminatedDate-term$MinOfSignedDate
...
(
TermDur<-ggplot(term,aes(x=PreTermLength,color=TermPctCat)) +
  stat_ecdf(geom = "step")+
...
(
TermOblByCeil<-ggplot(term,aes(x=TermPctCat,fill=TermPctCat,y=Action.Obligation)) +
