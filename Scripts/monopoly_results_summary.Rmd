...
nrow(term %>% filter(PreTerm<=0))
nrow(term %>% filter(PreTerm<=PostTerm))
nrow(term %>% filter(PreTerm/10<=PostTerm))
...
Weird<-(term %>% filter(ObligatedBeforeMaxTerminatedDate<=0))
summary(Weird$ObligatedAfterMaxTerminatedDate)
...
term$PostTermFloor0<-term$PostTerm
term$PostTermFloor0[term$PostTermFloor0<0]<-0
term$PreTermFloor1<-term$PreTerm
term$PreTermFloor1[term$PreTerm<1]<-1
term$TermRatio<-term$PostTermFloor0/term$PreTermFloor1
term$TermRatioCapped[term$Term>1]<-1
...
term$NoPreTermObl<-"No"
term$NoPreTermObl[term$PreTerm<=0]<-"Yes"
term$NoPreTermObl<-factor(term$NoPreTermObl)

ggplot(term,aes(x=PostTermFloor0+1)) +
  geom_histogram(bins=100)+scale_x_log10(labels = comma)+scale_y_log10(labels = comma)

ggplot(term,aes(x=TermRatioCapped)) +
  geom_histogram(bins=100)+scale_y_log10(labels = comma)


ggplot(term %>% filter(TermRatioCapped>0),aes(x=TermRatioCapped )) +
  geom_histogram(bins=100)#+scale_y_log10(labels = comma)


ggplot(term %>% filter(TermRatioCapped>0),aes(x=TermRatio )) +
  geom_histogram(bins=100)+scale_x_log10(labels = comma)

ggplot(term %>% filter(TermRatioCapped>0),aes(x=TermRatio,fill=NoPreTermObl )) +
  geom_histogram(bins=100)+scale_x_log10(labels = comma)+facet_wrap(~Dur)

ggplot(term %>% filter(TermRatioCapped>0),aes(x=TermRatio,fill=NoPreTermObl )) +
  geom_histogram(bins=100)+scale_x_log10(breaks=c(0.001,0.01,0.1,0.5,1,10,100,1000))+
  theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  geom_vline(xintercept = 1)+geom_vline(xintercept = 0.1)+geom_vline(xintercept = 2)
#+facet_wrap(~Dur)

term$TermRatioCat<-Hmisc::cut2(term$TermRatio,c(0,0.10,2))
...
levels(term$TermRatioCat)<-list("[0]"="[0]",
                                "(0,0.1)"="[0.00e+00,1.00e-01)",
                                "[0.1,2)"="[1.00e-01,2.00e+00)",
                                "[2+]"="[2.00e+00,6.96e+06]"
                                )
summary(term$TermRatioCat)
