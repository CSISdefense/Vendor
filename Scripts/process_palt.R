...
(countlowSIDV<-ggplot(def_palt %>% filter(Veh=="SINGLE AWARD IDC"|
                                            (is.na(Veh)&IsIDV=="IDV")) , aes(x=PALT))+geom_histogram()+scale_x_log10()+facet_grid(qLowCeiling~StartFiscal_Year,scales="free_y"))

(countlowOther<-ggplot(def_palt %>% filter(Veh!="SINGLE AWARD IDC"&!is.na(Veh)) , aes(x=PALT))+geom_histogram()+scale_x_log10()+facet_grid(qLowCeiling~StartFiscal_Year,scales="free_y"))
