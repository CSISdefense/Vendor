...
if(!exists("login")) login<-askpass::askpass("Please enter the login account")
if(!exists("pwd")) pwd<-askpass::askpass("Please enter the account password")
...
def_all<-input_sample_criteria(def_all,  
                      ...
                      password=password,
                      IsDefense=1)
# def_all$Customer<-"Defense"

def_all<-def_all %>% filter(StartFiscal_Year<=2021 & StartFiscal_Year>=2007)# & Customer=="Defense")
...
def_all$CSIScontractID<-text_to_number(def_all$CSIScontractID)
def_all<-filter(def_all, !is.na(CSIScontractID))
#It appears there's some garbage at the end of the files, I do need to find a way to
#turn off the time stamps, but adding a pruge function called from read_and_join_experiment
...
if(!"Customer" %in% colnames(def_all))
  def_all<-read_and_join_experiment(def_all,
                                    "Agency_AgencyID.csv",
...
def_all<-def_all %>% filter(is.na(Customer)| Customer=="Defense")
save(file="..\\data\\clean\\defense_contract_all_detail.Rdata",def_all)
...
```{r CeilingBreaches}
if(!exists("def_all")) load(file="..\\data\\clean\\defense_contract_all_detail.Rdata")
# undebug(input_contract_ceiling_breach)



def_all<-input_contract_ceiling_breach(def_all,
             schema="Contract",
             sp="SP_ContractCeilingBreachCustomer",
             login=login,
             password=password)
...
##Location
```{r Location}
if(!exists("def_detail")) load(file="..\\data\\clean\\defense_contract_complete_detail.Rdata")
dir<-"../data/semi_clean/"
rda_file<-"Contract.SP_ContractLocationCustomer.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractLocationCustomer @IsDefense=NULL",sep=""))
    rm(con)
...
# load(file="..\\data\\clean\\defense_contract_complete_detail.Rda")
...
dir<-"../data/semi_clean/"
rda_file<-"Contract.SP_ContractUnmodifiedCompetitionvehicleCustomer.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    con <- dbConnect(odbc(),
                     ...
                     PWD =pwd)
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractUnmodifiedCompetitionvehicleCustomer @Customer=NULL",sep=""))
    rm(con)
    lookup<-standardize_variable_names(lookup)
...
rda_file<-"Contract.SP_ContractCompetitionVehicleCustomer.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    con <- dbConnect(odbc(),
                     ...
                     PWD =pwd)
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractCompetitionVehicleCustomer @IsDefense=NULL",sep=""))
    rm(con)
    lookup<-standardize_variable_names(lookup)
...
def_detail$UnmodifiedNumberOfOffersReceived<-text_to_number(def_detail$UnmodifiedNumberOfOffersReceived)
def_detail$NumberOfOffersReceived<-text_to_number(def_detail$NumberOfOffersReceived)
...
dir<-"../data/semi_clean/"
rda_file<-"Contract.SP_ContractPricingCustomer.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    con <- dbConnect(odbc(),
                     ...
                     PWD =pwd)
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractPricingCustomer @Customer=NULL",sep=""))
    rm(con)
    lookup<-standardize_variable_names(lookup)
...
rda_file<-"Contract.SP_ContractPricingUCA.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    con <- dbConnect(odbc(),
                     ...
                     PWD =pwd)
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractPricingUCA @Customer=NULL",sep=""))
    rm(con)
    lookup<-standardize_variable_names(lookup)
...
def_detail<-read_and_join_experiment(data=def_detail
  ...
  ,create_lookup_rdata=TRUE
  # ,col_types = "dcccdddccc"
)


#Impute UCA unmodified values when consistent for the remainder of the contract
...
dir<-"../data/semi_clean/"
rda_file<-"Contract.SP_ContractBucketPlatformCustomer.rda"
  ...
  if(!file.exists(file.path(dir,rda_file))){
    ...
    con <- dbConnect(odbc(),
                     ...
                     PWD =pwd)
    lookup<-dbGetQuery(con,  
                       paste("EXEC Contract.SP_ContractBucketPlatformCustomer @Customer=NULL, @StartFiscalYear=NULL",sep=""))
    rm(con)
    lookup<-standardize_variable_names(lookup)
...
def_detail<-read_and_join_experiment(data=def_detail
  ...
  ,by="CSIScontractID"
  ,new_var_checked=FALSE
  # ,col_types = "dccdddcccc"
  ,create_lookup_rdata=TRUE
)
# 
# def_detail$ObligatedAmountIsProducts<-text_to_number(def_detail$ObligatedAmountIsProducts)
# 
# def_detail$ObligatedAmountIsServices<-text_to_number(def_detail$ObligatedAmountIsServices)
# 
# def_detail$ObligatedAmountIsRnD<-text_to_number(def_detail$ObligatedAmountIsRnD)
# 
...
# 
# 
# def_detail$ObligatedAmountIsLand<-text_to_number(def_detail$ObligatedAmountIsLand)
# 
# def_detail$ObligatedAmountIsVessel<-text_to_number(def_detail$ObligatedAmountIsVessel)
# 
# 
# def_detail$ObligatedAmountIsVessel<-text_to_number(def_detail$ObligatedAmountIsAir)
# 
# def_detail$ObligatedAmountIsVessel<-text_to_number(def_detail$ObligatedAmountIsOtherPP)
# 
# def_detail$ObligatedAmountIsWnA<-text_to_number(def_detail$ObligatedAmountIsWnA)
# 
# def_detail$ObligatedAmountIsMnS<-text_to_number(def_detail$ObligatedAmountIsMnS)
# 
# def_detail$ObligatedAmountIsEnC<-text_to_number(def_detail$ObligatedAmountIsEnC)
# 
# def_detail$ObligatedAmountIsFRSnC<-text_to_number(def_detail$ObligatedAmountIsFRSnC)
# 
# 
...
def_detail$PlatformPortfolio[def_detail$PlatformPortfolio=="Unlabeled"]<-NA

#Impute UCA unmodified values when consistent for the remainder of the contract
def_detail$UnmodifiedPlatformPortfolio[is.na(def_detail$UnmodifiedPlatformPortfolio)&
  !is.na(def_detail$PlatformPortfolio)]<-
  def_detail$PlatformPortfolio[is.na(def_detail$UnmodifiedPlatformPortfolio)&
  !is.na(def_detail$PlatformPortfolio)]
...
def_all<-apply_standard_lookups(def_all)
save(file="..\\data\\clean\\defense_contract_all_detail.Rdata",def_all)
...
# debug(apply_lookups)
def_detail<-apply_standard_lookups(def_detail)
save(file="..\\data\\clean\\defense_contract_complete_detail.Rdata",def_detail)
...
def<-def %>% dplyr::filter(MinOfSignedDate>=as.Date("2008-01-01") & MinOfSignedDate<=as.Date("2015-12-31"))
...
#Renamed Variables
colnames(def)[colnames(def)=="clr_Def6toUS"]<-"cl_def6_ratio_lag1"
colnames(def)[colnames(def)=="clr_Def3toUS"]<-"cl_def3_ratio_lag1"
colnames(def)[colnames(def)=="cln_Def6HHI"]<-"cl_def6_HHI_lag1"
colnames(def)[colnames(def)=="cln_Def3HHI"]<-"cl_def3_HHI_lag1"


#Then Year rescaled  variables
def$cl_Ceil<-arm::rescale(
  na_non_positive_log(def$UnmodifiedCeiling_Then_Year))
def$cl_def6_obl_lag1<-arm::rescale(
  na_non_positive_log(def$def6_obl_lag1))
def$cl_US6_avg_sal_lag1<-arm::rescale(
  na_non_positive_log(def$US6_avg_sal_lag1))
def$cl_Days<-arm::rescale(
  na_non_positive_log(def$UnmodifiedDays)
)
if("UnmodifiedBase_Then_Year" %in%colnames(def))
  def$cln_Base_Then_Year<-arm::rescale(na_non_positive_log(def$UnmodifiedBase_Then_Year))
...
#Dropping the capped cln_days,  as vendor uses uncapped.
def<-def[,!colnames(def) %in% c("cln_Days")]
...
# load(file="../Data/Clean/transformed_def_2021_08_31.Rdata")
