...
def_all<-input_sample_criteria(def_all,  
    ...
    def_all$IsComplete<-0
      #Limit to completed def_alls that start in 2007 or later
      def_all$IsComplete[def_all$StartFiscal_Year>=2007 & 
                         #For unclosed out def_alls both current completion and last signed should before  2016
                         (def_all$MaxBoostDate<=as.Date(last_date)| is.na(def_all$MaxBoostDate))&
                         (def_all$LastCurrentCompletionDate<=as.Date(last_date)
                          #For closed out it's enough that the boost date is within the range.
                          | (def_all$IsClosed==1 | def_all$IsTerminated==1))
                       ]<-1
