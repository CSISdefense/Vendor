...
load(file=file.path("..","..","Data","semiclean","OSC","FAADCloanDataSet.rda"))
  ...
  write_csv(cfda,file="../../data/semi_clean/cfda.csv")
...
loan<-left_join(loan,labeled,by="transaction_description")
loan_crit<-loan %>% filter((AnyTag==TRUE & !is.na(AnyTag)) |
                   cfda_number %in% c("81.112","81.126","59.058") |
                   cfda_title %in% c("STEWARDSHIP SCIENCE GRANT PROGRAM",
                                     "FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES",
                                     "FEDERAL AND STATE TECHNOLOGY PARTNERSHIP PROGRAM"))
loan_crit$Renewable[loan_crit$cfda_number %in% c("81.126")|
                   loan_crit$cfda_title %in% c("FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES")]<-TRUE
loan_crit$AnyTag[loan_crit$cfda_number %in% c("81.126")|
                   loan_crit$cfda_title %in% c("FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES")]<-TRUE

write_csv(loan_crit %>% filter(AnyTag==TRUE & !is.na(AnyTag)),file="../../data/semi_clean/OSC/USAspending_OSCtagged.csv")
save(loan_crit,file="../../data/semi_clean/OSC/USAspending_OSCtagged.rda")
