...
load("..\\..\\Data\\semi_clean\\OSC\\SelectedLoanDataSet.rda")
  ...
  write_csv(cfda,file="../../data/semi_clean/selected_cfda.csv")
...
labeled<-left_join(loanSelected,labeled,by="transaction_description")
labeled$HighTech[labeled$cfda_num %in% c("81.112","81.126","59.058")|
...
labeled$AnyTag[labeled$cfda_num %in% c("81.112","81.126","59.058")|
                   labeled$cfda_title %in% c("STEWARDSHIP SCIENCE GRANT PROGRAM",
                                     "FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES",
                                     "FEDERAL AND STATE TECHNOLOGY PARTNERSHIP PROGRAM")]<-TRUE
...
loan_crit<-loanSelected %>% filter((AnyTag==TRUE & !is.na(AnyTag)) |
                   cfda_num %in% c("81.112","81.126","59.058") |
                   cfda_title %in% c("STEWARDSHIP SCIENCE GRANT PROGRAM",
                                     "FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES",
                                     "FEDERAL AND STATE TECHNOLOGY PARTNERSHIP PROGRAM"))
loan_crit$Renewable[loan_crit$cfda_num %in% c("81.126")|
                   loan_crit$cfda_title %in% c("FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES")]<-TRUE
loan_crit$AnyTag[loan_crit$cfda_num %in% c("81.126")|
                   loan_crit$cfda_title %in% c("FEDERAL LOAN GUARANTEES FOR INNOVATIVE ENERGY TECHNOLOGIES")]<-TRUE

write_csv(loan_crit %>% filter(AnyTag==TRUE & !is.na(AnyTag)),file="../../data/semi_clean/OSC/USAspending_selected_OSCtagged.csv")
save(loan_crit,file="../../data/semi_clean/OSC/USAspending_OSCselectedTagged.rda")
