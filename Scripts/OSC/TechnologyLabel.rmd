...
exim<-label_keyword(exim %>%select(-AnyTag),keyword,"Product.Description") 
# exim<-exim
colnames(exim)
exim_naics <- exim %>% group_by(Primary.Export.Product.NAICS,
...
naics<-left_join(naics%>%select(-AnyTag),exim_naics %>% group_by(Primary.Export.Product.NAICS,
                                               AnyTag) %>% summarise() %>%
                   dplyr::select(Primary.Export.Product.NAICS,AnyTag),
