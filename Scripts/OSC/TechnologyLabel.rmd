...
writeData(wb, sheet="Selected",loanSelected %>% filter(AnyTag==TRUE & !is.na(AnyTag)))
...
saveWorkbook(wb,file=(file.path("..","..","Output","OSC")),overwrite = TRUE)
...
```{r TagOutput}
write_csv(loanSelected %>% filter(AnyTag==TRUE & !is.na(AnyTag)),file="../../data/semi_clean/OSC/USAspending_Energy_OSCtagged.csv",na = "")
write_csv(label_exim %>% filter(AnyTag==TRUE & !is.na(AnyTag)),file="../../data/semi_clean/OSC/EXIMtagged.csv",na = "")
save(loanSelected,label_exim,file="../../data/semi_clean/OSC/OSCselectedTagged.rda",na = "")
```
