...
x<-label_keyword(descrip,keyword)
summary(x)

labeled<-left_join(loan,x,by="transaction_description")
...
write_csv(labeled %>% filter(AnyTag==TRUE & !is.na(AnyTag)),file="OSCtagged.csv")
write_csv(cfda,file="cfda.csv")
