...
bio_data %>% filter(nationalinterestactioncode=="P20C") %>% group_by(Fiscal.Year) %>% summarise(Action_Obligation=sum(Action_Obligation_Then_Year))
```
...
bio_data$biobasedPSC[is.na(bio_data$biobasedPSC)&bio_data$ProductServiceOrRnDarea=="FRS"]<-
...
summary(factor(bio_data$biobasedPSC))
