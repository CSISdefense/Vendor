...
##FFS
```{r life rnd}
colnames(life_HERD)[colnames(life_rnd)=="[Detailed Fields]"]<-"DetailedFields"
...
write.csv(pivot_wider(a$data,id_cols=DetailedFields, names_from = Fiscal.Year,values_from = value_OMB20_GDP20),
          file = file.path("..","Data","semi_clean","NCES_LifeSciences.csv"))
...
a<-ggplot(life_HERD, #%>% dplyr::filter(DetailedFields!="Biological aspects"),
          aes(x=Fiscal_Year,y=Expenditures,fill=Detailed_Field))+geom_col()+scale_fill_manual(values=cbPalette)+labs(
  x="Fiscal Year",y="Federal Funded University Expenditures for R&D\n(thousands, constant 2020 $)",caption="Source: National Center for Science and Engineering Statistics",fill="Detailed Field"
)+scale_x_continuous(breaks=c(2010,2015,2019))+scale_y_continuous(label=scales::comma)+facet_wrap(~Federal_and_Nonfederal)
)
ggsave(a,file=file.path("..","output","HERD_funds.png"))
a$data  
write.csv(pivot_wider(a$data,id_cols=DetailedFields, names_from = Fiscal.Year,values_from = value_OMB20_GDP20),
          file = file.path("..","Data","semi_clean","NCES_LifeSciences.csv"))

life_HERD$`[Basic Research and Applied Research]`
