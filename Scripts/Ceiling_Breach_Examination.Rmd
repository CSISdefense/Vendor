...
summary(W912UMtrans$vendorcountrycode)

W912UMtrans<-remove_bom(W912UMtrans)
W912UMtrans<-inner_join(W912UMtrans,W912UM %>% group_by() %>% dplyr::select(CSIScontractID,unmodWon,sumWon,changeWon),
                       by="CSIScontractID")
...
summary(W912UMtrans$modnumber)
ggplot(filter(W912UMtrans %>% filter(modnumber=='0')) ,aes(x=unmodWon))+geom_bar()+facet_wrap(~vendorcountrycode)
ggplot(W912UMtrans,aes(x=changeWon))+geom_bar()+facet_wrap(~vendorcountrycode)
ggplot(W912UMtrans,aes(x=sumWon))+geom_bar()+facet_wrap(~vendorcountrycode)

W912UMtrans$placeofperformancecountrycode
ggplot(W912UMtrans,aes(x=unmodWon))+geom_bar()+facet_wrap(~placeofmanufacture)
ggplot(W912UMtrans,aes(x=unmodWon))+geom_bar()+facet_wrap(~countryoforigin)
ggplot(W912UMtrans,aes(x=unmodWon))+geom_bar()+facet_wrap(~placeofperformancecountrycode)
                 

View(W912UMtrans %>% filter(unmodWon %in% c("? Unm","Won Unm") & vendorcountrycode=="USA: UNITED STATES OF AMERICA" & modnumber=='0'))

Miscategorized<-W912UMtrans %>% filter(unmodWon %in% c("? Unm","Won Unm") & vendorcountrycode=="USA: UNITED STATES OF AMERICA" & modnumber=='0')
