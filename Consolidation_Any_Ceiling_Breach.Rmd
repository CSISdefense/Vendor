...
* b_Crai is a binary variable that has a value of 1 for any contracts that have experienced a partial or complete termination, 0 otherwise. ('r summary(def$b_Crai)
NA_stats(def,"b_Crai"))
...
NA_stats(def,"n_Offr")'
* hh_index_lag1 is the natural log of the number of offers received. Uncompeted contracts are classified as having received one offer. 'r summary(def$hh_index_lag1);
NA_stats(def,"hh_index_lag1")'
...
summary(Term_smp$c_hh_index_lag1 )
...
# sum(subset(Term_smp,l_Ceil<cutoff)$b_Crai)/sum(Term_smp$b_Crai)
# 
# 
# sum(subset(Term_smp,l_Ceil<cutoff,na.rm=TRUE)$Action.Obligation)/sum(Term_smp$Action.Obligation)
# sum(subset(Term_smp,l_Ceil>=cutoff,na.rm=TRUE)$Action.Obligation)/sum(Term_smp$Action.Obligation)
# 
# sum(subset(Term_smp,l_Ceil<cutoff)$b_Crai)/nrow(subset(Term_smp,l_Ceil<cutoff,na.rm=TRUE))
# 
# sum(subset(Term_smp,l_Ceil>=cutoff)$b_Crai)/nrow(subset(Term_smp,l_Ceil<cutoff,na.rm=TRUE))
# 
# 
...
#Frequency Plot
freq_continuous_term_plot(Term_smp,"hh_index_lag1")

ggplot(data=Term_smp,aes(x=hh_index_lag1,weight=Action.Obligation))+geom_histogram()

freq_continuous_term_plot(Term_smp,"hh_index_lag1","Veh")

ggplot(data=Term_smp,aes(x=hh_index_lag1,weight=Action.Obligation))+geom_histogram()+facet_wrap(~Veh)

freq_continuous_term_plot(Term_smp,"hh_index_lag1","PSR")
freq_continuous_term_plot(Term_smp,"hh_index_lag1","FxCb")

#Percent Terminated Plot 
debug(binned_percent_term_plot)
binned_percent_term_plot(Term_smp,"hh_index_lag1")

#Percent Terminated Plot
binned_percent_term_plot(Term_smp,"hh_index_lag1","FxCb")

#Percent Terminated Plot
binned_percent_term_plot(Term_smp,"hh_index_lag1","PSR")

#Percent Terminated Plot
binned_percent_term_plot(Term_smp,"hh_index_lag1","Veh")
...
discrete_fitted_term_model(Term_smp,"chh_index_lag1") +
...
#Frequency Plot
freq_discrete_crai_plot(Term_smp,"Fee")
debug(discrete_percent_crai_plot)
#Percent Terminated Plot
discrete_percent_crai_plot(Term_smp,"Fee")
...
#Frequency Plot
freq_discrete_term_plot(Term_smp,"UCA","c_hh_index_lag1")

#Percent Terminated Plot 
discrete_percent_term_plot(Term_smp,"UCA","c_hh_index_lag1")
...
#Frequency Plot
freq_discrete_term_plot(Term_smp,"NAICS")

#Percent Terminated Plot 
discrete_percent_term_plot(Term_smp,"NAICS")
...
#Percent Terminated Plot 
discrete_percent_term_plot(Term_smp,"NAICS")
Term_smp$topContractingOfficeID
Term_09A <- glmer(data=Term_smp,
                 b_Crai ~c_hh_index_lag1 + 
                   cl_Ceil + cl_Days+ 
                   SIDV + MIDV + OIDV +
...
#Ran for two hours Model failed to converge with max|grad| = 0.186396 (tol = 0.001, component 1)
# Plot the model and Vehicle 
Term_09B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        UCA*Comp) %*%  coef(Term_09B))}

#Days curves
discrete_fitted_term_model(Term_smp,"SIDV") +
  stat_function(fun = Term_09B_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1,UCA=0),color="blue")+
  stat_function(fun = Term_09B_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(Term_09B)

#Plot residuals versus fittedO
residuals_term_plot(Term_09B)+
  labs(x="Estimated  Pr (Termination)")
