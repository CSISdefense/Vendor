...
def_palt<
def_palt_median<-def_palt %>% group_by(qLowCeiling,StartFiscal_Year,Veh.SIDC) %>%
  summarise(med=median(PALT,na.rm=TRUE))
...
PaltCeilLabels<-
    ...
    FacetValue=paste("$: $",round(sum(Action_Obligation)/1000000000,1),"B",sep=""),
    FacetMedian=paste(FacetValue,"\nMed: ",round(median(PALT,na.rm=TRUE),1),sep=""),
    YPos=max(hist(log10(PALT), breaks=25, plot=FALSE)$counts)
    ) %>% group_by(qLowCeiling,Veh.SIDC)%>%
  mutate(YPos=max(YPos)
         )
...
(countlowSIDV<-ggplot(def_palt %>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"),
                      aes(x=PALT))+geom_histogram()+
    geom_vline(data=def_palt_median%>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"),
               aes(xintercept=med),color="red")+
    scale_x_log10()+facet_grid(qLowCeiling~StartFiscal_Year,scales="free_y"))



(countlowOther<-ggplot(def_palt %>% filter(Veh.SIDC=="All Other Awards and Task Orders"),
                       aes(x=PALT))+geom_histogram()+
    geom_vline(data=def_palt_median%>% filter(Veh.SIDC=="All Other Awards and Task Orders"),
               aes(xintercept=med),color="red")+
    geom_text(data=PaltCeilLabels %>% filter(Veh.SIDC=="All Other Awards and Task Orders"),
              aes(x=500,y=YPos, label=FacetMedian),#,y=Ypos),
              # parse=TRUE,
              hjust=0,
              vjust=1,
              color="black")+
