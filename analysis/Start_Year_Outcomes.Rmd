...
def_dur %>% group_by(IsComplete,StartFiscal_Year,ActualDur) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation_OMB20_GDP18,na.rm=TRUE)) %>%
  ggplot(aes(fill=factor(ActualDur),x=StartFiscal_Year,y=Action_Obligation))+
  geom_col()#+facet_wrap(IsComplete~.,ncol=1)
...
### Initial vs. Actual
```{r StackedLastYear}
def_dur %>% group_by(IsComplete,StartFiscal_Year,ActualDur,qDuration) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation_OMB20_GDP18,na.rm=TRUE)) %>%
  ggplot(aes(fill=ActualDur,x=StartFiscal_Year,y=Action_Obligation))+
  geom_col()+facet_wrap(qDuration~.)
...
```{r Pricing}
# undebug(build_plot)
(dur_pricing <-  build_plot(data=def_dur,# %>% filter(Fiscal.Year>=2007&
                                                                    # !is.na(FirstYear)&
                                                                    # !is.na(qDuration)),
                            chart_geom="Bar Chart",
                            share=FALSE,
