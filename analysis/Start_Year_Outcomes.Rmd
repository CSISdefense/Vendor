...
def_dur$LastCurrentCompletionYear<-
  as.character(lubridate::year(def_dur$LastCurrentCompletionDate)+ifelse(lubridate::month(def_dur$LastCurrentCompletionDate)>=10,1,0))
def_dur$LastCurrentCompletionYear[def_dur$IsComplete==0 | is.na(def_dur$IsComplete)]<-"Active in 2018"

def_dur$LastCurrentCompletionYear<-factor(def_dur$LastCurrentCompletionYear,levels=c("Active in 2018",2017:2007))



def_dur<-deflate(def_dur,
                        money_var = "Action_Obligation",
                        # deflator_var="OMB.2019",
                        fy_var="StartFiscal_Year"
)

def_dur <- def_dur %>% group_by(
                      IsTerminated,
                      IsClosed,
                      IsComplete,
                      # pChangeOrderUnmodifiedBaseAndAll,
  ...
  summarise(Action_Obligation_OMB20_GDP18=sum(na.rm=TRUE),
  ...
  if (!"Dur" %in% colnames(def_dur)){
    def_dur$Dur<-Hmisc::cut2(def_dur$UnmodifiedDays,cuts=c(61,214,366,732))
  }
    
  if (levels(def_dur$Dur)[[2]] %in% c("[   61,  214)","[     61,    214)")){
    levels(def_dur$Dur)<- list(
      ...
      "(~2 years+]"=levels(def_dur$Dur)[5])
  }
  
  def_dur<-replace_nas_with_unlabeled(def_dur,"Dur")
...
### Initial vs. Actual
```{r StackedLastYear}
def_dur %>% group_by(IsComplete,StartFiscal_Year,ActualDur,Dur) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation_OMB20_GDP18,na.rm=TRUE)) %>%
  ggplot(aes(fill=ActualDur,x=StartFiscal_Year,y=Action_Obligation))+
  geom_col()+facet_wrap(Dur~.)
