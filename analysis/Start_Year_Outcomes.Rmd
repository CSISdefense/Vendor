...
if(!"Action_Obligation_OMB20_GDP18" %in% colnames(def_all))
...
def_dur<-def_all %>%
  ...
  summarise(Action_Obligation_OMB20_GDP18=sum(Action_Obligation_OMB20_GDP18,na.rm=TRUE),
...
def_dur %>% group_by(IsComplete,StartFiscal_Year,LastCurrentCompletionYear) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation_OMB20_GDP18,na.rm=TRUE)) %>%
...
summary(def_dur$Action_Obligation_OMB20_GDP18)
...
DTRA_palt_stat<-DTRA_palt %>% group_by(qLowCeiling,StartFiscal_Year,Veh.SIDC) %>%
  dplyr::summarise(Median=median(PALT,na.rm=TRUE),
            Mean=mean(PALT,na.rm=TRUE))%>% pivot_longer(cols=c(Mean,Median),names_to="Statistic")
...
(countlowSIDV<-ggplot(DTRA_palt %>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"& StartFiscal_Year>=2019),
                      aes(x=PALT))+geom_histogram()+
    geom_vline(data=DTRA_palt_stat%>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"& 
                                                StartFiscal_Year>=2019),
               aes(xintercept=value, color=Statistic))+
  
    # geom_text(data=PaltCeilLabels %>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"& 
    #                                            StartFiscal_Year>=2019),
              # aes(x=400,y=YPos, label=FacetMean),#,y=Ypos),
              # parse=TRUE,
              # hjust=0,
...
ggsave600dpi(width=6.5, height=6, size=11, lineheight=1.2,file=file.path("..","output","DTRA_PALT_SIDV.png"),
             countlowSIDV)
ggsave600dpi(width=6.5, height=6, size=11, lineheight=1.2, file=file.path("..","output","DTRA_PALT_SIDV.emf"),
             countlowSIDV)

(countlowOther<-ggplot(DTRA_palt %>% filter(Veh.SIDC=="All Other Awards and Task Orders"& StartFiscal_Year>=2019),
                       aes(x=PALT))+geom_histogram()+
    geom_vline(data=DTRA_palt_stat%>% filter(Veh.SIDC=="All Other Awards and Task Orders"& StartFiscal_Year>=2019),
               aes(xintercept=value, color=Statistic))+
    # geom_text(data=PaltCeilLabels %>% filter(Veh.SIDC=="All Other Awards and Task Orders"& StartFiscal_Year>=2019),
    #           aes(x=400,y=YPos, label=FacetMean),#,y=Ypos),
    #           # parse=TRUE,
...
ggsave600dpi(width=6.5, height=6, size=11, lineheight=1.2,file=file.path("..","output","DTRA_PALT_other.png"),
             countlowOther)
ggsave600dpi(width=6.5, height=6, size=11, lineheight=1.2, file=file.path("..","output","DTRA_PALT_other.emf"),
             countlowOther)
