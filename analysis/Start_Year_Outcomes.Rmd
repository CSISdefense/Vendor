...
PaltCeilLabels<-
    ...
    FacetMedian=paste(FacetValue,"\n",round(median(PALT,na.rm=TRUE),0),
                      ifelse(round(median(PALT,na.rm=TRUE),0)==1," Day"," Days"),sep=""),
...
(countlowSIDV<-ggplot(def_palt %>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"& StartFiscal_Year>=2019),
    ...
    geom_text(data=PaltCeilLabels %>% filter(Veh.SIDC=="Single Award or Unlabeled Task Order"& 
                                               StartFiscal_Year>=2019),
              aes(x=400,y=YPos, label=FacetMedian),#,y=Ypos),
              # parse=TRUE,
              hjust=0,
              vjust=1,
              color="black")+
...
(countlowOther<-ggplot(def_palt %>% filter(Veh.SIDC=="All Other Awards and Task Orders"& StartFiscal_Year>=2019),
                       aes(x=PALT))+geom_histogram()+
    geom_vline(data=def_palt_median%>% filter(Veh.SIDC=="All Other Awards and Task Orders"& StartFiscal_Year>=2019),
               aes(xintercept=med),color="red")+
...
write.csv(
  PaltCeilLabels %>% group_by(qLowCeiling,StartFiscal_Year,Veh.SIDC) %>%
    ...
    pivot_wider(id_cols=c(Veh.SIDC,qLowCeiling),
                        names_from=c(StartFiscal_Year),values_from=FacetMedian) %>%
