...
# add_labels_and_colors(space,"SpaceParentID")
...
(
SpaceParentOnly<-build_plot(
  data=space %>% filter(Fiscal_YQ<2024 & SpaceParentID=="SPACEX"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=space_lc,
  # NA, #VAR.ncol
...
log_plot(plot=SpaceParent, df=space %>% filter(Fiscal_YQ<=2024 & SpaceParentID=="SPACEX"),
                     filename="space_vend_cust",xlsx="SpaceX_Trends.xlsx",
                     sheet="VendCust",path="../output/Space/", height=4.25,width=8,
         excel_y_var=TRUE,excel_formula=TRUE,
                     startRow=1,startCol=12,format=TRUE,
         output_slide_png = TRUE, slide_size=16 ,var_list=c("SpaceParentID","Customer")
...
levels(launch_plus$SpaceParentID.sum)<-
 ...
 "Other Residual"=c("WYLE LABORATORIES","CALIFORNIA INSTITUTE OF TECHNOLOGY","JOHNS HOPKINS UNIVERSITY"   ,
                      "MAXAR TECHNOLOGIES" , "MDA" ,"LORAL SPACE","Other Residual","LOCKHEED MARTIN")
  )
...
### Area Commercial
```{r Launch}
launch_plus$AnyCommercialText<-factor(launch_plus$AnyCommercial)
levels(launch_plus$AnyCommercialText)<-list(
  "Not Classified\nas Commercial"="N",
  "Non-development\nor Commercial Similar"= "NonDev",
  "Any Commercial\nClassification"="Y"
...
LaunchCommArea<-build_plot(
  ...
  y_var="Action_Obligation_OMB25_GDP23", #VAR.y.variable
  color_var="AnyCommercialText", #color_var
  facet_var="SpaceAreaEELV", #facet_var
  alpha_var="YTD",
  column_key=launch_ck,
  format=TRUE,
  ytextposition=FALSE
  ...
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2023 $s)",title="Contract Obligations for Launch Providers and Related R&D,\nby Commercial Contracting FY 2007-FY 2023",x="Fiscal Year",
       caption="Note: The EELV and Russia Space Agency are entirely included, all other vendors are limited to launch product or service codes.\nNGC completed acquisition of Orbital ATK in June 2018, which is reflected in FY 2019.\nSource: FPDS and CSIS analysis")+
  date_x_year_breaks(2007,2023,3#,partial_year=2024,partial_label="\nQ1")
))
...
summary(factor(launch$SpaceParentID))
write_csv(file="Boeing_unique_contract_award_keys.csv",
launch %>% filter(SpaceParentID=="BOEING")%>%
  group_by(ParentID,contract_award_unique_key,PricingMechanism,
                                                       ProductOrServiceCodeText,
                                                       CompetitionClassification,
                                                       ClassifyNumberOfOffers,
                                                       anycommercial,Vehicle)%>%
...
LaunchComm<-build_plot(
  ...
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2023 $s)",title="Contract Obligations for 10 Major Launc Providers,\nby Commercial Contracting FY 2007-FY 2023",x="Fiscal Year",
       caption="Note: The EELV and Russia Space Agency are entirely included, all other vendors are limited to launch product or service codes.\nNGC completed acquisition of Orbital ATK in June 2018, which is reflected in FY 2019.\nSource: FPDS and CSIS analysis")+
  date_x_year_breaks(2007,2023,3#,partial_year=2024,partial_label="\nQ1")
))
...
log_plot(plot=LaunchCommVar, df=launch_plus %>% filter(Fiscal_YQ<=2024 & Fiscal_YQ>=2007),
