...
launch_residual<-launch_fedpsc[,colnames(launch_fedpsc) %in% colnames(launch)]
...
launch_join<-launch[,colnames(launch) %in% colnames(launch_fedpsc)]
...
launch_residual<-full_join(launch_residual,launch_join)
...
if(sum(launch_residual$Action_Obligation_Then_Year,na.rm=TRUE)!=sum(launch$Action_Obligation_Then_Year,na.rm=TRUE) |
   sum(launch_residual$Action_Obligation_OMB25_GDP23,na.rm=TRUE)!=sum(launch$Action_Obligation_OMB25_GDP23,na.rm=TRUE)
   )
  stop("Checksum error")
...
if(sum(launch_residual$Total_Action_Obligation_Then_Year,na.rm = TRUE)!=
   ...
   sum(launch_residual$Total_Action_Obligation_OMB25_GDP23,na.rm = TRUE)!=
   sum(launch_fedpsc$Action_Obligation_OMB25_GDP23,na.rm = TRUE) |
   sum(launch_residual$Action_Obligation_Then_Year,na.rm=TRUE)!=sum(launch$Action_Obligation_Then_Year,na.rm=TRUE) |
   sum(launch_residual$Action_Obligation_OMB25_GDP23,na.rm=TRUE)!=sum(launch$Action_Obligation_OMB25_GDP23,na.rm=TRUE)
   )
  stop("Checksum error")
...
sum(launch_residual$Total_Action_Obligation_OMB25_GDP23[launch$SpaceArea %in% c("R&D (Space Flight)",
                                                                "Space Transp. and Launch",
...
sum(launch_plus$Action_Obligation_OMB25_GDP23[launch$SpaceArea %in% c("R&D (Space Flight)",
                                                                "Space Transp. and Launch",
                                                                "Space Vehicle Launchers")],na.rm = TRUE)
...
sum(launch$Action_Obligation_OMB25_GDP23[launch$SpaceArea %in% c("Space Transp. and Launch",
                                                                "Space Vehicle Launchers")],na.rm = TRUE)

sum(launch_fedpsc$Action_Obligation_OMB25_GDP23[launch_fedpsc$SpaceArea %in% c(
                                                                "Space Transp. and Launch",
                                                                "Space Vehicle Launchers")],na.rm = TRUE)
```
...
SpaceArea<-build_plot(
  ...
  labs(y="Obligations (Constant 2023 $s)",title="Contract Obligations for Selected Launch Providers and Researches,\nby Area FY 2007-FY 2023",x="Fiscal Year",
...
SpaceAreaSum<-build_plot(
  ...
  theme(legend.position = "right")+guides(fill=guide_legend(ncol=1))+facet_wrap(~SpaceParentID.sum,nrow=3,
                                                                                 scale="free_y")+
  labs(y="Obligations (Constant 2023 $s)",title="Contract Obligations for Selected Major Launch Providers,\nby Area FY 2007-FY 2023",x="Fiscal Year",
       caption="Note: The EELV and Russia Space Agency are entirely included, all other vendors are limited to launch product or service codes.\nSource: FPDS and CSIS analysis")
)
