...
summary(factor(platpscintldef))
summary(platpscintldef$manufacturingorganizationtype)

# undebug(format_data_for_plot)
  (fpds_vendor_intl_place <-  build_plot(data=platpscintldef %>% filter(Fiscal_Year>=2005), #%>% filter((PlaceIsForeign==1|IsFMS==TRUE)&!is.na(PlaceIsForeign)&
                                                                #        !is.na(IsFMS)& Fiscal_Year>2011),
                                         #IsFMS==TRUE 
                                         chart_geom="Bar Chart",
                              x_var="Fiscal_Year",
                              y_var="Action_Obligation_OMB25_GDP23",
                              color_var="manufacturingorganizationText",
                              # facet_var="manufacturingorganizationtype",
                              # second_var="IsFMS",
                              labels_and_colors=intl_lc,
                              column_key=intl_ck,
                              legend=TRUE,
                              caption=FALSE,
                              format=TRUE)+theme(legend.position = "right")#+
      # facet_grid( .~manufacturingorganizationtype )#+labs(title="FMS or Performed Internationally, Including JSF")#, scales="free_y"
    )

platpscintldef$OriginIsForeign
# undebug(format_data_for_plot)
  (fpds_vendor_intl_place <-  build_plot(data=platpscintldef %>% filter(Fiscal_Year>=2012 & ParentHQisForeign==1
                                                                        &!is.na(PlaceIsForeign) & !is.na(OriginIsForeign)), #%>% filter((PlaceIsForeign==1|IsFMS==TRUE)&!is.na(PlaceIsForeign)&
                                                                #        !is.na(IsFMS)& Fiscal_Year>2011),
                                         #IsFMS==TRUE 
                                         chart_geom="Bar Chart",
                              x_var="Fiscal_Year",
...
# undebug(format_data_for_plot)
(fpds_vendor_intl_place <-  build_plot(data=platpscintldef %>% filter((PlaceIsForeign==1|IsFMS==TRUE|
                                                                         )&!is.na(PlaceIsForeign)&
                                                                      !is.na(IsFMS)& Fiscal_Year>2011),
                                       #IsFMS==TRUE 
                                       chart_geom="Bar Chart",
                            x_var="Fiscal_Year",
