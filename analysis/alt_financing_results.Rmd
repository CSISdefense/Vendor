...
agency_list<-data.frame(
  ...
  yaxis=c("Commitment Level (2022 $s)","Gross Amount or Instance Contract (2022 $s)","Approved or Declined Amount (2022 $s)","Loan Amount (2022 $s)"),
...
for(i in 1:length(agency_list$agency)){
  ...
  (Breakout<-build_plot(
    ...
    ytextposition=FALSE
      ...
      labs(y=agency_list$yaxis[i],
           ...
           title=paste(agency_list$agency[i],"Financing to Critical Technology Areas by Area"))+
      coord_flip()
  )
  # 
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Crit",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_text = FALSE,
...
```{r FinancingStatus}
  ...
  agency_assist<-read_and_join_experiment(agency_assist,directory="assistance//",lookup_file="FinancingStatus.csv",
                               by=c("program","FinancingStatus"),add_var="FinancingStatus.sum",
...
overall<-build_plot(
  ...
  labs(y="Gross Amount or Instance Contract",
       x="Financing Status",
           paste(agency_list$agency[i],"Financing to Critical Technology Areas by Status"))+
    coord_flip()# (Constant 2022 $s)
)
...
for(i in 1:length(agency_list$agency)){
  ...
  (Breakout<-build_plot(
    ...
    ytextposition=FALSE
      ...
      labs(y=agency_list$yaxis[i],
           ...
           x="Financing Status",
           title=paste(agency_list$agency[i],"Financing to Critical Technology Areas\nby Status"))+
      coord_flip()
  )
  # 
  if(!dir.exists(file.path("../output/Assistance/",file_c)))
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Status",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_text = FALSE,
           sheet="Status",path=file.path("../output/Assistance/",file_c),second_path=file.path(sharepoint_log_path,file_c), height=3.5,
           startRow=1,startCol=13,format=TRUE,#,var_list=c("PricingUCA.sumlong","PricingUCA"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
...
overall<-build_plot(
  ...
  labs(y="Gross Amount or Instance Contract",
       x="Financing Mechanism",
           title=paste("Financing to Critical Technology Areas\nby Mecahnism"))+
    coord_flip()# (Constant 2022 $s)
)
...
for(i in 1:length(agency_list$agency)){
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Type",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_text = FALSE,
           sheet="Type",path=file.path("../output/Assistance/",file_c),second_path=file.path(sharepoint_log_path,file_c), height=3.5,
           startRow=1,startCol=13,format=TRUE,#,var_list=c("PricingUCA.sumlong","PricingUCA"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
