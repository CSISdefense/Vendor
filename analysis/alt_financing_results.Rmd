...
for(i in 1:length(agency_list$agency)){
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Crit",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_doc_svg_text = FALSE,
...
overall_status<-build_plot(
  data=agency_assist  %>%  filter(AnyTag==TRUE & Agency!="DFC"),
  chart_geom = "Bar Chart",
  share = FALSE,
   labels_and_colors=aa_lc,
  column_key=aa_ck,
  ...
  labs(y="Gross Amount or\nInstance Contract",
       x="Financing Status",
           title="Financing to Critical Technology\nSectors by Status")+
    coord_flip()+facet_wrap(~Agency,scale="free_x",nrow=1)
    
)

  log_plot(plot=overall_status, df=agency_assist  %>%  filter(AnyTag==TRUE & Agency!="DFC"),#Fiscal_YQ<=2023.2&
           filename="status_overall",xlsx=paste("Overall_Assistance.xlsx",sep="_"),suppress_doc_svg_text = FALSE,
           sheet="Status",path=file.path("../output/Assistance/"),second_path=file.path(sharepoint_log_path), height=3.5,
           format=TRUE,#,var_list=c("PricingUCA.sumlong","PricingUCA"),
           output_doc_png=TRUE,excel_y_var=TRUE,output_slide_png=TRUE,slide_width=6,
           slide_size=16
  )  
...
for(i in 1:length(agency_list$agency)){
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Status",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_doc_svg_text = FALSE,
...
for(i in 1:length(agency_list$agency)){
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Type",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),suppress_doc_svg_text = FALSE,
