...
InnovEnergyAward<-award_summary(loanSelected %>% filter(awarding_agency_name =="Department of Energy"))
InnovEnergyRecip<-recipient_summary(loanSelected %>% filter(awarding_agency_name =="Department of Energy"))
view(InnovEnergyRecip %>% arrange(recipient_name))
write.csv(file=file.path("..","data","semi_clean","OSC","uei_innov_energy.csv"),InnovEnergyRecip%>% arrange(recipient_name),
          row.names = FALSE)


wb<-loadWorkbook(file=file.path("..","Output","OSC","selected_assistance_critical_tech.xlsx"))
writeData(wb, sheet="InnEngy-USAS",InnovEnergyAward)
...
write.csv(file=file.path("..","Output","OSC","critical_tech_csv",
                         "usas_innovative_energy_award.csv"),InnovEnergyAward,
          row.names = FALSE)
