...
agency_list<-data.frame(
  agency=c("SBA","EXIM"),
  source=c("SBA FOIA datasets","EXIM dataset"),
  yaxis=c("Gross Amount or Instance Contract","Approved or Declined Amount")
)
```
...
award_summary<-function(df){
  ...
  df<-df %>% group_by(assistance_award_unique_key) %>%
    ...
    df<-read_and_join_experiment(df,
               ...
               energy_innovation_category=stringi::stri_trans_nfc(energy_innovation_category),
...
project_summary<-function(df){
  ...
  if(any(is.na(test)&!is.na(df$Issuance_Date))){
    ...
    df$IssuanceYear<-year(df$Issuance_Date)
...
agency_assist<-
  rbind(sba.504 %>% mutate(StartFiscalYear=ApprovalFiscalYear,
       ...
       exim %>% mutate(StartFiscalYear=Fiscal.Year,
                           ...
                           program="EXIM") %>%
          dplyr::select(StartFiscalYear, Amount,CriticalTech,Agency,program,AnyTag),
        LPOproject %>% mutate(StartFiscalYear=Fiscal.Year,
                           Amount=Approved.Declined.Amount,
                           Agency="EXIM",
                           program="EXIM") %>%
          dplyr::select(StartFiscalYear, Amount,CriticalTech,Agency,program,AnyTag)
...
(
sba_crosscut<-build_plot(
  data=sba_unified  %>% filter(!is.na(StartFiscalYear)) %>% filter(AnyTag==TRUE),
  chart_geom = "Bar Chart",
  share = FALSE,
  # labels_and_colors=loan_lc,
  # column_key=loan_ck,
...
undebug(log_plot)
for(c in agency_list$agency){
  file_c<-gsub("__*","_",gsub("&","and",gsub("[ |,]","_",c)))
  (Breakout<-build_plot(
    data=agency_assist %>% filter(AnyTag==TRUE&Agency==c),#& Fiscal_YQ<=2023.2 Fiscal_Year>=2000& 
    chart_geom = "Bar Chart",
    share = FALSE,
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==c),#Fiscal_YQ<=2023.2&
