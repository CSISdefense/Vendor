...
(
overall<-build_plot(
  data=agency_assist  %>% filter(!is.na(StartFiscalYear)) %>% filter(AnyTag==TRUE & !is.na(PartnerRegion)),
  chart_geom = "Bar Chart",
  share = FALSE,
   labels_and_colors=aa_lc,
  column_key=aa_ck,
...
for(i in 1:length(agency_list$agency)){
  if(!any(
    agency_assist %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]&!is.na(AnyTag)&!is.na(PartnerRegion))
  ))
    next
