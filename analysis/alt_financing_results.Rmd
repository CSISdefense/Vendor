...
load("..\\Data\\clean\\OSC\\SelectedLoanDataSet.rda")
load(file.path("..","data","Clean","OSC","sba_programs.rda"))
load(file.path("..","data","Clean","OSC","dfc.rda"))
load(file.path("..","data","Clean","OSC","exim.rda"))

agency_list<-data.frame(
  agency=c("DFC","SBA","EXIM","DOE"),
  source=c("DFC.gov","SBA FOIA datasets","EXIM dataset","LPO Portfolio Projects and FAADS"),
  yaxis=c("Commitment Level (then year)","Gross Amount or Instance Contract (2022 $s)","Approved or Declined Amount (2022 $s)","Loan Amount (2022 $s)")
)
...
# write.csv(file=file.path("..","data","semi_clean","OSC","uei_innov_energy.csv"),LPOrecip%>% arrange(recipient_name),
#           row.names = FALSE)
...
writeData(wb, sheet="DOE-LPO",LPOproject %>% 
...
writeData(wb, sheet="DOE-LPO-long",LPOproject)
...
agency_assist<-
  rbind(sba.504 %>% mutate(StartFiscalYear=ApprovalFiscalYear,
        ...
        dfc %>% mutate(StartFiscalYear=Year,
          ...
          dplyr::select(StartFiscalYear, Amount,Agency,program,
                        FinancingType,FinancingStatus,CriticalTech,AnyTag,State,PartnerCountry)
  ) %>% group_by(StartFiscalYear, Agency,program,FinancingType,FinancingStatus,CriticalTech,AnyTag,
...
sba_crosscut<-build_plot(
  ...
  theme(legend.position = "right")+
  labs(y="Gross Amount or Instance Contract")# (Constant 2022 $s)
)
...
for(i in 1:length(agency_list$agency)){
  ...
  (Breakout<-build_plot(
    data=agency_assist %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#& Fiscal_YQ<=2023.2 Fiscal_Year>=2000& 
    chart_geom = "Bar Chart",
    share = FALSE,
  ...
  # 
  if(!dir.exists(file.path("../output/OSC/",file_c)))
    dir.create(file.path("../output/OSC",file_c))
  
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Crit",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),
           sheet="Crit",path=file.path("../output/OSC/",file_c),second_path=sharepoint_log_path,
           height=3.5,
...
annual<-build_plot(
  ...
  # alpha_var="YTD",  #x_var 
  y_var="Amount_OMB24_GDP22", #VAR.y.variable
  color_var="FinancingStatus", #color_var
  facet_var="program", #facet_var
  format=TRUE,
  ytextposition=FALSE
)+#date_x_year_breaks(2000,2020,3,partial_year=2023,partial_label="\nQ1-Q2")+scale_alpha_ordinal(range=c(1,0.5))+
...
for(i in 1:length(agency_list$agency)){
  c<-agency_list$agency[i]
  if(all(is.na(agency_assist$FinancingStatus[agency_assist$Agency==agency_list$agency[i]])))
  ...
  (Breakout<-build_plot(
    ...
    # labels_and_colors=def_lc,
    # NA, #VAR.ncol
    x_var="FinancingStatus",#alpha_var="YTD", #x_var
    y_var="Amount_OMB24_GDP22", #VAR.y.variable
    color_var="program", #color_var
    # facet_var="PlatformPortfolio", #facet_var
    # column_key=def_ck,
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Status",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),
           sheet="Status",path=file.path("../output/OSC/",file_c),second_path=sharepoint_log_path, height=3.5,
           startRow=1,startCol=13,format=TRUE,#,var_list=c("PricingUCA.sumlong","PricingUCA"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
...
annual<-build_plot(
  ...
  # alpha_var="YTD",  #x_var 
  y_var="Amount_OMB24_GDP22", #VAR.y.variable
  color_var="FinancingType", #color_var
  facet_var="program", #facet_var
  format=TRUE,
  ytextposition=FALSE
)+#date_x_year_breaks(2000,2020,3,partial_year=2023,partial_label="\nQ1-Q2")+scale_alpha_ordinal(range=c(1,0.5))+
...
for(i in 1:length(agency_list$agency)){
  c<-agency_list$agency[i]
  if(all(is.na(agency_assist$FinancingType[agency_assist$Agency==agency_list$agency[i]])))
    next #Skip when the core data is missing.
  file_c<-gsub("__*","_",gsub("&","and",gsub("[ |,]","_",agency_list$agency[i])))
  (Breakout<-build_plot(
    data=agency_assist %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#& Fiscal_YQ<=2023.2 Fiscal_Year>=2000& 
    ...
    # labels_and_colors=def_lc,
    # NA, #VAR.ncol
    x_var="FinancingType",#alpha_var="YTD", #x_var
    y_var="Amount_OMB24_GDP22", #VAR.y.variable
    color_var="program", #color_var
    # facet_var="PlatformPortfolio", #facet_var
    # column_key=def_ck,
  ...
  log_plot(plot=Breakout, df=agency_assist  %>% filter(AnyTag==TRUE&Agency==agency_list$agency[i]),#Fiscal_YQ<=2023.2&
           filename=paste(file_c,"Type",sep="_"),xlsx=paste(file_c,"Assistance.xlsx",sep="_"),
           sheet="Type",path=file.path("../output/OSC/",file_c),second_path=sharepoint_log_path, height=3.5,
           startRow=1,startCol=13,format=TRUE,#,var_list=c("PricingUCA.sumlong","PricingUCA"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
