...
ToplinePricingLine<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  ggsave600dpi(path="..//Output//AcqTrends//",filename="Topline_PricingUCA_Share.png", ToplinePricingLine,  
               width=6.5, height= 3, units="in",size=12, lineheight=1.2
               )
  
  
  ggsave600dpi(path="..//Output//AcqTrends//",filename="Topline_PricingUCA_Share.svg", ToplinePricingLine,  
               width=12, height= 5.5, units="in",size=, lineheight=1.2
               )
...
RnDPricing<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  ggsave600dpi(path="..//Output//AcqTrends//",filename="RnD_PricingUCA.png", RnDPricing,  
               width=6.5, height= 3, units="in",size=12, lineheight=1.2
               )
  
  
  ggsave600dpi(path="..//Output//AcqTrends//",filename="RnD_PricingUCA.svg", RnDPricing,  
               width=12, height= 5.5, units="in",size=, lineheight=1.2
               )
...
RnDpricingHistory<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  ggsave600dpi(path="..//Output//AcqTrends//",filename="RnD_PricingUCA_Share.png", RnDpricingHistory,  
               width=6.5, height= 3, units="in",size=12, lineheight=1.2
               )
  
  
  ggsave600dpi(path="..//Output//AcqTrends//",filename="RnD_PricingUCA_Share.svg", RnDpricingHistory,  
               width=12, height= 5.5, units="in",size=, lineheight=1.2
               )

log_plot(plot=RnDpricingHistory, df=def_data  %>% filter(Fiscal_Year<2025&Fiscal_Year>=2000 & SimpleArea=="R&D"),
                     ...
                     format=TRUE,var_list=c("PricingUCA.sum"),
         excel_y_var =TRUE,
         excel_share =TRUE
         )
...
ota_merge<-ota_def%>%filter(Fiscal_Year >=2015) %>%
  group_by(Fiscal_Year,SimpleArea,YTD)%>%
  dplyr::summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23),
                   Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year)
                   )
...
write.csv(file="..//Output//AcqTrends//contractota_RnDPhase.csv",row.names = FALSE, na = "",
           RnDphase$data %>% mutate(Fiscal_Year=lubridate::year(dFYear)) %>%
          pivot_wider(id_cols=c(ProductServiceOrRnDarea,Authority),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB25_GDP23)%>%
...
cust_covid<-def_data %>% filter(Fiscal_YQ<=2025.2) %>% group_by(SubCustomer.JPO,SubCustomer.sum,dFYear,Fiscal_Year) %>%
...
write.csv(file="..//Output//AcqTrends//subcustomer_covid.csv",row.names = FALSE, na = "",
           SubCustomerCovid$data %>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
