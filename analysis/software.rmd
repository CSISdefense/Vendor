...
label_SWpsc<-function(x){
  ...
  # x$IsPossibleSoftwareEngineering[x$ProductOrServiceCode %in% 
  # c("7010","7020","7021","7022","7025")]<-0
...
write.csv(platpscintldef %>% filter(Fiscal_Year>=2000 & !is.na(IsPossibleSoftwareEngineering)) %>%
  group_by(IsPossibleSoftwareEngineering,AsAServiceLaborProduct,ProductOrServiceCode,ProductOrServiceCodeText,Fiscal_Year)%>% summarise(o=sum(Action_Obligation_OMB24_GDP22))%>%#/1000000000
    arrange(Fiscal_Year)%>%pivot_wider(id_cols=c(ProductOrServiceCode,ProductOrServiceCodeText,IsPossibleSoftwareEngineering,AsAServiceLaborProduct),
                names_from=Fiscal_Year,values_from=o)%>%arrange(IsPossibleSoftwareEngineering,AsAServiceLaborProduct,ProductOrServiceCode,ProductOrServiceCodeText)
...
(
SWpscdetail<-build_plot(
  data=platpscintldef %>% filter(Fiscal_Year>=2002 & ProductServiceOrRnDarea!="Unlabeled" & !is.na(IsPossibleSoftwareEngineering) & IsPossibleSoftwareEngineering!="Other Electrical and Light")%>%
    mutate(IsPossibleSoftwareEngineering=factor_wrap(IsPossibleSoftwareEngineering)),
  chart_geom = "Bar Chart",
  share = FALSE,
  # labels_and_colors=sw_lc,
  # NA, #VAR.ncol
 ...
 #             )+labs(title=NULL,
  ...
  # color="Competition")+
  theme(legend.position = "right")+
  labs(caption="Note: Other electric and lighting not shown. Source: FPDS; CSIS analysis.",
       y="Defense Prime Contract Obligations (2022 $s)",x="Fiscal Year")+
    facet_wrap(~IsPossibleSoftwareEngineering, ncol=3)
)
