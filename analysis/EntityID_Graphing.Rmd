...
log_plot(plot=all_10k_small_bar+labs(title=NULL)+theme(legend.position="right"), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                     ...
                     filename="nps_fig09_def_vendor_count",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                     sheet="9 DIB count",path="../output/AcqTrends/NPS2024/", 
...
ggsave600dpi(all_10k_small_line+labs(title=NULL),file="nps_fig09a_vendor_count_line.svg",size=12,lineheight = 1.2,
             height=3,width=6.5, path="../output/AcqTrends/NPS2024/", 
         second_path=online_path)
...
def_rpuh %>% dplyr::filter( Fiscal_Year>=2007 & Fiscal_Year<2024)  %>% group_by(LargestContract2018dollars) %>% summarise(DefenseObligated_OMB25_GDP23=sum(DefenseObligated_OMB25_GDP23))
  ...
  log_plot(plot=all_10k_small_dollar+labs(title=NULL), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                            & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig10_def_vendor_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="DIB dollars",path="../output/AcqTrends/NPS2024/", height=2.5,
           excel_formulas=TRUE,
                       startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
           output_doc_png = TRUE
           )
...
#                      sheet="DIB count",path="../output/AcqTrends/NPS2024/", 
#          second_path=online_path,
#          height=3.5,
#          csv_then_year=FALSE  ,excel_then_year=FALSE,excel_y_var=TRUE,
#            output_doc_png = TRUE,
...
rpuh_ck<-get_column_key(def_rpuh)
  ...
  #                      sheet="8 NTDC",path="../output/AcqTrends/NPS2024/",
  #          height=3.5,
  #          excel_formulas=TRUE,
  #                      startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
  #          output_doc_png = TRUE
...
(
  ...
  log_plot(plot=ntdc_label+theme(legend.position = "right"),
           df=cas %>% dplyr::filter(
                                            Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig11a_ntdc_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="11 NTDC",path="../output/AcqTrends/NPS2024/",
           height=3.5,
...
```{r def_rpuh_hhi}
  ...
  log_plot(plot=def_rpuh_hhi+labs(title=NULL),
           df=def_rpuh %>% dplyr::filter(
                                            Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_figXX_hhi",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="XX hhi",path="../output/AcqTrends/NPS2024/",
           height=2,
