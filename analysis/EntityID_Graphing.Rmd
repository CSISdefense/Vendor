...
def_rpuh %>% dplyr::filter( Fiscal_Year>=2007 & Fiscal_Year<2024)  %>% group_by(LargestContract2018dollars) %>% summarise(DefenseObligated_OMB25_GDP23=sum(DefenseObligated_OMB25_GDP23))
  ...
  all_10k_small_dollar<-build_plot(
    ...
    format=TRUE
         ...
         caption="Note: Threshold values are adjusted for inflation (in 2018 $s).\nSource: FPDS Unique Entity (UEI) reporting and CSIS analysis.")+theme(legend.position = "right")#guides(fill=guide_legend(nrow=2))
  )
...
cas <- cas %>%
  mutate(
    ...
    TRUE ~ "Variably Small / Larger\nand Largest Contract\nOver $7.5 M"),
    cas_label=case_when(
      CostAccountingStandardsClause=="Y"~"Cost Accounting Standards",
  ...
  TRUE~"Cost Accounting Standards")
...
(
  ntdc_label<-build_plot(
    data=cas %>% filter (Fiscal_Year>=2011& Fiscal_Year<2024), #%>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          # & Fiscal_Year>=2007& Fiscal_Year<2024), ##where is FPDS summary?
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="Fiscal_Year",
