...
log_plot(plot=all_10k_small_bar+theme(legend.position="right"), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                     ...
                     filename="nps_fig09_def_vendor_count",xlsx="DoD_2024_NPS.xlsx",
...
log_plot(plot=all_10k_small_dollar, df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                                                  & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
         second_path=online_path,
         filename="nps_fig10_def_vendor_obl",xlsx="DoD_2024_NPS.xlsx",
         sheet="10 DIB dollars",path="../output/AcqTrends/NPS2024/", height=2.5,
         excel_formulas=TRUE,
         startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
...
#                                           & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
#                      filename="def_vendor_count",xlsx="DoD_2024_NPS.xlsx",
...
rpuh_ck<-get_column_key(def_rpuh)
  ...
  #                                           Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
  #          second_path=online_path,
  #                      filename="nps_fig8_ntdc_obl",xlsx="DoD_2024_NPS.xlsx",
...
(
  ...
  log_plot(plot=ntdc_label+theme(legend.position = "right"),
           df=cas %>% dplyr::filter(
                                            Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig11_ntdc_obl",xlsx="DoD_2024_NPS.xlsx",
...
```{r def_rpuh_hhi}
  ...
  log_plot(plot=def_rpuh_hhi+labs(title=NULL),
           df=def_rpuh %>% dplyr::filter(
                                            Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_figXX_hhi",xlsx="DoD_2024_NPS.xlsx",
...
(eplat<-build_plot(ecp %>% filter(Fiscal_Year>=2005 &
           ...
           format=TRUE)+labs(x="Fiscal Year",y="Vendor Count")+
   labs(color="Vendor receives over $1 million threshold in 2016 $s")+
   labs(title="DOD Contractor Count, 2005-2020",x="Fiscal Year"
...
(eplatbar<-build_plot(ecp %>% filter(Fiscal_Year>=2005 &
           ...
           # second_var="",
           labels_and_colors = ecp_lc,
           format=TRUE)+ labs(title="DOD Contractor Count, 2005-2020",x="Fiscal Year"
        ,y="Number of Entities with Contracts\nValued over $10k (2018 Constant $s)",
        caption="Notes: Some entities appear in multiple categories. Entities with unlabeled platforms not shown.
        Source: FPDS including Duns & Bradstreet identifiers, CSIS roll-ups and analysis.")+
...
(eplatline<-build_plot(ecp %>% filter(Fiscal_Year>=2005 &
           ...
           # second_var="",
           labels_and_colors = ecp_lc,
           format=TRUE)+facet_wrap(~PlatformPortfolio,ncol=6)+scale_x_date("Fiscal Year",labels = scales::label_date("'%y"),date_breaks = "3 years")+ labs(title="DOD Contractor Count, 2005-2020",x="Fiscal Year"
        ,y="Number of Entities with Contracts\nValued over $10k (2018 Constant $s)",
        caption="Notes: Some entities appear in multiple categories. Entities with unlabeled platforms not shown.
        Source: FPDS including Duns & Bradstreet identifiers, CSIS roll-ups and analysis.")
)
...
(eplat<-build_plot(ecp %>% filter(Fiscal_Year>=2005 &
           ...
           # labels_and_colors = ecp_lc,
           format=TRUE)+labs(x="Fiscal Year",y="Vendor Count")+ labs(title="DOD Contractor Count, 2005-2020",x="Fiscal Year"
        ,y="Number of Entities with Contracts\nValued over $10k (2018 Constant $s)",
        caption="Notes: Some entities appear in multiple categories. Entities with unlabeled platforms not shown.
        Source: FPDS including Duns & Bradstreet identifiers, CSIS roll-ups and analysis.")
)#+facet_wrap(~PlatformPortfolio,scale="free_y")
