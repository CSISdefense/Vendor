...
log_plot(plot=all_10k_small_bar+labs(title=NULL), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
                     filename="def_vendor_count",xlsx="CSIS_DoD_DIB_Trends.xlsx",second_path=online_path,
                     sheet="DIB count",path="../output/AcqTrends/", height=3.5,
         csv_then_year=FALSE  ,excel_then_year=FALSE,excel_y_var=TRUE,
         excel_formulas = TRUE,
                     startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
         output_doc_png = TRUE,
...
ggsave600dpi(all_10k_small_line+labs(title=NULL),file="def_vendor_count_line.png",size = 12,height=3.5,width=6.5,
             path="../output/AcqTrends/",lineheight = 1.2,second_path=online_path)
  ...
  log_plot(plot=all_10k_small_dollar+labs(title=NULL), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                            & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="def_vendor_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="DIB dollars",path="../output/AcqTrends/", height=2.75,
           excel_formulas=TRUE,
                       startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
           output_doc_png = TRUE
           )
...
colnames(cas)
  ...
  build_plot(
    ...
    color_var="costorpricingdata",       # name of coloration variable, as string
    facet_var="CostAccountingStandardsClause",        # name of facet variable, as string
    # second_var="CostAccountingStandardsClause",
    legend=TRUE, #Include a legend
    caption=TRUE, #Include a source caption
    # labels_and_colors=rpuh_lc,
    # column_key=rpuh_ck,
...
cas<- cas %>% mutate(cas_label=case_when(
  ...
  IsAbove2018constantCommercialItem7500k==0~"Vendor Size or Largest Contract",
