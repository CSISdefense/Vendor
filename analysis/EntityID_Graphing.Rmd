...
```{r def_rpuh}
  ...
  all_10k_small_bar<-build_plot(
    data=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2014& Fiscal_Year<2024), ##where is FPDS summary?
...
log_plot(plot=all_10k_small_bar, df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2014& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
...
log_plot(plot=all_10k_small_bar, df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
         ...
         excel_formulas = FALSE,
                     startRow=1,startCol=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
         output_doc_png = TRUE,
         include_YTD = FALSE,
         num_format = "0.00,\"K\""
         )
...
ggsave600dpi(all_10k_small_line,file="def_vendor_count_line.png",size = 12,height=3.5,width=6.5,
             path="../output/AcqTrends/",lineheight = 1.2)

  (
  all_10k_small_dollar<-build_plot(
    data=def_rpuh %>% dplyr::filter( Fiscal_Year>=2014 & Fiscal_Year<2024), ##where is FPDS summary?
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="Fiscal_Year",
...
#                      sheet="DIB dollars",path="../output/AcqTrends/", height=3.5,
#                      startRow=1,startCol=3,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
#          output_doc_png = TRUE
#          )
...
```{r def_rpuh}
  ...
  all_10k_small_bar<-build_plot(
    ...
    x_var="Fiscal_Year",
    y_var="count", #Name of variable to plot on y-axis
    color_var="IsEntityAbove2018constantCostAccounting2000kThreshold",       # name of coloration variable, as string
    facet_var="AlwaysIsSmallLabel",        # name of facet variable, as string
