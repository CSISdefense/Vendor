...
```{r def_rpuh_uei}
  ...
  all_10k_small_bar<-build_plot(
    data=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2007& Fiscal_Year<2024), ##where is FPDS summary?
    ...
    format=TRUE
         ...
         title="Number of Defense Contractors by Annual Small Business\nStatus and Size of Largest Federal Contract",
         caption="Note: Threshold values are adjusted for inflation (in 2018 $s).\nSource: FPDS Unique Entity (UEI) reporting and CSIS analysis.")+
    date_x_year_breaks(2007,2023,4)+date_x_year_breaks(2007,2023,4)
  )


log_plot(plot=all_10k_small_bar+labs(title=NULL)+theme(legend.position="right"), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
                     filename="def_vendor_count",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                     sheet="DIB count",path="../output/AcqTrends/", 
...
(
  all_10k_small_line<-build_plot(
    ...
    format=TRUE
         ...
         title="Number of Defense Contractors by Annual Small Business\nStatus and Size of Largest Federal Contract",
         caption="Note: Threshold values are adjusted for inflation (in 2018 $s).\nSource: FPDS Unique Entity (UEI) reporting and CSIS analysis.")+date_x_year_breaks(2007,2023,4)
  )


ggsave600dpi(all_10k_small_line+labs(title=NULL),file="nps_fig6_vendor_count_line.svg",size=12,lineheight = 1.2,
             height=3,width=6.5, path="../output/AcqTrends/", 
         second_path=online_path)

def_rpuh %>% dplyr::filter( Fiscal_Year>=2007 & Fiscal_Year<2024)  %>% group_by(LargestContract2018dollars) %>% summarise(DefenseObligated_OMB25_GDP23=sum(DefenseObligated_OMB25_GDP23))
  (
  all_10k_small_dollar<-build_plot(
    data=def_rpuh %>% dplyr::filter( Fiscal_Year>=2007 & Fiscal_Year<2024), ##where is FPDS summary?
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="dFYear",
    ...
    format=TRUE
         ...
         title="Defense Obligations by Annual Contractor Small Business\nStatus and Size of Largest Federal Contract",
         caption="Note: Threshold values are adjusted for inflation (in 2018 $s).\nSource: FPDS Unique Entity (UEI) reporting and CSIS analysis.")+date_x_year_breaks(2007,2023,4)+
      theme(legend.position = "right")#guides(fill=guide_legend(nrow=2))
  )
  
  log_plot(plot=all_10k_small_dollar+labs(title=NULL), df=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                            & Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig7_def_vendor_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="DIB dollars",path="../output/AcqTrends/", height=2.5,
           excel_formulas=TRUE,
                       startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
           output_doc_png = TRUE
           )
...
```{r def_rpuh_uei}
  ...
  ntdc_count_bar<-build_plot(
    ...
    format=TRUE
         ...
         title="Number of Defense Contractors by Annual Small Business\nStatus and Size of Largest Federal Contract",
         caption="Note: Threshold values are adjusted for inflation (in 2018 $s).\nSource: FPDS Unique Entity (UEI) reporting and CSIS analysis.")+
    date_x_year_breaks(2007,2023,4)+date_x_year_breaks(2007,2023,4)+facet_wrap(~IsEntityTraditionalLabel, scales="free_y")
  )
...
# ggsave600dpi(ntdc_count_line+labs(title=NULL),file="nps_fig6_vendor_count_line.svg",size=12,lineheight = 1.2,
#              height=3,width=6.5, path="../output/AcqTrends/", 
#          second_path=online_path)
# 
# def_rpuh %>% dplyr::filter( Fiscal_Year>=2007 & Fiscal_Year<2024)  %>% group_by(LargestContract2018dollars) %>% summarise(DefenseObligated_OMB25_GDP23=sum(DefenseObligated_OMB25_GDP23))
...
#            second_path=online_path,
#                        filename="nps_fig7_def_vendor_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
#                        sheet="DIB dollars",path="../output/AcqTrends/", height=3,
#            excel_formulas=TRUE,
#                        startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
#            output_doc_png = TRUE
#            )
...
```{r def_rpuh}
  ...
  all_10k_small_bar<-build_plot(
    data=def_rpuh %>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          & Fiscal_Year>=2011& Fiscal_Year<2024), ##where is FPDS summary?
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="Fiscal_Year",
    y_var="count", #Name of variable to plot on y-axis
  ...
  (
  all_10k_small_dollar<-build_plot(
    data=def_rpuh %>% dplyr::filter( Fiscal_Year>=2001 & Fiscal_Year<2024), ##where is FPDS summary?
...
rpuh_ck<-get_column_key(def_rpuh)
  ...
  (
  ntdc<-build_plot(
    data=def_rpuh %>% dplyr::filter( Fiscal_Year>=2011 & Fiscal_Year<2024), ##where is FPDS summary?
    
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="Fiscal_Year",
    y_var="DefenseObligated_OMB25_GDP23", #Name of variable to plot on y-axis
    color_var="IsEntityTraditionalLabel",       # name of coloration variable, as string
  ...
  log_plot(plot=ntdc+labs(title=NULL)+theme(legend.position = "right"),
           df=def_rpuh %>% dplyr::filter(
                                            Fiscal_Year>=2007& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig8_ntdc_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="8 NTDC",path="../output/AcqTrends/",
           height=3.5,
           excel_formulas=TRUE,
                       startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
           output_doc_png = TRUE
...
colnames(cas)
  ...
  build_plot(
    ...
    # column_key=rpuh_ck,
    format=TRUE
  ))+labs(title="IsAbove2018constantCostAccounting2000kThreshold")
...
(
  ntdc_size_label<-build_plot(
    data=cas %>% filter (Fiscal_Year>=2007& Fiscal_Year<2024), #%>% dplyr::filter(IsEntityAbove2018constantMTAthreshold==1
                                          # & Fiscal_Year>=2007& Fiscal_Year<2024), ##where is FPDS summary?
    chart_geom = "Bar Chart",
    share = FALSE,
    x_var="Fiscal_Year",
  ...
  log_plot(plot=ntdc_label+labs(title=NULL)+theme(legend.position = "right"),
           df=cas %>% dplyr::filter(
                                            Fiscal_Year>=2011& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig8a_ntdc_obl",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="8A NTDC",path="../output/AcqTrends/",
           height=4,
           excel_formulas=TRUE,
                       startRow=1,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum"),
           output_doc_png = TRUE
...
(
  ...
  labs(title="AlwaysCAUisCASexemptOrWaived",
                                          y="Obligations (2023 $s)",x="Fiscal Year")+date_x_year_breaks(2007,2023,4))
...
```{r def_rpuh_hhi}
  ...
  log_plot(plot=def_rpuh_hhi+labs(title=NULL),
           df=def_rpuh %>% dplyr::filter(
                                            Fiscal_Year>=2011& Fiscal_Year<2024),#%>% filter(Fiscal_YQ<=2023.2)
           second_path=online_path,
                       filename="nps_fig10_hhi",xlsx="CSIS_DoD_DIB_Trends.xlsx",
                       sheet="10 hhi",path="../output/AcqTrends/",
           height=2,
