...
levels(disa_data$PricingUCA.sumlong)<-list("Firm-Fixed-Price"="FFP",
                                           ...
                                           "Undefinitized\nContract Award"="UCA",
                                           "Unclear"="Unclear"   )
full_data$ContractingSubCustomer<-factor(full_data$ContractingSubCustomer)
...
disa_sum$DISAprodserv.sum[disa_sum$DISAprodserv =="Products" ]<-"Other Products"
...
# full_labels_and_colors<-prepare_labels_and_colors(full_data,path="C:\\Users\\gsand\\Repositories\\Lookup-Tables\\style\\")
...
full_ICT<-standardize_variable_names(ict_data)
colnames(full_ICT)[colnames(ict_data)=="Fiscal.Year"]<-"fiscal_year"
full_ICT<-deflate(full_ICT,
                   money_var = "Action_Obligation",
                   fy_var="fiscal_year",
                   deflator_var="OMB20_GDP20"
)
ict_data$dFYear<-as.Date(paste("1/1/",as.character(ict_data$fiscal_year),sep=""),"%m/%d/%Y")
...
ICTfunder<-build_plot(
  ...
  facet_var="fundingagency", #facet_var
  column_key=column_key,
  format=TRUE,
  ytextposition=FALSE
)+
...
write.csv(file="..//Output//DISAProdServdetail.csv",row.names = FALSE,
          DISAProdServdetail$data%>% mutate(fiscal_year=lubridate::year(dFYear),
                                bAction_Obligation_OMB20_GDP20=Action_Obligation_OMB20_GDP20/1000000000)%>%
            arrange(fiscal_year)%>%
          pivot_wider(id_cols=c(DISAprodserv,Commercial),
                      names_from=fiscal_year,values_from=bAction_Obligation_OMB20_GDP20)%>%
            arrange(DISAprodserv,Commercial),na="")
```
