...
(
PricingHistory<-build_plot(
  data=PricingUpdate %>% filter(Fiscal_Year < 2025),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=hist_lc,
  # NA, #VAR.ncol
...
log_plot(plot=PricingHistory, df=PricingUpdate  %>% filter(Fiscal_YQ<2025.2),
...
# undebug(build_plot)
# undebug(build_plot)
(fpds_pricing <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<2025.2),
                            chart_geom="Line Chart",
                            x_var="dFYear",
...
(fpds_pricing_bar <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<2025.2),
                            chart_geom="Bar Chart",
                            x_var="dFYear",
                            y_var="Action_Obligation_OMB25_GDP23",
...
(fpds_pricing_psr <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<2025.2) %>%
...
log_plot(plot=fpds_pricing_psr_bar, df=full_data  %>% filter(Fiscal_YQ<2025.2),
...
(
OTAPSR<-build_plot(
  data=def_kota %>% filter(Fiscal_Year>=2015 & Fiscal_YQ<2025 &
...
log_plot(plot=OTAPSR, df=def_kota %>% filter(Fiscal_YQ<2025.2),
                     ...
                     sheet="3-1a KOta",path="../output/AcqTrends/", height=3.5,
                     startRow=1,startCol=12,format=TRUE,#var_list=c("SimpleArea","Competition.multisum"),
         excel_y_var = TRUE,excel_formulas = TRUE
         )
...
(
Services<-build_plot(
  data=full_data %>% filter(SimpleArea=="Services (Non-R&D)" & Fiscal_YQ<2025.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
log_plot(plot=Services, df=full_data  %>% filter(Fiscal_YQ<2025.2 &SimpleArea=="Services (Non-R&D)"),
...
(
SimpleVendor_dollar<-build_plot(
  data=def_data %>% mutate(YTD=ifelse(Fiscal_Year>=2023,"YTD","Full Year")) %>%
    filter(Fiscal_Year>=2007 & Fiscal_YQ<=2023.2 & !is.na(SimpleArea) & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  x_var="dFYear",
  ...
  reverse_color = TRUE,
  share=FALSE
)+date_x_year_breaks(2007,2020,4,partial_year=2023,partial_label="\nQ1-Q2")+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
(
SimpleVendor_share<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2007 & Fiscal_Year<=2023.2 & !is.na(SimpleArea) & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  x_var="dFYear",#alpha_var="YTD", #x_var
...
log_plot(plot=SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1)+
           date_x_year_breaks(1990,2022,8), df=def_data %>% filter(Fiscal_YQ<2025.2),
...
ggsave600dpi(path="..//Output//AcqTrends//",filename="PlatformQ_keynote.svg", PlatformQ+
               labs(title="Ordnance & Missile Contract Obligations, FY 2000–FY 2023"),
...
PlatformVendorIntl_share<-build_plot(
  data=full_data %>% filter(PlatformPortfolio!="Unlabeled" & Fiscal_YQ<2025.2 & Fiscal_YQ>=2000) ,
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
ggsave600dpi(path="..//Output//AcqTrends//",filename="platform_vendor_intl_share.png", PlatformVendorIntl_share, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
log_plot(plot=PlatformVendorIntl_dollar, df=full_data  %>% filter(Fiscal_YQ<2025.2),
...
PlatformVendorNTDC_share<-build_plot(
  data=def_data %>% filter(PlatformPortfolio!="Unlabeled" & Fiscal_YQ<=2023.2 & Fiscal_YQ>=2007) ,
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi(path="..//Output//AcqTrends//",filename="platform_vendor_NTDC_share.png", PlatformVendorNTDC_share, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
# 
# log_plot(plot=PlatformVendorNTDC_dollar, df=full_data  %>% filter(Fiscal_YQ<2025.2),
#                      filename="acq_fig6_2_plat_vendor_NTDC",xlsx="DoD_Acq_Trends_Contracts.xlsx",
#                      sheet="6-2 PlatVend",path="../output/AcqTrends/", height=5,
#          excel_y_var = TRUE, excel_formulas = TRUE,
#                      format=TRUE,#,var_list=c("SimpleArea","Competition.multisum")
#          )
...
PlatformMultiYearSubCust<-build_plot(
  ...
  theme(legend.position = "bottom")+
  labs(y="Obligations (Constant 2023 $s)",
       title="DOD Contract Obligations to Multi-Year Procuement\nby Platform and Component, FY 2000–FY 2023",
       caption="Note: When the multi-year column is blank, transactions are treated as not multiyear. Unlabeled platform not shown.\nSource: FPDS and CSIS analysis.")
)
...
ggsave600dpi(path="..//Output//AcqTrends//",filename="Platform_MultiYearShare_keynote.png", 
               PlatformMultiYearShare+labs(
                 title="DOD Share of Contract Obligations to Multi-Year Contracts, FY 2000–FY 2023")+
...
log_plot(plot=PlatformMultiYearSubCust+facet_wrap(PlatformPortfolio~.,nrow=2),
         df=def_data %>% filter(Fiscal_Year>=2000 &Fiscal_YQ<2025.2),
                     filename="nps_fig07b_multiyearCust",xlsx="DoD_2024_NPS.xlsx",
                     sheet="7b MultCust",path="../output/AcqTrends/NPS2024/",
         second_path = online_path,
...
ggsave600dpi(path="..//Output//AcqTrends//NPS2024/",filename="nps_fig07c_multiyear_slide.png", PlatformMultiYear+facet_wrap(PlatformPortfolio~.,nrow=2),  second_path = online_path,
             width=13, height= 5.5, units="in",size=20, lineheight=1.2
)
...
log_plot(plot=PlatformMultiYearShare+facet_wrap(PlatformPortfolio~.,nrow=2),
         df=def_data %>% filter(Fiscal_Year>=2000 &Fiscal_YQ<2025.2),
                     filename="nps_fig07a_multiyear",xlsx="DoD_2024_NPS.xlsx",
                     sheet="7a Mult",path="../output/AcqTrends/NPS2024/",
...
log_plot(plot=ToplineVendor, df=full_data  %>% filter(Fiscal_YQ<2025.2 ),
...
for(p in platform_levels){
  file_p<-gsub("__*","_",gsub("&","and",gsub("[ |,]","_",p)))
  (Breakout<-build_plot(
    data=full_data %>% filter(Fiscal_Year>=2000&PlatformPortfolio==p& Fiscal_YQ<2025.2),
chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  ...
  log_plot(plot=Breakout, df=full_data  %>% filter(Fiscal_YQ<2025.2&PlatformPortfolio==p),
...
VendorSizePricing<-build_plot(
  data=full_data %>% filter(Fiscal_YQ<2025.2 & Fiscal_Year>2000 & Shiny.VendorSize!="Unlabeled" &
...
log_plot(plot=waiversimple, df=full_data   %>% filter(Fiscal_YQ<2025.2 & 
