...
for(c in subcustomer_levels){
  ...
  log_plot(plot=NULL, df=platpscintldef %>% filter(Fiscal_Year>=2000&Fiscal_Year<=2023&SubCustomer.sum==c),
           ...
           csv_then_year=TRUE,excel_then_year = FALSE, excel_y_var=TRUE,
           startRow=1,startCol=15,format=TRUE,var_list=c("PlatformPortfolio","Project.Name","ProjectID"),
...
for(c in subcustomer_levels){
  ...
  log_plot(plot=NULL, df=platpscintldef %>% filter(Fiscal_Year>=2000&Fiscal_Year<=2023&SubCustomer.sum==c),
           ...
           sheet="AllProj",path=file.path("../output/AcqTrends/Customer/",file_c),
           csv_then_year=TRUE,excel_then_year = FALSE, excel_y_var=TRUE,
           startRow=1,startCol=13,format=TRUE,var_list=c("ProductOrServiceArea","ProductOrServiceCode","ProductOrServiceCodeText"),
           output_doc_png=FALSE, output_doc_svg = FALSE
           )
