...
topState<-platpscintldef %>% group_by (pop_state_code,ProductOrServiceArea) %>%
  summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23),
            Action_Obligation_2023=sum(ifelse(Fiscal_Year==2022,Action_Obligation_OMB25_GDP23,0)))%>%
...
platpscintldef %>% filter(Fiscal_Year==2022) %>% group_by(SubCustomer) %>% summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23))
...
#                         names_from=Fiscal_Year,values_from=Action_Obligation_OMB25_GDP23)%>% arrange(VendorNTIB,VendorSize_Intl,ProductOrServiceArea))
