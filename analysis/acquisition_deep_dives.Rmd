...
write.csv(file="..//Output//..//Output//Deep_Dives//DTRA//DTRAcomm.csv",row.names = FALSE, na = "",
          pivot_wider(SubCustomer$data,id_cols=c(AnyCommercialText,AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB20_GDP20))

write.csv(file="..//Output//Deep_Dives//DTRA//DTRAcomm_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(AnyCommercialText,AnyCommercialText,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(AnyCommercialText,AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
            arrange(AnyCommercialText,AnyCommercialText))
