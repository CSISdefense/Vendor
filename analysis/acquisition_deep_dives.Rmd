...
topplat<-platpscintldef %>% group_by (Project.Name,PlatformPortfolio) %>%
  ...
  mutate(Action_Obligation_2023=if_else(is.na(Project.Name),Action_Obligation_2023,NA),
    Action_Obligation_2023=if_else(Fiscal_Year>=2023,Action_Obligation_OMB24_GDP22,NA))%>%
...
platpscintldef<-left_join(platpscintldef,topplat %>% select(-Action_Obligation_OMB24_GDP22,Action_Obligation_2023),
...
platpscintldef<-label_top(platpscintldef,
                    col="Project.Name",
                    n=7,
                    top_name="TopProjectTest",
                    group_list="PlatformPortfolio",
                    recent=2023,
                    retain_rank=TRUE,
                    write_file=file.path("..","Output","AcqTrends","TopProjectTest.csv"))

View(platpscintldef%>% group_by(PlatformPortfolio,TopProjectTest,TopProject,rank_total,TopProjectTest_rank_total,rank_2023,TopProjectTest_rank_recent) %>%  
       filter(!TopProjectTest=="Other Labeled"& TopProject=="Other Labeled Project") %>%
...
platpscintldef<-left_join(platpscintldef,topPSC %>% select(-Action_Obligation_OMB24_GDP22,Action_Obligation_2023),
...
topState<-platpscintldef %>% group_by (pop_state_code,ProductOrServiceArea) %>%
  summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22),
            Action_Obligation_2023=sum(ifelse(Fiscal_Year==2022,Action_Obligation_OMB24_GDP22,0)))%>%
...
platpscintldef<-left_join(platpscintldef,topState %>% select(-Action_Obligation_OMB24_GDP22,Action_Obligation_2023),
...
platpscintldef %>% filter(Fiscal_Year==2022) %>% group_by(SubCustomer) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22))
...
platpscintldef %>% group_by(SubCustomer,Fiscal_Year) %>% filter(SubCustomer=="SDA") %>%
  summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22)) %>% arrange(
    Fiscal_Year
  )
...
log_plot(plot=costaccountingstandardsclause, df=hightech%>% filter(Fiscal_Year>=2000),
  ...
  costorpricingdata<-build_plot(
    ...
    theme(legend.position = "right")+facet_wrap(~PricingUCA, scales="free_y")+
    labs(y="Obligations (Constant 2022 $s, Variable Scale)",x="Fiscal Year",
...
write.csv(file="..//Output//ntibintl_contract.csv",row.names = FALSE,na = "",
          ntibintl$data%>%# mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(PlaceIsForeign,VendorNTIB,ProductOrServiceArea),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>% arrange(PlaceIsForeign,VendorNTIB,ProductOrServiceArea))
...
#             pivot_wider(id_cols=c(VendorNTIB,VendorSize_Intl,ProductOrServiceArea),
#                         names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>% arrange(VendorNTIB,VendorSize_Intl,ProductOrServiceArea))
...
write.csv(file="..//Output//fmsintl_service.csv",row.names = FALSE,na = "",
          fmsintl_service$data%>%# mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(PlaceIsForeign,IsFMS,ProductOrServiceArea),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>% arrange(PlaceIsForeign,IsFMS,ProductOrServiceArea))
...
write.csv(file="..//Output//fmsintl_service_vendor.csv",row.names = FALSE,na = "",
          fmsintl_service_vendor$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(IsFMSplaceIntl,VendorSize_Intl,ProductOrServiceArea),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>% arrange(IsFMSplaceIntl,VendorSize_Intl,ProductOrServiceArea))
...
SpaceParentOnly<-build_plot(
  data=space %>% filter(Fiscal_YQ<=2024.1 & Fiscal_YQ>=2007),
  chart_geom = "Bar Chart",
  share = FALSE,
  ...
  x_var="dFYear", #x_var
  y_var="Action_Obligation_OMB24_GDP22", #VAR.y.variable
  color_var="ParentID", #color_var
  facet_var="ParentID", #facet_var
  alpha_var="YTD",
  column_key=space_ck,
  format=TRUE,
  ytextposition=FALSE
 ...
 #             )+labs(title=NULL,
  ...
  # color="Competition")+
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2022 $s)",title="Contract Obligations for 9 Major Space Providers, 2007-2024 Q1",x="Fiscal Year",
...
(SpaceParentVar<-SpaceParent+facet_wrap(~ParentID,scales="free_y")+labs(y="Obligations (Constant 2022 $s) Variable Scale"))


  
log_plot(plot=SpaceParent, df=space %>% filter(Fiscal_YQ<=2024.1 & Fiscal_YQ>=2007),
...
log_plot(plot=SpaceParentOnly, df=space %>% filter(Fiscal_YQ<=2024.1 & Fiscal_YQ>=2007),
                     filename="space_vend",xlsx="Space_Acq_Trends.xlsx",
                     sheet="Vend",path="../output/Space/", height=4.25,width=8,
         excel_y_var=TRUE,excel_formula=TRUE,
                     startRow=1,startCol=12,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum")
         )
ggsave600dpi("..//Output//Space//space_vend_cust_variable.svg", SpaceParentVar, 
             width=8, height= 4.25, units="in",size=11,lineheight = 1
             )
```