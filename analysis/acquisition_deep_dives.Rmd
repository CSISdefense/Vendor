...
platpsc$TopPStext<-string_wrap(platpsc$TopPStext)
...
topAgency<-sw %>% group_by (Contracting_Agency_ID,AgencyIDtext,Customer) %>% #,PlatformPortfolio
  summarise(Action_Obligation_OMB20_GDP20=sum(Action_Obligation_OMB20_GDP20),
            Action_Obligation_2020=sum(ifelse(fiscal_year==2020,Action_Obligation_OMB20_GDP20,0)))%>%
              group_by (Customer) %>%
    mutate(rank_total=rank(desc(Action_Obligation_OMB20_GDP20)),
...
topAgency$TopAgencyText<-
  ifelse(topAgency$rank_2020<=7 | topAgency$rank_total<=7,topAgency$AgencyIDtext,NA)
...
sw$TopAgencyText<-string_wrap(sw$TopAgencyText)

summary(factor(sw$TopAgencyText))
...
(jsf_vendor_intl_place <-  build_plot(data=platpscintldef %>% filter(IsJSF=="JSF (F-35)"& Fiscal.Year>2011),
                                       #IsFMS==TRUE 
                                       chart_geom="Bar Chart",
                            x_var="Fiscal.Year",
                            y_var="Action_Obligation_OMB20_GDP20",
...
# undebug(format_data_for_plot)
(jsf_manufacture_intl_place <-  build_plot(data=platpscintldef%>% filter(IsJSF=="JSF (F-35)"& Fiscal.Year>2011),              
                                            chart_geom="Bar Chart",
                            x_var="Fiscal.Year",
                            y_var="Action_Obligation_OMB20_GDP20",
...
write.csv(file="..//Output//fmsintl_service.csv",row.names = FALSE,
          fmsintl_service$data%>%# mutate(Fiscal.Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(PlaceIsForeign,IsFMS,CrisisProductOrServiceArea),
                        names_from=Fiscal.Year,values_from=Action_Obligation_OMB20_GDP20)%>% arrange(PlaceIsForeign,IsFMS,CrisisProductOrServiceArea))
...
write.csv(file="..//Output//fmsintl_service_vendor.csv",row.names = FALSE,
          fmsintl_service_vendor$data%>% mutate(Fiscal.Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(IsFMSplaceIntl,VendorSize_Intl,CrisisProductOrServiceArea),
                        names_from=Fiscal.Year,values_from=Action_Obligation_OMB20_GDP20)%>% arrange(IsFMSplaceIntl,VendorSize_Intl,CrisisProductOrServiceArea))
...
SWpscdetail<-build_plot(
  data=platpsc %>% filter(fiscal_year>=2002 & ProductServiceOrRnDarea!="Unlabeled" &
...
platpsc$ContractingAgencyTextSwr<-string_wrap(platpsc$ContractingAgencyText)


write.csv(file="..//Output//DISA_prodserv.csv",row.names = FALSE,
          arrange(pivot_wider(SWpscdetail$data %>% 
                        arrange(dFYear) %>% mutate(fiscal_year=lubridate::year(dFYear)),
                      id_cols=c(SWpsc),
                      names_from=fiscal_year,values_from=Action_Obligation_OMB20_GDP20),SWpsc))
...
ggsave600dpi(SWpscdetail,file="..//output//DISA_prodserv.png",  
             width=13, height= 6, units="in",size=16, lineheight=1.2
             )
ggsave600dpi(SWpscdetail,file="..//output//DISA_prodserv.eps",  
             width=13, height= 6, units="in",size=16, lineheight=1.2
             )
```
...
ggsave600dpi(SWpscdetail,file="..//output//Software_Defense_Agency.png",  
             width=13, height= 6, units="in",size=8, lineheight=1.2
             )
ggsave600dpi(SWpscdetail,file="..//output//Software_Defense_Agency.eps",  
             width=13, height= 6, units="in",size=16, lineheight=1.2
             )
...
SoftwareComp<-build_plot(
  ...
  facet_var="ProductOrServiceCodeText", #facet_var
  column_key=sw_ck,
  format=TRUE,
  ytextposition=FALSE
)+
...
write.csv(file="..//Output//Software_competition.csv",row.names = FALSE,
          SoftwareComp$data%>% #mutate(dFYear=lubridate::year(dFYear))%>%
...
SoftwareVehicle<-build_plot(
  ...
  facet_var="ProductOrServiceCodeText",
  column_key=sw_ck,
  format=TRUE,
  ytextposition=FALSE
)+scale_x_date("Fiscal Year",labels = date_format("'%y"))+
...
write.csv(file="..//Output//Software_pricing.csv",row.names = FALSE,
  arrange(pivot_wider(SoftwarePricing$data %>% 
                        arrange(dFYear) %>% mutate(fiscal_year=lubridate::year(dFYear)),
