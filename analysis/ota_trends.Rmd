...
output_TY<-ota_def  %>% select(Fiscal_Year,Action_Obligation_Then_Year,`Base_and_All_Options_Value_(Total_Contract_Value)`)%>%
  ...
  summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE),
            `Base_and_All_Options_Value_(Total_Contract_Value)`=sum(text_to_number(`Base_and_All_Options_Value_(Total_Contract_Value)`),
                                                                    na.rm=TRUE))%>%
  pivot_longer(cols=c(Action_Obligation_Then_Year,`Base_and_All_Options_Value_(Total_Contract_Value)`)) %>%
...
write.csv(file="..//Output//ota_topline_psr_cur.csv",row.names = FALSE,
...
platpsc$Action_Obligation_OMB24_GDP22
