...
write.csv(file="..//Output//topline_pricing.csv",row.names = FALSE,
          ToplinePricing$data%>% mutate(dFYear=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(PricingUCA,PricingUCA.sumlong),
                        names_from=dFYear,values_from=Action_Obligation_OMB23_GDP21))
...
ToplinePSR<-build_plot(
  data=ota_def %>% filter(Fiscal_Year>=2015& Fiscal_Year<=2021),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=ota_lc,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//ota_topline_psr.png", ToplinePSR, 
...
platpsc$Action_Obligation_OMB23_GDP21
