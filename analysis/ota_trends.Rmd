...
a<-OTAquarterly %>% group_by(Fiscal_Year) %>% filter(Fiscal_Year<2021 & !is.na(fiscal_quarter))%>% mutate(
  q1toq3=if_else(fiscal_quarter<=3,Action_Obligation_OMB25_GDP23,NA),
  q4=if_else(fiscal_quarter==4,Action_Obligation_OMB25_GDP23,NA)) %>%
  summarise(
  q1toq3=sum(q1toq3,na.rm=TRUE),
  q4=sum(q4,na.rm = TRUE),
  total=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE)
)
...
q1to13_2021<-sum(ifelse(OTAquarterly$fiscal_quarter<=3 & !is.na(OTAquarterly$fiscal_quarter) & OTAquarterly$Fiscal_Year==2021,OTAquarterly$Action_Obligation_OMB25_GDP23,0))
...
write.csv(file="..//Output//OTA//ota_topline_qest.csv",row.names = FALSE,na="",
          ...
          format_data_for_plot(
                               ...
                               group=TRUE) %>%
            arrange(Fiscal_Year) %>%
            mutate(Dollars_GDP20=Action_Obligation_OMB25_GDP23/1000000000)%>%
...
ToplineType<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  labs(y="Obligations (Constant 2023 $s)",x="Fiscal Year",
       title="Figure 12: Defense OTA Obligations by Customer and Type, FY 2015–FY 2024 YTD",
...
TypeSubCustomer<-build_plot(
  data=ota_def %>% filter(Fiscal_Year>=2015 & #Fiscal_YQ<=2025.2 &
                            !is.na(TypeOfAgreement)),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=ota_lc,
  ...
  format=TRUE,
  ytextposition=FALSE
)+date_x_year_breaks(2000,2024,3)+
    facet_grid(TypeOfAgreement~., scales="free_y",space="free_y")+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 ...
 #             )+labs(title=NULL,
  ...
  theme(legend.position = "right",strip.text.y=element_text(angle=0))+
  labs(y="Obligations (Constant 2023 $s)",x="Fiscal Year",
       title="DOD OTA Obligations by Component and Type, FY 2015–FY 2025-Q2",
       caption="Note: Unlabeled values not show.\nSource: FPDS and CSIS analysis.")
)
...
log_plot(plot=TypeSubCustomer, df=ota_def ,#%>% filter(Fiscal_YQ<=2024.1)
                     ...
                     sheet="TypeCust",path="..//Output//OTA//",
         width=4,height=5.5,
                     startRow=1,startCol=12,format=TRUE,#var_list=c("Source","fiscal_quarter"),
         )       

log_plot(plot=TypeSubCustomer, df=ota_def ,#%>% filter(Fiscal_YQ<=2024.1)
                     filename="nps_fig12_ota_type_cust",xlsx="DoD_2024_NPS.xlsx",
                     sheet="12 OTAtype",
         path="../output/AcqTrends/NPS2025/",second_path=online_path,
         include_YTD=FALSE, width=4,height=5.5,
                     startRow=1,format=TRUE,excel_y_var=TRUE,excel_formulas = TRUE,
         output_doc_svg=TRUE
         )


ggsave600dpi(TypeSubCustomer+facet_wrap(~TypeOfAgreement),
            filename= "nps_fig12_ota_type_cust_slide.png",path="..//Output//OTA//NPS//",
            second_path=online_path,
             width=13, height= 5.5, units="in",size=20,lineheight = 1.2
             )

log_plot(plot=TypeSubCustomer, df=ota_def %>% filter(Fiscal_Year<=2024),
                     filename="nps_fig12_ota_typecust",xlsx="DoD_2024_NPS.xlsx",
                     sheet="12 OTAtype",path="../output/AcqTrends/NPS2025/",second_path=online_path,
...
write.csv(file="..//Output//OTA//psr_RnDPhase.csv",row.names = FALSE,
          pivot_wider(RnDphase$data%>%mutate(Fiscal_Year=year(dFYear)),id_cols=c(ProductServiceOrRnDarea),
                      names_from=dFYear,values_from=Action_Obligation_OMB25_GDP23)%>%
