...
(
  cbre_plot<-ggplot(def %>% filter(b_CBre==1 & !is.na(qHighCeiling)), aes(x=n_CBre_Then_Year)) +
    ...
    labs(y="Count of Contracts that Experienced a Ceiling Breach\n(Variable Scale)",x="Ceiling Breach Size on a Logarithmic Scale")#+, space="free_y"
...
ggsave(cbre_plot+theme(text=element_text(size=13)),file="..//Output\\n_CBre_distribution.png",width=9.5,height=4,dpi=600)
ggsave(cbre_plot+theme(text=element_text(size=13)),file="..//Output\\n_CBre_distribution.eps",width=9.5,height=4,dpi=600)
...
debug(NA_stats)
NA_stats(def,"CompOffr")
sum(def$Action_Obligation.OMB20_GDP18[def$CompOffr=="No Competition"],na.rm=TRUE)/sum(def$Action_Obligation.OMB20_GDP18,na.rm=TRUE)
sum(def$Action_Obligation.OMB20_GDP18[def$CompOffr=="1 offer"],na.rm=TRUE)/sum(def$Action_Obligation.OMB20_GDP18,na.rm=TRUE)
sum(def$Action_Obligation.OMB20_GDP18[def$CompOffr=="2 offers"],na.rm=TRUE)/sum(def$Action_Obligation.OMB20_GDP18,na.rm=TRUE)
sum(def$Action_Obligation.OMB20_GDP18[def$CompOffr=="3-4 offers"],na.rm=TRUE)/sum(def$Action_Obligation.OMB20_GDP18,na.rm=TRUE)
sum(def$Action_Obligation.OMB20_GDP18[def$CompOffr=="5+ offers"],na.rm=TRUE)/sum(def$Action_Obligation.OMB20_GDP18,na.rm=TRUE)
