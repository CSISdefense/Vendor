...
q2021<-y2021 %>% mutate(fiscal_quarter=lubridate::quarter(Date.Signed,fiscal_start=10),
  ...
  summarize(Action_Obligation=sum(csis360::text_to_number(Dollars.Obligated),na.rm=TRUE),
            minSD=min(Date.Signed),
            maxSD=max(Date.Signed),
  )%>% filter(fiscal_quarter<=3)
y2021
...
a<-quarterlyYTD %>% group_by(Fiscal.Year) %>% filter(Fiscal.Year<2021)%>% summarise(
...
q1to13_2021<-sum(ifelse(quarterlyYTD$fiscal_quarter<=3 & quarterlyYTD$Fiscal.Year==2021,quarterlyYTD$Action_Obligation_OMB20_GDP20,0))
...
e2021<-data.frame(Fiscal.Year=2021,
           ...
           Action_Obligation_OMB20_GDP20=p[1],
           lwr=p[2]+q1to13_2021,
           upr=p[3]+q1to13_2021,
...
(topline_q<-ggplot(data=quarterlyEst,#rbind(quarterly,q2021,e2021),
       aes(x=Fiscal.Year,
           y=Action_Obligation_OMB20_GDP20,
           alpha=Actual,
           fill = fiscal_quarter
       ))+geom_col()+
  scale_alpha_discrete(range=c(0.4,1))+
  geom_errorbar(aes(x=Fiscal.Year,ymin=lwr,ymax=upr))+labs(x="Fiscal Year",y="Obligated Amount",fill="Quarter")
)

write.csv(file="..//Output//toa_topline.csv",row.names = FALSE,na="",
          format_data_for_plot(quarterlyEst,
                               fy_var="Fiscal.Year",
                               y_var="Action_Obligation_OMB20_GDP20",
                               color_var="fiscal_quarter",
                               # facet="SubCustomer",
                               ...
                               group=TRUE) %>%
            arrange(Fiscal.Year) %>%
            mutate(Action_Obligation_OMB20_GDP20=Action_Obligation_OMB20_GDP20/1000000000)%>%
          pivot_wider(id_cols=c(fiscal_quarter),
                      names_from=Fiscal.Year,values_from=Action_Obligation_OMB20_GDP20) %>% 
            ...
            #                                 labels=gsub("\\n"," ",levels(PlatformPortfolio))))%>%
                   arrange(desc(fiscal_quarter)))

p
ggsave600dpi(topline_q,file=file.path("..","Output","topline_q.png"),  
