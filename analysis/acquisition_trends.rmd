...
write.csv(file="..//Output//topline_vehicle.csv",row.names = FALSE, na = "",
          ToplineVehicle$data%>% mutate(Fiscal_Year=lubridate::year(dFYear),
            ...
            pivot_wider(id_cols=c(Vehicle.sum,Vehicle.AwardTask),
                        names_from=Fiscal_Year,values_from=bAction_Obligation_OMB24_GDP22)%>%arrange(Vehicle.sum,Vehicle.AwardTask))
...
PlatformMultiYearShare<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000 & !PlatformPortfolio %in% c("Unlabeled","Other Products","Other Services","Other R&D and Knowledge Based")&
