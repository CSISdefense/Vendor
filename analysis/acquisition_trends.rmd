...
write.csv(
  ...
  summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE),
            Action_Obligation_OMB20_GDP20=sum(Action_Obligation_Then_Year,na.rm=TRUE)),"contracting.csv",row.names = FALSE)
...
PSRComp_line<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  theme(legend.position = "bottom")+
  labs(x="Fiscal Year",
       y="Obligations (Constant 2020 $s)")+scale_x_date(date_breaks = "4 years",date_labels = "'%y")
)
...
PlatformComp_bar<-build_plot(
  data=full_data %>% filter(PlatformPortfolio!="Unlabeled"  &Fiscal_Year>=1991),
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
GFE<-build_plot(
  ...
  x_var="Fiscal_Year", #x_var
  y_var="Action_Obligation_OMB20_GDP20", #VAR.y.variable
  color_var="gfe_gfp_code", #color_var
  facet_var="SubCustomer.platform", #facet_var
  column_key=def_ck,
  format=TRUE,
  ytextposition=FALSE
...
GFE<-build_plot(
  ...
  facet_var="gfe_gfp_code", #facet_var
  column_key=def_ck,
  format=TRUE,
  ytextposition=FALSE
)+
