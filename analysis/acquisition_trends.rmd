...
fpds_annual<-full_data %>% filter(Fiscal_YQ<2024) %>% group_by(Fiscal_Year,dFYear,IsFMS,
...
ToplinePricing<-build_plot(
  ...
  ytextposition=FALSE,
  alpha_var="YTD",
)+date_x_year_breaks(2000,2020,6)+#,partial_year=2024,partial_label="\nQ1")
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
Multiyear<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<2024),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
(
PSR_Q<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000&  Fiscal_YQ<2024 & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
PSRComp_line<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  labs(x="Fiscal Year",
       y="Obligations (Constant 2022 $s)")+
    date_x_year_breaks(1993,2021,by=6)#,partial_year=2024,partial_label="\nQ1")
)
...
ToplineVendorIntl<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000& Fiscal_YQ<2024 ),# & Fiscal_Year<=2021
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
log_plot(plot=ToplineVendorIntl, df=full_data  %>% filter(Fiscal_YQ<2024),
