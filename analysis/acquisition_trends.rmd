...
for(p in platform_levels){
  ...
  (Breakout<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2000&Fiscal_YQ<=2023.2&PlatformPortfolio==p),
      ...
      date_x_year_breaks(2000,2020,2,partial_year=2023,partial_label="\nQ1–Q2")+   theme(legend.position = "right")+
...
log_plot(plot=topline, 
         df=output_TY   ,y_var="Then_Year_Dollars",#%>% filter(Fiscal_YQ<=2023.2),
...
for(p in platform_levels){
  file_p<-gsub("__*","_",gsub("&","and",gsub("[ |,]","_",p)))
  (Breakout<-build_plot(
    data=full_data %>% filter(Fiscal_Year>=2000&PlatformPortfolio==p& Fiscal_YQ<=2023.2),
chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  ...
  log_plot(plot=Breakout, df=full_data  %>% filter(Fiscal_YQ<=2023.2&PlatformPortfolio==p),
