...
(toplineq<-build_plot(data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.2),#rbind(quarterly,q2021,e2021),
           ...
           column_key=def_ck
       )+date_x_year_breaks(2000,2020,2,partial_year=2023,partial_label="\nQ1-Q2")+scale_alpha_ordinal(range=c(1,0.5))    theme(legend.position = "right")+
...
for(p in platform_levels){
  ...
  (Breakout<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2000&Fiscal_YQ<=2023.2&PlatformPortfolio==p),
                     chart_geom="Bar Chart",
                     x_var="dFYear",alpha_var="YTD",
                     y_var="Action_Obligation_OMB24_GDP22",
           color_var="fiscal_quarter",
...
summary(factor(full_data$multiyearcontract))
full_data$AnyMultiyearText<-factor(full_data$multiyearcontract)
levels(full_data$AnyMultiyearText)<-list(
  "Not Classified\nas Multiyear"="N",
...
log_plot(plot=Multiyear, df=full_data %>% filter(Fiscal_YQ<=2023.2),
...
(
Services<-build_plot(
  data=full_data %>% filter(SimpleArea=="Services (Non-R&D)" & Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
log_plot(plot=Services, df=full_data  %>% filter(Fiscal_YQ<=2023.2 &SimpleArea=="Services (Non-R&D)"),
