...
ggsave600dpi("..//Output//topline_MultiYear.png", ToplinePricing,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

write.csv(file="..//Output//topline_MultiYear.csv",row.names = FALSE, na = "",
  arrange(pivot_wider(ToplineMultiYear$data %>% 
                        arrange(dFYear) %>% mutate(Fiscal_Year=lubridate::year(dFYear)),
                      id_cols=c(multiyearcontract),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21),multiyearcontract))
...
write.csv(file="..//Output//platform_vendor_share.csv",row.names = FALSE, na = "",
          PlatformVendor_share$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
...
VendorSizePricing2021<-build_plot(
  data=full_data %>% filter(Fiscal_Year<=2021 & Fiscal_Year>2020 & Shiny.VendorSize!="Unlabeled"),
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
