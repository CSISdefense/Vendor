...
output_TY<-def_data %>% filter(SimpleArea=="R&D") %>%
...
contract_merge<-def_data%>%filter(Fiscal_Year >=2015) %>%
  group_by(Fiscal_Year,
                         ProductServiceOrRnDarea,
                         SimpleArea)%>%
  dplyr::summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22))
...
(
Services<-build_plot(
  data=def_data %>% filter(SimpleArea=="Services (Non-R&D)"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
PSRComp_line<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1991 & SimpleArea!="Unlabeled"),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//topline_subcustomer_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(SubCustomer.sum,SubCustomer.JPO,Fiscal_Year) %>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SubCustomer.sum,SubCustomer.JPO),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year))
...
## SubCustomer Covid19
```{r SubCustomer}
cust_covid<-def_data %>% group_by(SubCustomer.JPO,SubCustomer.sum,Fiscal_Year) %>%
  summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm = TRUE),
            COVID19obligated=sum(COVID19obligated,na.rm = TRUE))%>%
...
write.csv(file="..//Output//platform_multiyear_cur.csv",row.names = FALSE, na = "",
          def_data %>% group_by(PlatformPortfolio,multiyearcontracttext,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
VendorSizePricing<-build_plot(
  data=def_data %>% filter(Fiscal_Year<=2020 & Fiscal_Year>2000 & Shiny.VendorSize!="Unlabeled" &
