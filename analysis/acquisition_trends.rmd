...
log_plot(plot=topline, df=output_TY,y_var="Dollars_OMB25_GDP23",
...
(topline<-build_plot(data=fpds_annual %>% filter(Fiscal_Year>=1990),#rbind(quarterly,q2021,e2021),
    ...
    geom_path(aes(x=dFYear,y=Outlays_OMB25_GDP23,color=TOA))+
  # scale_alpha_discrete(range=c(0.4,1))+
    labs(x="Fiscal Year",y="Constant FY 2023 $",fill="Contract",color="Budget",
...
ggsave600dpi(topline,file=file.path("..","Output","AcqTrends","topline_outlay_annual.png"),  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )
...
log_plot(plot=PlatformMultiYearShare, df=def_data %>% filter(Fiscal_YQ<2024),
