...
if(!"Action_Obligation_OMB24_GDP22" %in% colnames(ota_def))
  ota_def<-deflate(ota_def, "Dollars_Obligated_Then_Year",deflator_var="OMB23_GDP21")
colnames(ota_def)[colnames(ota_def)=="Dollars_Obligated_Then_Year_OMB23_GDP21"]<-"Action_Obligation_OMB24_GDP22"
colnames(ota_def)[colnames(ota_def)=="Dollars_Obligated_Then_Year_Then_Year"]<-"Dollars_Obligated_Then_Year"

#Kludge
toa<-toa[11,c(-1,-3)]
colnames(def_data)[colnames(def_data)=="Action_Obligation_Then_Year_OMB23_GDP21"]<-"Action_Obligation_OMB24_GDP22"
...
labels_and_colors<-prepare_labels_and_colors(full_data,path=file.path(local_path,"style//"))
...
(toplineq<-build_plot(data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.2),#rbind(quarterly,q2021,e2021),
    ...
    # geom_path(aes(x=dFYear,y=Action_Obligation_OMB24_GDP22,color=TOA))+
  # scale_alpha_discrete(range=c(0.4,1))+
    # labs(x="Fiscal Year",y="Constant FY 2022 $ Billions",fill="Contract",color="Budget",
    #      caption="Source: FPDS, CSIS Analysis")+
    theme(legend.position = "right")+
  labs(y="Obligations (Constant 2022 $s)")
)

ggsave600dpi(toplineq,file=file.path("..","Output","toplineq.svg"),  
             width=6.5, height= 3.5, units="in",size=11, lineheight=1.2
             )

ggsave600dpi(toplineq,file=file.path("..","Output","toplineq.png"),  
             width=6.5, height= 3.5, units="in",size=11, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//toplineqr_keynote.svg", toplineq+
               labs(title="DOD Contract Obligations, 2000-2023Q1"),
             width=13, height= 5.5, units="in", size=20, lineheight = 0.75
             )

ggsave600dpi(toplineq+labs(caption=NULL),
            file="../output/AcqTrends/acq_fig2_1_fq.svg",size=12,caption_fraction=8/12,lineheight=1, height =3, width=6.5)



output_TY<-def_data %>% mutate(Source="Contract") %>%  group_by(Source,fiscal_quarter,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
  arrange(Fiscal_Year)%>% 
          pivot_wider(id_cols=c(Source,fiscal_quarter),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year) %>%arrange(Source,fiscal_quarter)

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
# addWorksheet(wb, sheetName = "2-2 FYQ")
 writeData(wb, output_TY, sheet = "2-2 FYQ", startRow = 1, startCol = 9)
...
fpds_annual<-full_data %>% group_by(Fiscal_Year,dFYear,IsFMS,
                                    ContractingCustomer) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE),
...
write.csv(file="..//Output//AcqTrends//toa_annual.csv",row.names = FALSE,na="",output_TY
          )
...
writeData(wb, output_TY, sheet = "TOA", startRow = 2, startCol = 1)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
rm(wb)
xlcFreeMemory()
...
fpds_annual<-def_psc_join %>% group_by(Fiscal_Year,dFYear) %>%
...
fpds_annual$Total_Obligation_Authority_Then_Year<-NA
fpds_annual$Total_Obligation_Authority_OMB20_GDP20<-NA
fpds_annual<-rbind(as.data.frame(fpds_annual[,colnames(fpds_annual)%in%colnames(toa)]),
                   toa[,colnames(toa)%in%colnames(fpds_annual)])
colnames(toa)[!colnames(toa) %in% colnames(fpds_annual)]
colnames(fpds_annual)[!colnames(fpds_annual) %in% colnames(toa)]
...
(topline<-build_plot(data=fpds_annual %>% filter(Fiscal_Year>=1990),#rbind(quarterly,q2021,e2021),
    ...
    geom_path(aes(x=Fiscal_Year,y=Total_Obligation_Authority_OMB20_GDP20,color=TOA))#+
  # scale_alpha_discrete(range=c(0.4,1))+
    # labs(x="Fiscal Year",y="Constant FY 2020 $ Billions",fill="Contract",color="Budget",
    #      caption="Source: FPDS, FY 2022 DoD Greenbook, CSIS Analysis")+
    # theme(legend.position = "right")#+
...
output_TY<-fpds_annual %>% mutate(
  source=ifelse(!is.na(Total_Obligation_Authority_Then_Year),"TOA","Contract"),
  Dollars=ifelse(source=="TOA",
                 Total_Obligation_Authority_Then_Year,Action_Obligation_Then_Year))%>%
            filter(Fiscal_Year>=1990)%>%
          format_data_for_plot(
...
write.csv(file="..//Output//AcqTrends//toa_sam_annual_TY.csv",row.names = FALSE,na="",output_TY
          )
write.csv(file="..//Output//AcqTrends//toa_sam_annual.csv",row.names = FALSE,na="",output_TY
          )
...
## Topline Competition
```{r ToplineComp}
def_data %>% group_by(Fiscal_Year) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22))
def_data %>% filter(Fiscal_Year==2021) %>% group_by(Competition.sum) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22))

(
ToplineCompMulti<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1991),
  chart_geom = "Bar Chart",
  share = FALSE,
 ...
 #             )+labs(title=NULL,
  ...
  labs(x="Fiscal Year",
    ...
    date_x_year_breaks(1991,2021, by=2)
)

(
ToplineComp<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1997),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//topline_comp.png", ToplineComp,  
             width=11, height= 5.5, units="in",size=14, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_comp.emf", ToplineComp+theme(legend.position = "right"),  
             width=6.5, height= 3, units="in",size=11, lineheight=1.2
             )


ggsave600dpi("..//Output//AcqTrends//topline_comp_multi.png", ToplineCompMulti,  
             width=11, height= 5.5, units="in",size=14, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_comp_multi.svg", ToplineCompMulti,  
             width=6.5, height= 4, units="in",size=12, lineheight=1.2
             )



ggsave600dpi("..//Output//AcqTrends//topline_comp_multi.emf", ToplineCompMulti+theme(legend.position = "right"),  
             width=6.5, height= 3, units="in",size=11, lineheight=1.2
             )


ggsave600dpi("..//Output//AcqTrends//topline_comp_square.png", ToplineComp,  
...
log_plot(plot=ToplineComp, df=def_data,
                     filename="acq_fig6_5_competition",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="6-5 Comp",path="../output/AcqTrends/", height=3.5,
                     startRow=1,startCol=11,format=TRUE#,var_list=c("Vehicle.AwardTask","Vehicle.sum7")
         )
```
...
ggsave600dpi("..//Output//AcqTrends//unlabeled_comp.png", UnlabeledComp,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )
...
write.csv(big5_wide,file=file.path("..","output","big5_wide.csv"),row.names=FALSE)
...
ggsave600dpi("..//Output//AcqTrends//topline_vendor_wide.png", ToplineVendor+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies took place in April 2020 and thus did not make the cutofff for inclusion in FY2020."), 
             width=12, height= 6, units="in",size=12,lineheight=1
             )


ggsave600dpi("..//Output//AcqTrends//topline_vendor_cq.png", ToplineVendor+
...
ggsave600dpi("..//Output//AcqTrends//topline_vendor_cq.svg", ToplineVendor+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies took place in April\nof 2020 and thus did not make the cutofff for inclusion in FY2020."), 
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//topline_vendor.png", ToplineVendor, 
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//topline_vendor.eps", ToplineVendor+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies took place in April\nof 2020 and thus did not make the cutofff for inclusion in FY2020."), 
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )
...
write.csv(file="..//Output//AcqTrends//topline_vendor.csv",row.names = FALSE, na = "",
          ToplineVendor$data%>% mutate(Fiscal_Year=lubridate::year(dFYear),
            ...
            pivot_wider(id_cols=c(Shiny.VendorSize),
                        names_from=Fiscal_Year,values_from=bAction_Obligation_OMB24_GDP22)%>%arrange(Shiny.VendorSize))
...
ggsave600dpi("..//Output//AcqTrends//topline_vehicle7.png", ToplineVehicle7, 
...
ggsave600dpi("..//Output//AcqTrends//topline_vehicle.png", ToplineVehicle, 
             width=13, height= 4.5, units="in",size=11
             )
...
log_plot(plot=ToplineVehicle7, df=def_data,
...
#   
# ggsave600dpi(ToplineVehicle7+labs(caption=NULL,title=NULL),
#             file="../output/AcqTrends/acq_fig4_2_vehicle.svg",size=12,caption_fraction=8/12,lineheight=1, height =4, width=6.5)
# 
# output_TY<-def_data %>% group_by(Vehicle.sum7,Vehicle.AwardTask,Fiscal_Year)%>%
#             dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
#   arrange(Fiscal_Year)%>%
#           pivot_wider(id_cols=c(Vehicle.AwardTask,Vehicle.sum7),
#                       names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%arrange(Vehicle.AwardTask,Vehicle.sum7)
# 
# 
# write.csv(output_TY,file="..//Output//AcqTrends//then_year_csv//acq_fig4_2_vehicle.csv",row.names = FALSE, na = "",
#           )
# 
# wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
# writeData(wb, output_TY, sheet = "4-2 Veh", startRow = 1, startCol = 10)
# saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
# rm(wb)
...
ggsave600dpi("..//Output//AcqTrends//topline_pricing.png", ToplinePricing,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

# write.csv(file="..//Output//AcqTrends//topline_pricing.csv",row.names = FALSE, na = "",
...
#                       names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22),PricingUCA.sumlong)%>%
#     arrange(PricingUCA.sumlong,PricingUCA))


ggsave600dpi(ToplinePricing+labs(caption=NULL,title=NULL),
            file="../output/AcqTrends/acq_fig4_1_pricing.svg",size=12,caption_fraction=8/12,lineheight=1, height =4, width=6.5)

output_TY<-def_data %>% group_by(PricingUCA.sumlong,PricingUCA,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
  arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(PricingUCA.sumlong,PricingUCA),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%arrange(PricingUCA.sumlong,PricingUCA)


write.csv(output_TY,file="..//Output//AcqTrends//then_year_csv//acq_fig4_1_pricing.csv",row.names = FALSE, na = "",
          )

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
writeData(wb, output_TY, sheet = "4-1 Price", startRow = 1, startCol = 10)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
# ggsave600dpi("..//Output//AcqTrends//Topline_Pricing_History.png", PricingHistory, 
...
#              )
  ...
  ggsave600dpi("..//Output//AcqTrends//Topline_Pricing_History.svg", PricingHistory,  
               width=6.5, height= 3, units="in",size=12, lineheight=1.2
               )
  

write.csv(file="..//Output//AcqTrends//Topline_Pricing_History.csv",row.names = FALSE, na = "",
  arrange(pivot_wider(PricingHistory$data %>% 
                        arrange(dFYear) %>% mutate(Fiscal_Year=lubridate::year(dFYear)),
                      id_cols=c(PricingInflation),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22),PricingInflation)%>%
    arrange(PricingInflation))

write.csv(file="..//Output//AcqTrends//Topline_Pricing_History_cur.csv",row.names = FALSE, na = "",
  PricingUpdate %>% group_by(Fiscal_Year,PricingInflation)%>%
                        summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE)) %>%
                        arrange(Fiscal_Year) %>%# mutate(Fiscal_Year=lubridate::year(dFYear)) %>%
                        pivot_wider(
                      id_cols=c(PricingInflation),
...
Commercial<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//topline_commercial.png", Commercial, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_commercial.svg", Commercial+
  labs(y="Obligations\n(Constant 2022 $s)"),#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 2.25, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi(Commercial+labs(caption=NULL,title=NULL,y="Obligations\n(Constant 2022 $s)"),
            file="../output/AcqTrends/acq_fig5_4_commercial.svg",size=12,caption_fraction=8/12,lineheight=1, height =2.25, width=6.5)


output_TY<-def_data %>% group_by(AnyCommercialText,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
  arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%arrange(AnyCommercialText)


write.csv(output_TY,file="..//Output//AcqTrends//then_year_csv//acq_fig5_4_commercial.csv",row.names = FALSE, na = "",
          )

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
writeData(wb, output_TY, sheet = "5-4 Comm", startRow = 1, startCol = 11)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
ggsave600dpi("..//Output//AcqTrends//topline_UnmDuration.png", UnmDuration, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )


ggsave600dpi("..//Output//AcqTrends//topline_CurDuration.png", CurDuration, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_CurDuration.svg", CurDuration,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 3, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_UnmDuration.png", UnmDuration,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 3, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_UnmDuration.svg", UnmDuration,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 3, units="in",size=11,lineheight = 1.2
             )

write.csv(file="..//Output//AcqTrends//topline_CurDuration_current.csv",row.names = FALSE, na = "",
...
write.csv(file="..//Output//AcqTrends//topline_UmdDuration_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(UnmodifiedUltimateDurationCategory,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(UnmodifiedUltimateDurationCategory),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
            arrange(UnmodifiedUltimateDurationCategory))
...
ggsave600dpi("..//Output//AcqTrends//topline_psr.png", ToplinePSR, 
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_psr.svg", ToplinePSR, 
             width=6.5, height= 3, units="in",size=11, lineheight=1.2
             )
...
ggsave600dpi(ToplinePSR+labs(caption=NULL,title=NULL),
            file="../output/AcqTrends/acq_fig3_1_area.svg",size=12,caption_fraction=8/12,lineheight=1, height =3, width=6.5)


output_TY<-def_data %>% group_by(SimpleArea,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SimpleArea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)

write.csv(file="..//Output//AcqTrends//topline_psr.csv",row.names = FALSE, na = "",
          output_TY)

write.csv(output_TY,file="..//Output//AcqTrends//then_year_csv//acq_fig3_1_area.csv",row.names = FALSE, na = "",
          )

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
 writeData(wb, output_TY, sheet = "3-1 Area", startRow = 1, startCol = 9)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
(
PSR_Q<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000&  Fiscal_YQ<=2023.2 & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//PSR_Q.png", PSR_Q, 
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//PSR_Q.svg", PSR_Q, 
             width=6.5, height= 3, units="in",size=11, lineheight=1.2
             )


output_TY<-def_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(SimpleArea,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SimpleArea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)
...
(
RnDphase<-build_plot(
  data=def_data %>% filter(Fiscal_YQ<=2023.2) %>% filter(SimpleArea=="R&D"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//psr_RnDPhase.png", RnDphase, 
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )
                           

write.csv(file="..//Output//AcqTrends//psr_RnDPhase.csv",row.names = FALSE, na = "",
          pivot_wider(RnDphase$data,id_cols=c(ProductServiceOrRnDarea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>%
            arrange(ProductServiceOrRnDarea))
            

output_TY<-def_data %>% filter(Fiscal_YQ<=2023.2) %>% filter(SimpleArea=="R&D") %>%
...
# writeData(wb, output_TY, sheet = "R&D", startRow = 15, startCol = 13)
# saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
contract_merge<-def_data %>% filter(Fiscal_YQ<=2023.2) %>%
  group_by(Fiscal_Year,
                         ProductServiceOrRnDarea,
                         SimpleArea)%>%
  dplyr::summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22),
                   Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year))
...
ggsave600dpi("..//Output//AcqTrends//RnDPhasePlusOTA.png", RnDphasePlusOTA+labs(caption="Source: FPDS and CSIS analysis."),
             width=5, height= 4.75, units="in",size=11, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//RnDPhasePlusOTA.svg", RnDphasePlusOTA+labs(caption="Source: FPDS and CSIS analysis."),
             width=5, height= 4.75, units="in",size=11, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//RnDPhasePlusOTA.emf", RnDphasePlusOTA+labs(caption="Source: FPDS and CSIS analysis."),
             width=5, height= 4.75, units="in",size=11, lineheight=1.2
             )
...
ggsave600dpi("..//Output//AcqTrends//contractota_RnDPhase.png", RnDphase,
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )
#                            
# 
...
(
Services<-build_plot(
  data=def_data %>% filter(SimpleArea=="Services (Non-R&D)" & Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//psr_Services.png", Services, 
             width=12, height= 6, units="in",size=14, lineheight=1.2
             )
              
ggsave600dpi("..//Output//AcqTrends//psr_Services.svg", Services, 
             width=6.5, height= 3.25, units="in",size=10, lineheight=1.2
             )             

write.csv(file="..//Output//AcqTrends//psr_Services.csv",row.names = FALSE, na = "",
          pivot_wider(Services$data,id_cols=c(ProductServiceOrRnDarea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>%
            arrange(ProductServiceOrRnDarea))
            
output_TY<-def_data %>% filter(Fiscal_YQ<=2023.2) %>% 
...
write.csv(file="..//Output//AcqTrends//psr_Services_current.csv",row.names = FALSE, na = "",
          output_TY)

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
writeData(wb, output_TY %>% filter(!ServicesCategory.detail %in% c("Products","Fuels","R&D","Unlabeled")), sheet = "3-3 Serv", startRow = 1, startCol = 9)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
rm(wb)
```
...
ggsave600dpi("..//Output//AcqTrends//psr_vendorsize_dollar.png", SimpleVendor_dollar, 
...
ggsave600dpi("..//Output//AcqTrends//psr_vendorsize_share.png", SimpleVendor_share, 
             width=6.5, height= 5, units="in",size=11, lineheight=1.2
             )
...
PSRComp_line<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1991 & Fiscal_YQ<=2023.2 & SimpleArea!="Unlabeled"),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
 ...
 #             )+labs(title=NULL,
  ...
  labs(x="Fiscal Year",
       y="Obligations (Constant 2022 $s)")+
    date_x_year_breaks(1993,2021,by=4)
)
...
ggsave600dpi("..//Output//AcqTrends//PSRComp_line.png", PSRComp_line, 
...
ggsave("..//Output//AcqTrends//PSRComp_both_keynote.png", 
             gridExtra::grid.arrange(
               PSRComp_bar+labs(
                 title="DOD Contract Obligations by Commercial Categorization, 2000-2022",
                  y="Obligations\n(Const. 2022 $s)",
...
ggsave("..//Output//AcqTrends//acq_fig6_6_competitionPSR.svg", 
             gridExtra::grid.arrange(
               PSRComp_bar+labs(caption = NULL,x=NULL)+theme(legend.position = "none"),
               PSRComp_line), 
...
log_plot(plot=PSRComp_bar, df=def_data,
                     filename="acq_fig6_6_competitionPSR",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="6-6 CompPSR",path="../output/AcqTrends/", height=3.5,
...
SubCustomer<-build_plot(
  data=def_data %>% filter(Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
 ...
 #             )+labs(title=NULL,
  ...
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2022 $s)")
)#+date_x_year_breaks(1990,2022, by=4)

ggsave600dpi("..//Output//AcqTrends//topline_subcustomer.png", SubCustomer, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_subcustomer.svg", SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1), 
...
ggsave600dpi("..//Output//AcqTrends//topline_subcustomer_keynote.svg", SubCustomer+
               facet_wrap(~SubCustomer.sum,nrow=1)+
               labs(title="DOD Contract Obligations by Component, 1990-2022")+
               date_x_year_breaks(1990,2022, by=5), 
             width=13, height= 5.5, units="in", size=20, lineheight = 0.75
             )


ggsave600dpi("..//Output//AcqTrends//topline_platform_bar_keynote.svg", Platform_Bar+
               labs(title="DOD Contract Obligations by Platform Portfolio, 2000-2022"),
             width=13, height= 5.5, units="in", size=20, lineheight = 0.75
             )


ggsave600dpi("..//Output//AcqTrends//topline_subcustomer.emf", SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1), 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi(SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1)+labs(caption=NULL,title=NULL),
            file="../output/AcqTrends/acq_fig2_2_subcustomer.svg",size=12,caption_fraction=8/12,lineheight=1, height =3, width=6.5)


output_TY<-def_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(SubCustomer.sum,SubCustomer.JPO,Fiscal_Year) %>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SubCustomer.sum,SubCustomer.JPO),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)

write.csv(output_TY,file="..//Output//AcqTrends//topline_subcustomer_current.csv",row.names = FALSE, na = "",
          )

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
 writeData(wb, output_TY, sheet = "2-3 Cust", startRow = 1, startCol = 10)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
## SubCustomer Covid19
```{r SubCustomer}
cust_covid<-def_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(SubCustomer.JPO,SubCustomer.sum,Fiscal_Year) %>%
  summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm = TRUE),
            COVID19obligated=sum(COVID19obligated,na.rm = TRUE))%>%
...
ggsave600dpi("..//Output//AcqTrends//subcustomer_covid.png", SubCustomerCovid, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//subcustomer_covid.svg", SubCustomerCovid+facet_wrap(~SubCustomer.sum,nrow=1), 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )
...
# ggsave600dpi("..//Output//AcqTrends//subcustomer_PSR.png", SubCustomerPSR, 
#              width=6.5, height= 4, units="in",size=11
#              )
ggsave600dpi("..//Output//AcqTrends//subcustomer_PSR.png", SubCustomerPSR, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
...
#              width=6.5, height= 4, units="in",size=11
#              )
ggsave600dpi("..//Output//AcqTrends//subcustomer_Comp_line.png", SubCustomerComp_line, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
             )
...
ggsave600dpi("..//Output//AcqTrends//subcustomer_Comp_bar.png", SubCustomerComp_bar, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
             )
...
# ggsave600dpi("..//Output//AcqTrends//SubCustomer_Pricing_History.png", PricingHistory, 
#              width=6.5, height= 4, units="in",size=11
#              )
...
ggsave600dpi("..//Output//AcqTrends//SubCustomer_Pricing_History.svg", PricingHistory,  
             width=6.5, height= 3, units="in",size=12, lineheight=1.2
             )


write.csv(file="..//Output//AcqTrends//SubCustomer_Pricing_History_share.csv",row.names = FALSE, na = "",
  arrange(pivot_wider(PricingHistory$data %>% 
                        arrange(dFYear) %>% mutate(Fiscal_Year=lubridate::year(dFYear)),
                      id_cols=c(PricingInflation),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22),PricingInflation)%>%
...
write.csv(file="..//Output//AcqTrends//SubCustomer_Pricing_History.csv",row.names = FALSE, na = "",
  pricing %>% group_by(Fiscal_Year,PricingInflation)%>%
                        summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE)) %>%
                        arrange(Fiscal_Year) %>%# mutate(Fiscal_Year=lubridate::year(dFYear)) %>%
                        pivot_wider(
                      id_cols=c(PricingInflation),
...
ggsave600dpi("..//Output//AcqTrends//subcustomer_commercial.png", CommercialCust, 
             width=6, height= 5.5, units="in",size=12,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//subcustomer_commercial.svg", CommercialCust,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//subcustomer_commercial_keynote.png", CommercialCust+
               labs(title="DOD Contract Obligations by  Commercial Categorization, 2000-2022"),
             width=13, height= 5.5, units="in", size=20, lineheight = 0.75
             )

ggsave600dpi("..//Output//AcqTrends//subcustomer_commercial_keynote.svg", CommercialCust+
               labs(title="DOD Contract Obligations by  Commercial Categorization, 2000-2022"),
             width=13, height= 5.5, units="in", size=20, lineheight = 0.75
             )


write.csv(file="..//Output//AcqTrends//subcustomer_commercial.csv",row.names = FALSE, na = "",
          CommercialCust$data%>%mutate(Fiscal_Year=year(dFYear)) %>%
          pivot_wider(id_cols=c(SubCustomer.platform,AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22))

write.csv(file="..//Output//AcqTrends//subcustomer_commercial_current.csv",row.names = FALSE, na = "",
          def_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(SubCustomer.platform,AnyCommercialText,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(SubCustomer.platform,AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
...
(
Platform<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1990 & Fiscal_YQ<=2023.2),
  chart_geom = "Line Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//topline_platform.png", Platform, 
             width=12, height= 6, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_platform.svg", Platform, 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_platform.emf", Platform+geom_line(
...
ggsave600dpi("..//Output//AcqTrends//topline_platform_bar.png", Platform_Bar, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_platform_bar.emf", Platform_Bar,
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )
...
ggsave600dpi(Platform_Bar+labs(caption=NULL,title=NULL),
            file="../output/AcqTrends/acq_fig3_2_platform.svg",size=12,caption_fraction=8/12,lineheight=1, height =4, width=6.5)

output_TY<-def_data %>% group_by(PlatformPortfolio,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
write.csv(output_TY,file="..//Output//AcqTrends//then_year_csv//acq_fig3_2_platform.csv",row.names = FALSE, na = "",
          )

wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx")
writeData(wb, output_TY, sheet = "3-2 Plat", startRow = 1, startCol = 9)
saveWorkbook(wb,file="..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx",overwrite = TRUE)
...
ggsave600dpi("..//Output//AcqTrends//platform_subcustomer.png", PlatformSubCustomer, 
...
ggsave600dpi("..//Output//AcqTrends//platform_PSR.png", PlatformPSR, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//platform_products.png", PlatformProducts, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//PlatformRnD.png", PlatformRnD, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
PlatformQ<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2013 & Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  ...
  format=TRUE,
  ytextposition=FALSE
)+scale_x_date("Fiscal Year",labels = label_date("'%y"))+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
ggsave600dpi("..//Output//AcqTrends//PlatformQ.png", PlatformQ, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )

ggsave600dpi("..//Output//AcqTrends//PlatformQ_slide.svg", PlatformQ+
               labs(title="Defense Contract Obligations by Platform Portfolio, FY2013-FY2023Q1")+
...
ggsave600dpi("..//Output//AcqTrends//PlatformQ_keynote.svg", PlatformQ+
               labs(title="Ordnance & Missile Contract Obligations, 2000-2023Q1"),
             width=9, height= 5.5, units="in", size=20, lineheight = 0.75
             )


ggsave600dpi("..//Output//AcqTrends//OnMQ_keynote.png", OnMQ+
               labs(title="Ordnance & Missile Contract Obligations, 2000-2023Q1"),
             width=9, height= 5.5, units="in", size=20, lineheight = 0.75
             )

ggsave600dpi("..//Output//AcqTrends//OnMQ_keynote.svg", OnMQ+
               labs(title="Ordnance & Missile Contract Obligations, 2000-2023Q1"),
             width=9, height= 5.5, units="in", size=20, lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//OnMQ.png", OnMQ, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//platform_vendor_share.png", PlatformVendor_share, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//platform_vendor_dollar.png", PlatformVendor_dollar, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
PlatformComp_bar<-build_plot(
  data=def_data %>% filter(PlatformPortfolio!="Unlabeled"  &Fiscal_Year>=1991 & Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//platform_comp_bar.png", PlatformComp_bar, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )


ggsave600dpi("..//Output//AcqTrends//platform_comp_line.png", PlatformComp_line, 
             width=6.5, height= 4, units="in",size=11,lineheight = 0.75
             )
...
ggsave600dpi("..//Output//AcqTrends//platform_commercial.png", CommercialPlat, 
             width=6, height= 5.5, units="in",size=12,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//platform_commercial.svg", CommercialPlat,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//platform_commercial.emf", CommercialPlat,#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )
...
full_data<-full_data %>% group_by(Fiscal_Year,PlatformPortfolio)%>%
...
PlatformMultiYearShare<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000 &
                             (multiyearcontract==1)), #is.na(multiyearcontract)|
  chart_geom = "Line Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//AcqTrends//platform_multiyear_cur.csv",row.names = FALSE, na = "",
          def_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(PlatformPortfolio,multiyearcontracttext,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(PlatformPortfolio,multiyearcontracttext),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
            arrange(PlatformPortfolio,multiyearcontracttext))

ggsave600dpi("..//Output//AcqTrends//Platform_MultiYear.png", PlatformMultiYear,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//Platform_MultiYearShare.png", PlatformMultiYearShare+facet_wrap(PlatformPortfolio~.,nrow=2)+
                labs(y="Share of Obligations",
...
ggsave("..//Output//AcqTrends//Platform_MultiYearShare_keynote.png", 
               PlatformMultiYearShare+labs(
                 title="DOD Share of Contract Obligations to Multiyear Contracts, 2000-2023Q1")+
...
ggsave("..//Output//AcqTrends//Platform_MultiYearShare_keynote.svg", 
               PlatformMultiYearShare+labs(
                 title="DOD Share of Contract Obligations to Multiyear Contracts, 2000-2023Q1")+
                 facet_wrap(PlatformPortfolio~.,nrow=2)+
                labs(y="Share of Obligations",
...
write.csv(file="..//Output//AcqTrends//PlatformMultiYearShare.csv",row.names = FALSE, na = "",
  pivot_wider(PlatformMultiYearShare$data %>% 
                        arrange(dFYear) %>% mutate(Fiscal_Year=lubridate::year(dFYear)),
                      id_cols=c(PlatformPortfolio,multiyearcontracttext),
...
ukr_data %>% group_by(nationalinterestactioncode)%>%
  filter(Fiscal_Year==2022) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22))
...
ggsave600dpi("..//Output//AcqTrends//Munitions//ukraine_platform_bar.png", Platform_Ukr_Bar, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//Munitions//ukraine_platform_bar.emf", Platform_Ukr_Bar,
             width=6.5, height= 4, units="in",size=11,lineheight = 1.2
             )
...
(
ToplineVendor<-build_plot(
  data=full_data %>% filter(Fiscal_Year<=2021),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
  ...
  format=TRUE,
  ytextposition=FALSE
)+scale_x_date("Fiscal Year",labels = scales::date_format("'%y"),date_breaks = "2 years")+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
ToplineVendorIntl<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000 & Fiscal_Year<=2021),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
ggsave600dpi("..//Output//AcqTrends//topline_vendor_intl_square.png", ToplineVendorIntl+
...
ggsave600dpi("..//Output//AcqTrends//topline_vendor_intl.svg", ToplineVendorIntl+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies is took place in \nApril 2020 and thus did not make the cutofff for inclusion in FY2020."), 
...
write.csv(file="..//Output//AcqTrends//topline_vendor_intl_cur.csv",row.names = FALSE, na = "",
          full_data %>% group_by(VendorSize_Intl,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(VendorSize_Intl),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>% arrange(VendorSize_Intl))



log_plot(plot=ToplineVendor, df=ToplineVendorIntl,
                     filename="acq_fig6_1_vend",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="6-1 Vend",path="../output/AcqTrends/", height=3.5,
                     startRow=1,startCol=11,format=TRUE,#,var_list=c("SimpleArea","Competition.multisum")
         )
...
ggsave600dpi("..//Output//AcqTrends//vendorsize_vehicle.png", VendorSizeVehicle+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies is took place in \nApril 2020 and thus did not make the cutofff for inclusion in FY2020.\nUnlabeled vendor size excluded."),
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//vendorsize_vehicle.svg", VendorSizeVehicle+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies is took place in \nApril 2020 and thus did not make the cutofff for inclusion in FY2020\nUnlabeled vendor size excluded.."),
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//vendorsize_awardtaskshare.svg", VendorSizeVehicleSumLine
+
...
ggsave600dpi("..//Output//AcqTrends//vendorsize_awardtask.svg", VendorSizeVehicleSum
+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.\nNote: The merger of Raytheon and United Technologies is took place in \nApril 2020 and thus did not make the cutofff for inclusion in FY2020.\nUnlabeled vendor size excluded."),
             width=6.5, height= 3.25, units="in",size=11,lineheight=1
             )
...
(
ToplineVendor_line<-build_plot(
  data=def_data %>% filter(Fiscal_Year<=2021 &  Fiscal_Year>=2000 & !is.na(SimpleArea) & SimpleArea!="Unlabeled"),
  chart_geom = "Line Chart",
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  x_var="dFYear", #x_var
...
def_lc<-prepare_labels_and_colors(def_data %>% select(-PricingMechanism),path=local_path)

(
VendorSizePricing2021share<-build_plot(
  data=def_data %>% filter(Fiscal_Year==2021& Shiny.VendorSize!="Unlabeled" & CurrentDurationIsYear!="Unlabeled"&
                             PricingUCA.sum!="Unclear"),
  chart_geom = "Bar Chart",
  share = TRUE,
  # NA, #VAR.ncol
  x_var="PricingInflation.1yearUCA", #x_var
  y_var="Action_Obligation_OMB24_GDP22", #VAR.y.variable
  color_var="PricingInflation.1yearUCA", #color_var
  facet_var="Shiny.VendorSize", #facet_var
  # second_var="CurrentDurationIsYear",
  labels_and_colors=def_lc,
  column_key=def_ck,
...
ggsave600dpi("..//Output//AcqTrends//VendorSize_Pricing2021share.png", VendorSizePricing2021share+
               facet_wrap(~Shiny.VendorSize,nrow=1),
             width=6.5, height= 3, units="in",size=10,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//VendorSize_Pricing2021share.svg", VendorSizePricing2021share+
               facet_wrap(~Shiny.VendorSize,nrow=1),
             width=6.5, height= 3, units="in",size=10,lineheight=1)



ggsave600dpi("..//Output//AcqTrends//vendorsize_pricing.png", VendorSizePricing+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS; CSIS analysis.\nNote: The merger of Raytheon and United Technologies is took place in \nApril 2020 and thus did not make the cutofff for inclusion in FY2020."),
             width=6, height= 5, units="in",size=12,lineheight=1
             )

ggsave600dpi("..//Output//AcqTrends//vendorsize_pricing_2021.png", VendorSizePricing2021+theme(legend.position = "none")+
  labs(y="Obligations (Constant 2022 $s)",caption="Source: FPDS and CSIS analysis.")+facet_wrap(~Shiny.VendorSize,nrow=1),
             width=6.5, height= 2.5, units="in",size=12,lineheight=1
             )
...
write.csv(file="..//Output//AcqTrends//vendorsize_pricing_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(Shiny.VendorSize,PricingUCA.sum,Fiscal_Year)%>%
            filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.2)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
write.csv(file="..//Output//AcqTrends//VendorSize_Pricing2021.csv",row.names = FALSE, na = "",
          def_data %>% group_by(Shiny.VendorSize,PricingUCA.1year,Fiscal_Year)%>%
            filter(Fiscal_Year>=2007 & Fiscal_YQ<=2023.2) %>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
#     ifelse(multiyearcontracttext=="Multiyear Contract",Action_Obligation_OMB24_GDP22,0),na.rm=TRUE))

full_data %>% group_by(Vehicle,multiyearcontracttext) %>%
  dplyr::summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE))

levels(factor(full_data$Vehicle))
...
VehicleMultiYearShare<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000 & Vehicle.sum7!="Unlabeled"&
...
ggsave600dpi("..//Output//AcqTrends//Vehicle_MultiYear.png", VehicleMultiYear,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//Vehicle_MultiYearShare.png", VehicleMultiYearShare,  
             width=6.5, height= 4, units="in",size=12, lineheight=1.2
             )
...
ggsave600dpi("..//Output//AcqTrends//PSR_MultiYear.png", PSRMultiYear,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//AcqTrends//PSR_MultiYearShare.png", PSRMultiYearShare,  
             width=6.5, height= 4, units="in",size=12, lineheight=1.2
             )
...
ggsave600dpi("..//Output//AcqTrends//platform_GFE.png", GFEplat, 
             width=12, height= 6, units="in",size=12,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//platform_GFE.svg", GFEplat, 
             width=6.5, height= 4.5, units="in",size=11,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//platform_GFE.emf", GFEplat, 
             width=6.5, height= 4.5, units="in",size=11,lineheight = 1.2
             )
...
write.csv(file="..//Output//AcqTrends//platform_GFE_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(PlatformPortfolio,gfe_gfp_value,Fiscal_Year)%>%
            filter(Fiscal_Year>=2004 & Fiscal_YQ<=2023.2)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
...
write.csv(full_data %>% group_by(PlaceOfManufactureText,PlaceIsForeign,VendorIsForeign,OriginIsForeign) %>%
  summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22)) %>% pivot_wider(names_from=PlaceOfManufactureText,
                                                                                       values_from=Action_Obligation_OMB24_GDP22),
...
write.csv(file="..//Output//AcqTrends//ota//ntibintl_plat.csv",row.names = FALSE,na = "",
          ntibintl_plat$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(VendorNTIB,PlatformPortfolioUAV),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB24_GDP22)%>% arrange(VendorNTIB,PlatformPortfolioUAV))
