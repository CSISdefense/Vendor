...
PricingUpdate<-rbind(pricing[,colnames(pricing) %in% colnames(def_data)],
...
PricingHistory<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  theme(legend.position = "right",
    ...
    xlim(c(as.Date("1980-01-01"), as.Date("2022-12-30")))+
...
write.csv(file="..//Output//Topline_Pricing_History_share.csv",row.names = FALSE, na = "",
...
PlatformMultiYearShare<-build_plot(
  data=full_data %>% filter(Fiscal_Year>=2000 & !PlatformPortfolio %in% c("Unlabeled","Other Products","Other Services","Other R&D and Knowledge Based")&
