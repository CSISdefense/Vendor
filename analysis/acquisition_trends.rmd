...
write.csv(file="..//Output//toa_annual.csv",row.names = FALSE,na="",
...
ggsave600dpi("..//Output//topline_comp.eps", ToplineComp,  
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )
...
ggsave600dpi("..//Output//topline_vendor_cq.eps", ToplineVendor+
  labs(y="Obligations (Constant 2020 $s)",caption="Source: FPDS; CSIS Analysis.\nNote: The Merger of Raytheon and United Technologies took place in April\nof 2020 and thus did not make the cutofff for inclusion in FY2020."), 
             width=6.5, height= 4, units="in",size=11,lineheight=1
             )
...
ggsave600dpi("..//Output//RnDPhasePlusOTA.eps", RnDphasePlusOTA+labs(caption="Source: FPDS and CSIS analysis."),
             width=5, height= 4.75, units="in",size=11, lineheight=1.2
             )
#                            
# 
...
(waiverdetail <-  build_plot(data=full_data %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                            ...
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB20_GDP20",
                            color_var="OriginManufactureVendor",
                            facet_var="SimpleArea",
  ...
  ggsave600dpi(waiverdetail,file="../output/waiverdetail.png",size=8,lineheight=1, height =5, width=9)
...
(waiverdetail <-  build_plot(data=full_data %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                            ...
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB20_GDP20",
                            color_var="PlaceOfManufactureText",
                            facet_var="SimpleArea",
                            # second_var="StateRegion",
                            # labels_and_colors=labels_and_colors,
                            # column_key=column_key,
  ...
  ggsave600dpi(waiverdetail,file="../output/waiverdetail.png",size=8,lineheight=1, height =5, width=9)
    ...
    write.csv(file="..//Output//waiverdetail.csv",row.names = FALSE,
            # ToplineComp$data%>% mutate(dFYear=lubridate::year(dFYear))%>%
              pivot_wider(waiverdetail$data,id_cols=c(PlaceOfManufactureText,SimpleArea),
                          names_from=Fiscal_Year,values_from=Action_Obligation_OMB20_GDP20)
            %>% arrange(PlaceOfManufactureText,SimpleArea))

```
### Waiverplace
```{r WaiverPlace}
  (waiverplace <-  build_plot(data=full_data %>% filter(PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1|OriginIsForeign==1) ,
                            chart_geom="Bar Chart",
...
ggsave600dpi(waiverplace,file="../output/waverplace.png",size=8,lineheight=1, height =5, width=9)

(waiver <-  build_plot(data=full_data %>% filter((PlaceOfManufacture_Sum %in% c("In U.S. but Foreign Services or Content",
                                                                          "Manufactured Outside U.S.",
                                                                          "Performed/Manufactured Outside U.S.")|
                                                   VendorIsForeign==1 | OriginIsForeign==1)&
                                                   SimpleArea!="Unlabeled") ,
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal_Year",
                            y_var="Action_Obligation_OMB20_GDP20",
                            color_var="PlaceOfManufactureText20",
                            facet_var="PlaceOfManufactureText20",
                            # second_var="StateRegion",
                            # labels_and_colors=labels_and_colors,
                            # column_key=column_key,
                            legend=TRUE,
  ...
  labs(#x="Year of Delivery (Calendar or Fiscal Varies by Source)",
       y="Obligations (Constant FY 2020 $s)",
       caption="Excludes unlabeled. Source: FPDS; CSIS analysis.",
       main="DoD Contracts with Foreign Manufacture, Vendor, or Origin")+
    theme(legend.position = "right")
    # guides(fill=guide_legend(ncol=2)
)
...
summary(full_data$PlaceStateRegion)
full_data$PlaceOfManufactureText20<-factor_wrap(full_data$PlaceOfManufactureText)
# undebug(build_plot)
summary(factor(full_data$PlaceOfManufactureText20))
...
ggsave600dpi(placeregion,file="../output/place_of_manufacture_place_regionwaivr.png",size=8,lineheight=1, height =5, width=9)
