...
PricingUpdate<-rbind(pricing[pricing$Fiscal_Year>=2000,colnames(pricing) %in% colnames(def_data)],
                     def_data[def_data$Fiscal_Year<2000,
...
log_plot(plot=PricingHistory, df=def_data  %>% filter(Fiscal_YQ<=2025.2 & Fiscal_Year>=2000), 
...
PSRComp_bar<-build_plot(
  ...
  labs(x="Fiscal Year",
    ...
    date_x_year_breaks(1993,2023,by=6,partial_year=2025,partial_label="\nQ1-Q2")
)
...
CustVendor_dollar<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2007 & Fiscal_YQ<=2025.2 & !is.na(SubCustomer.platform) & SubCustomer.platform!="Unlabeled"),
  chart_geom = "Bar Chart",
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
  x_var="dFYear",alpha_var="YTD", #x_var
...
PlatformVendorIntl_share<-build_plot(
  data=def_data %>% filter(PlatformPortfolio!="Unlabeled" & Fiscal_YQ<=2025.2 & Fiscal_YQ>=2000) ,
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
log_plot(plot=PlatformVendorIntl_dollar, df=def_data  %>% filter(Fiscal_YQ<=2025.2),
...
PlatformVendorNTDC_share<-build_plot(
  data=def_data %>% filter(PlatformPortfolio!="Unlabeled" & Fiscal_YQ<=2025.2 & Fiscal_YQ>=2007) ,
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
# 
# log_plot(plot=PlatformVendorNTDC_dollar, df=def_data  %>% filter(Fiscal_YQ<=2025.2),
...
ToplineVendorIntl<-build_plot(
  ...
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2023 $s)",
       title="DOD Contract Obligations by Vendor Size, FY 2000–FY 2025 Q1-Q2",       caption="Source: FPDS and CSIS analysis.")
)
...
ggsave600dpi(path="..//Output//AcqTrends//",
             filename="topline_vendor_intl_keynote.svg", ToplineVendorIntl+
               labs(title="DOD Contract Obligations, FY 2000–FY 2025 Q2"),
             width=9, height= 5.5, units="in", size=20, lineheight = 0.75
             )

log_plot(plot=ToplineVendorIntl, df=def_data  %>% filter(Fiscal_YQ<=2025.2),
...
log_plot(plot=ToplineVendorIntl, df=def_data %>% filter(Fiscal_YQ<=2025.2),
                     ...
                     sheet="5a VendIntl",path="../output/AcqTrends/NPS2025/",
...
ToplineVendorNTDC<-build_plot(
  ...
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2023 $s)",
       title="DOD Contract Obligations by Vendor Size, FY 2000–FY 2025 Q1-Q2",
       caption="Source: FPDS and CSIS analysis.")
)
...
log_plot(plot=ToplineVendorNTDC, df=def_data  %>% filter(Fiscal_YQ<=2025.2),
...
log_plot(plot=ToplineVendorNTDC, df=def_data %>% filter(Fiscal_YQ<=2025.2),
                     ...
                     sheet="5 VendNTDC",path="../output/AcqTrends/NPS2025/",
         height=3,include_YTD=TRUE,second_path=online_path,
                     startRow=1,format=TRUE,excel_y_var=TRUE,excel_formulas = TRUE,
         suppress_doc_svg_text="title",output_slide_png=TRUE,
         output_doc_svg=TRUE
...
(
ToplineVendor<-build_plot(
  data=def_data %>% filter(Fiscal_Year<=2020),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//AcqTrends//topline_vendor_cur.csv",row.names = FALSE, na = "",
          def_data %>% group_by(Shiny.VendorSize,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
