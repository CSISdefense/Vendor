...
## R&D phase and OTA
```{r Services}
contract_data<-full_data %>% filter(fiscal_year >=2015)
contract_data<-format_data_for_plot(contract_data,
...
ota_def<-format_data_for_plot(ota_def,
                     fy_var="Fiscal_Year", #x_var
...
colnames(contract_data)
colnames(ota_def)[colnames(ota_def)=="Dollars_Obligated_OMB20_GDP20"]<-"Action_Obligation_OMB20_GDP20"
ota_def$type<-"OTA"
colnames(ota_def)[colnames(ota_def)=="Fiscal_Year"]<-"fiscal_year"
contract_data$type<-"contract"
contract_data<-rbind(ota_def,contract_data)
