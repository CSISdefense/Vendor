...
toa<-deflate(toa,money_var = "Total_Obligation_Authority",deflator_var="OMB24_GDP22")
toa$dFYear<-as.Date(paste(toa$Fiscal_Year,1,1,sep="-"))

def_data$YTD<-ifelse(def_data$Fiscal_Year==max(def_data$Fiscal_Year),"YTD","Full Year")
summary(factor(def_data$YTD))
full_data$YTD<-ifelse(full_data$Fiscal_Year==max(full_data$Fiscal_Year),"YTD","Full Year")
...
fpds_annual<-full_data %>% filter(Fiscal_YQ<=2023.2) %>% group_by(Fiscal_Year,dFYear,IsFMS,
                                    ContractingCustomer,YTD) %>% summarise(Action_Obligation_OMB24_GDP22=sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE),
...
fpds_annual$Total_Obligation_Authority_OMB24_GDP22<-NA
...
fpds_annual$FMS[!is.na(fpds_annual$Total_Obligation_Authority_OMB24_GDP22)]<-"Obligations (no FMS)"
...
(topline<-build_plot(data=fpds_annual %>% filter(Fiscal_Year>=1990),#rbind(quarterly,q2021,e2021),
    ...
    geom_path(aes(x=dFYear,y=Total_Obligation_Authority_OMB24_GDP22,color=TOA))+
  # scale_alpha_discrete(range=c(0.4,1))+
    labs(x="Fiscal Year",y="Constant FY 2022 $ Billions",fill="Contract",color="Budget",
         caption="Source: FPDS, FY 2024 DoD Greenbook, CSIS Analysis")+
    theme(legend.position = "right")+
    date_x_year_breaks(1990,2028,3,partial_year=2023,partial_label="\nQ1-Q2")+scale_alpha_ordinal(range=c(1,0.5))
)
...
log_plot(plot=topline, df=output_TY   ,y_var="Dollars",#%>% filter(Fiscal_YQ<=2023.2),
                     filename="acq_fig2_1_toa_annual",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="2-1 TOA",path="../output/AcqTrends/", height=3,
...
log_plot(plot=ToplineComp, df=def_data %>% filter(Fiscal_YQ<=2023.2),
...
ToplineVendor_line<-build_plot(
  ...
  ytextposition=FALSE,
  share=TRUE
)+date_x_year_breaks(2000,2022,2,partial_year=2023,partial_label="\nQ1-Q2")+   scale_alpha_ordinal(range=c(1,0.5))+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
log_plot(plot=ToplineVehicle7, df=def_data %>% filter(Fiscal_YQ<=2023.2),
...
for(p in platform_levels){
  ...
  (BreakoutPricing<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2000&PlatformPortfolio==p),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
  ...
  theme(legend.position = "right")+
    labs(y="Obligations (Constant 2022 $s)",main=p))

  if(!dir.exists(file.path("../output/AcqTrends/Platform",file_p)))
    dir.create(file.path("../output/AcqTrends/Platform",file_p))
  undebug(log_plot)
  log_plot(plot=Breakout, df=def_data  %>% filter(Fiscal_YQ<=2023.2&PlatformPortfolio==p),
...
for(c in subcustomer_levels){
  ...
  (Breakout<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2000&SubCustomer.sum==c& Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
  ...
  theme(legend.position = "right")+
    labs(y="Obligations (Constant 2022 $s)",main=c))
  
  if(!dir.exists(file.path("../output/AcqTrends/Customer",file_c)))
    dir.create(file.path("../output/AcqTrends/Customer",file_c))
  
  log_plot(plot=Breakout, df=def_data  %>% filter(Fiscal_YQ<=2023.2&SubCustomer.sum==c),
...
ToplinePricing<-build_plot(
  ...
  ytextposition=FALSE,
  alpha_var="YTD",
)+date_x_year_breaks(2000,2020,6,partial_year=2023,partial_label="\nQ1-Q2")+scale_alpha_ordinal(range=c(1,0.5))+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
log_plot(plot=ToplinePricing, df=def_data  %>% filter(Fiscal_YQ<=2023.2),
                     filename="acq_fig4_1_pricing",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="4-1 Price",path="../output/AcqTrends/", height=3.5,
...
log_plot(plot=PricingHistory, df=PricingUpdate  %>% filter(Fiscal_Year<=2022),
...
full_data<- full_data %>%      group_by(Fiscal_Year,IsFMS) %>%
                               dplyr::mutate(Action_FMS_Share=Action_Obligation_OMB24_GDP22/
                                           sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE))%>% 
                               group_by(Fiscal_Year,IsFMS,ProductOrService) %>%
                               dplyr::mutate(Action_FMS_PS_Share=Action_Obligation_OMB24_GDP22/
                                           sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE))%>% 
                               group_by(Fiscal_Year,IsFMS,SimpleArea) %>%
                               dplyr::mutate(Action_FMS_PSR_Share=Action_Obligation_OMB24_GDP22/
                                           sum(Action_Obligation_OMB24_GDP22,na.rm=TRUE))
...
# undebug(build_plot)
# undebug(build_plot)
(fpds_pricing <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<=2023.2),
                            chart_geom="Line Chart",
                            x_var="dFYear",
                            ...
                            labels_and_colors=fpds_labels_and_colors,
                            column_key=fpds_column_key,
                            legend=TRUE,
                            caption=FALSE,
                            format=TRUE,
                            alpha_var="YTD",
)+date_x_year_breaks(2012,2022,2,partial_year=2023,partial_label="\nQ1-Q2")+
    scale_alpha_ordinal(range=c(1,0.5))+theme(legend.position = "right")+
...
(fpds_pricing_bar <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<=2023.2),
                            chart_geom="Bar Chart",
                            x_var="dFYear",
                            y_var="Action_Obligation_OMB24_GDP22",
...
(fpds_pricing_psr <-  build_plot(data=full_data %>% filter(Fiscal_Year>=2012 & Fiscal_YQ<=2023.2) %>%
                                   filter(SimpleArea!="Unlabeled"),
                            chart_geom="Line Chart",
                            x_var="dFYear",
...
(fpds_pricing_psr_bar <-  build_plot(data=full_data  %>% filter(SimpleArea!="Unlabeled")%>%
                                       filter(Fiscal_Year>=2012 & Fiscal_YQ<=2023.2),
                            chart_geom="Bar Chart",
                            x_var="dFYear",
                            y_var="Action_Obligation_OMB24_GDP22",
                            color_var="PricingUCA",
...
log_plot(plot=fpds_pricing_psr_bar, df=full_data  %>% filter(Fiscal_YQ<=2023.2),
...
log_plot(plot=Commercial+labs(y="Obligations\n(Constant 2022 $s)"), df=def_data %>% filter(Fiscal_YQ<=2023.2),
...
log_plot(plot=UnmDuration, df=def_data  %>% filter(Fiscal_YQ<=2023.2),
...
## Topline Multiyear
```{r Multiyear}
def_data$AnyMultiyearText<-factor(def_data$AnyMultiyear)
levels(def_data$AnyMultiyearText)<-list(
  "Not Classified\nas Multiyear"="N",
  "Non-Development\nor Multiyear Similar"= "NonDev",
  "Any Multiyear\nClassification"="Y"
...
Multiyear<-build_plot(
  ...
  x_var="dFYear",alpha_var="YTD", #x_var
  y_var="Action_Obligation_OMB24_GDP22", #VAR.y.variable
  color_var="AnyMultiyearText", #color_var
  # facet_var="SubCustomer.platform", #facet_var
  column_key=def_ck,
  format=TRUE,
  ytextposition=FALSE
...
ggsave600dpi("..//Output//AcqTrends//topline_Multiyear.png", Multiyear, 
             width=11, height= 5.5, units="in",size=14,lineheight = 1.2
             )

ggsave600dpi("..//Output//AcqTrends//topline_Multiyear.svg", Multiyear+
  labs(y="Obligations\n(Constant 2022 $s)"),#+facet_wrap(~SubCustomer.platform,nrow=1), 
             width=6.5, height= 2.25, units="in",size=11,lineheight = 1.2
             )


log_plot(plot=Multiyear, df=def_data %>% filter(Fiscal_YQ<=2023.2),
                     filename="acq_fig5_4_Multiyear",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="5-4 Comm",path="../output/AcqTrends/", height=2.25,
                     startRow=1,startCol=12,format=TRUE,#var_list=c("SimpleArea","Competition.multisum"),
         output_plot=FALSE
         )
...
for(p in platform_levels){
  file_p<-gsub("__*","_",gsub("&","and",gsub("[ |,]","_",p)))
  (Breakout<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2000&PlatformPortfolio==p& Fiscal_YQ<=2023.2),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
log_plot(plot=Services, df=def_data  %>% filter(Fiscal_YQ<=2023.2 &SimpleArea=="Services (Non-R&D)"),
...
PSRComp_line<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  labs(x="Fiscal Year",
       y="Obligations (Constant 2022 $s)")+
    date_x_year_breaks(1993,2021,by=6)
)
...
PSRComp_bar<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  labs(x="Fiscal Year",
       y="Obligations (Constant 2022 $s)")+
    date_x_year_breaks(1993,2021,by=6,partial_year=2023,partial_label="\nQ1-Q2")+   scale_alpha_ordinal(range=c(1,0.5))
)
...
ggsave("..//Output//AcqTrends//acq_fig6_6_PSR_competition.svg", 
             gridExtra::grid.arrange(
               ...
               PSRComp_line+labs(y="Share of\nObl.",caption=NULL)+theme(text=element_text(size=12),
               plot.caption = element_text(size=round(20 * 5/6,0))
               )),
...
SubCustomer<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  # color="Competition")+
  theme(legend.position = "right")+
  labs(y="Obligations (Constant 2022 $s)") +date_x_year_breaks(1990,2020,5,partial_year=2023,partial_label="\nQ1-Q2")+
  scale_alpha_ordinal(range=c(1,0.5))
)
...
log_plot(plot=SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1)+
           date_x_year_breaks(1990,2020,8,partial_year=2023,partial_label="\nQ1-Q2"),
         df=def_data %>% filter(Fiscal_YQ<=2023.2),
...
log_plot(plot=CommercialCust, df=def_data %>% filter(Fiscal_YQ<=2023.2),
...
PlatformMultiYear<-build_plot(
  ...
  format=TRUE,
  ytextposition=FALSE
)+date_x_year_breaks(2000,2022,6,partial_year=2023,partial_label="\nQ1-Q2")+   scale_alpha_ordinal(range=c(1,0.5))+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
log_plot(PlatformMultiYearShare, full_data %>% filter(Fiscal_YQ<=2023.2) ,
...
ToplineVendor_line<-build_plot(
  ...
  ytextposition=FALSE,
  share=TRUE
)+date_x_year_breaks(1990,2019,5,partial_year=2023,partial_label="\nQ1-Q2")+   scale_alpha_ordinal(range=c(1,0.5))+
  # theme(strip.text.y = element_text(angle=0))+
 # theme(axis.text.x = element_text(angle=90))+
 #    scale_y_continuous("Percent of Obligations", labels = percent_format(accuracy=1))+
 #      facet_grid(.~Competition.sum ,scales="free_x", space="free_x"
...
log_plot(plot=ToplineVendor, df=full_data  %>% filter(Fiscal_Year<=2022),
                     filename="acq_fig6_1_vendor",xlsx="DoD_Acq_Trends_Contracts.xlsx",
...
log_plot(plot=ToplineVendorIntl, df=full_data  %>% filter(Fiscal_Year<=2022),
...
log_plot(plot=ToplineVendorIntl, df=full_data  %>% filter(Fiscal_Year<=2022),
                     filename="acq_fig7_X_vendor_intl",xlsx="DoD_Acq_Trends_Contracts.xlsx",
...
log_plot(plot=waiversimple, df=full_data   %>% filter(Fiscal_Year<=2022),
                     filename="acq_fig7_4_ManufacturePlace",xlsx="DoD_Acq_Trends_Contracts.xlsx",
                     sheet="7-4 PS_MFG",path="../output/AcqTrends/", height=3,
                     startRow=1,startCol=12,format=TRUE,var_list=c("Service","PlaceOfManufacture_DoD09")
         )
