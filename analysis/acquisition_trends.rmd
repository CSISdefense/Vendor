...
output_TY<-topline$data %>%
  ...
  filter(Fiscal_Year>=1990)%>%
          format_data_for_plot(
                               ...
                               group=TRUE) %>%
            arrange(Fiscal_Year) %>% deflate(money_var="Dollars",
...
log_plot(plot=topline, 
         df=output_TY   ,y_var="Then_Year_Dollars",#%>% filter(Fiscal_YQ<2024),
                     ...
                     sheet="2-1 TOA",path="../output/AcqTrends/",
         height=3,excel_formulas = TRUE,
  ...
  log_plot(plot=topline, 
           df=output_TY ,y_var="Dollars_OMB25_GDP23",#%>% filter(Fiscal_YQ<2024),
                       filename="nps_fig1_budg_annual",xlsx="DoD_2024_NPS.xlsx",
                       sheet="1 Budg",path="../output/AcqTrends/NPS2024/",second_path=online_path,
           height=3,excel_formulas = TRUE,include_YTD=FALSE,
                       startRow=1,format=TRUE,var_list=c("source","FMS"),excel_y_var = TRUE,
           output_doc_svg=TRUE
           )
```
...
log_plot(plot=SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1)+
           date_x_year_breaks(1990,2022,8),#,partial_year=2024,partial_label="\nQ1"),
...
ggsave600dpi(path="..//Output//AcqTrends//",filename="Platform_MultiYearShare_keynote.svg", 
               PlatformMultiYearShare+labs(
                 title="DOD Share of Contract Obligations to Multiyear Contracts, 2000–2023")+
                 facet_wrap(PlatformPortfolio~.,nrow=2)+
                labs(y="Share of Obligations",
...
log_plot(plot=OTAPSR, df=def_kota %>% filter(Fiscal_YQ<=2023.2),
                     filename="nps_fig2_kota_areatype",xlsx="DoD_2024_NPS.xlsx",
                     sheet="5 Vend",path="../output/AcqTrends/NPS2024/",second_path=online_path,
