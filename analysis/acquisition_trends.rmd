...
(toplineq<-build_plot(data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.1),#rbind(quarterly,q2021,e2021),
...
(
PSR_Q<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000&  Fiscal_YQ<=2023.1 & SimpleArea!="Unlabeled"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//topline_subcustomer_current.csv",row.names = FALSE, na = "",
          def_data %>% filter(Fiscal_YQ<2023) %>% group_by(SubCustomer.sum,SubCustomer.JPO,Fiscal_Year) %>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SubCustomer.sum,SubCustomer.JPO),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year))
...
PlatformQ<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2013 & Fiscal_YQ<=2023.1),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
(
OnMQ<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<=2023.1 &
...
```{r ToplinePricing}
    ...
    full_data<-def_data%>% mutate(multiyearcontracttext=factor(multiyearcontract,levels=c("0","1"),
...
PlatformMultiYearShare<-build_plot(
 ...
 #             )+labs(title=NULL,
  ...
  theme(legend.position = "bottom")+
  labs(y="Share of Obligations",
       caption="Other products / services / R&D not shown.\nWhen the multiyear is blank, transactions are treated as not multiyear.")+
