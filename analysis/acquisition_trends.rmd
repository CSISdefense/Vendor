...
wb <- loadWorkbook("..//Output//AcqTrends//DoD_Acq_Trends_Contracts.xlsx", create = TRUE)
...
## SAM Product/Service/R&D
```{r ToplinePSR}
def_psc<-read_csv("..\\Data_Raw\\SAM\\PSC_code_2011_2022.csv")

def_psc<-standardize_variable_names(def_psc)

def_psc<-apply_standard_lookups(def_psc,path="..\\..\\Lookup-Tables\\")
sam_lc<-prepare_labels_and_colors(def_psc)


# def_data<-deflate(def_data,"Action_Obligation_Then_Year")


def_psc_join<-def_psc %>% filter(Fiscal_Year==2022)
def_psc_join<-rbind(def_psc_join[,colnames(def_psc)%in%colnames(def_data)],
                    def_data[,colnames(def_data)%in%colnames(def_psc)])

def_psc_join$SimpleArea<-factor(def_psc_join$SimpleArea)
levels(def_psc_join$SimpleArea)<-list(
  "Products"=c("Products","Products (All)"),
  "R&D"="R&D",
  "Services"=c( "Services (Non-R&D)","Services"),
  "Unlabeled"="Unlabeled"  
)

(
ToplinePSRsam<-build_plot(
  data=def_psc_join,
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=sam_lc,
  # NA, #VAR.ncol
  x_var="dFYear", #x_var
  y_var="Action_Obligation_OMB24_GDP22", #VAR.y.variable
  color_var="SimpleArea", #color_var
...
ggsave600dpi("..//Output//topline_sam_psr.png", ToplinePSRsam, 
             width=12, height= 6, units="in",size=12, lineheight=1.2
             )

ggsave600dpi("..//Output//topline_sam_psr.emf", ToplinePSRsam, 
             width=6.5, height= 3, units="in",size=11, lineheight=1.2
             )
...
output_TY<-def_data %>% group_by(SimpleArea,Fiscal_Year)%>%
          ...
          pivot_wider(id_cols=c(SimpleArea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)
write.csv(file="..//Output//topline_sam_psr.csv",row.names = FALSE, na = "",
          output_TY)
...
output_TY<-def_data %>% filter(Fiscal_YQ<2023) %>% group_by(SimpleArea,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
          pivot_wider(id_cols=c(SimpleArea),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)
...
(
RnDphase<-build_plot(
  data=def_data %>% filter(Fiscal_YQ<2023) %>% filter(SimpleArea=="R&D"),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
contract_merge<-def_data %>% filter(Fiscal_YQ<2023) %>%
...
(
Services<-build_plot(
  data=def_data %>% filter(SimpleArea=="Services (Non-R&D)" & Fiscal_YQ<2023),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
output_TY<-def_data %>% filter(Fiscal_YQ<2023) %>% 
...
PSRComp_line<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1991 & Fiscal_YQ<2023 & SimpleArea!="Unlabeled"),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
ggsave600dpi(SubCustomer+facet_wrap(~SubCustomer.sum,nrow=1)+labs(caption=NULL,title=NULL),
            file="../output/AcqTrends/acq_fig3_2_subcustomer.svg",size=12,caption_fraction=8/12,lineheight=1, height =3, width=6.5)
...
## SubCustomer Covid19
```{r SubCustomer}
cust_covid<-def_data %>% filter(Fiscal_YQ<2023) %>% group_by(SubCustomer.JPO,SubCustomer.sum,Fiscal_Year) %>%
  summarise(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm = TRUE),
            COVID19obligated=sum(COVID19obligated,na.rm = TRUE))%>%
...
SubCustomerComp_line<-build_plot(
  data=def_data %>% filter(Fiscal_YQ<2023),
  chart_geom = "Line Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
CommercialCust<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2000 & Fiscal_YQ<2023),
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//subcustomer_commercial_current.csv",row.names = FALSE, na = "",
          def_data %>% filter(Fiscal_YQ<2023) %>% group_by(SubCustomer.platform,AnyCommercialText,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
          pivot_wider(id_cols=c(SubCustomer.platform,AnyCommercialText),
                      names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year)%>%
...
(
Platform<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=1990 & Fiscal_YQ<2023),
  chart_geom = "Line Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
PlatformComp_bar<-build_plot(
  data=def_data %>% filter(PlatformPortfolio!="Unlabeled"  &Fiscal_Year>=1991 & Fiscal_YQ<2023),
  chart_geom = "Bar Chart",
  share = TRUE,
  labels_and_colors=labels_and_colors,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//platform_multiyear_cur.csv",row.names = FALSE, na = "",
          def_data %>% filter(Fiscal_YQ<2023) %>% group_by(PlatformPortfolio,multiyearcontracttext,Fiscal_Year)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
write.csv(file="..//Output//vendorsize_pricing_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(Shiny.VendorSize,PricingUCA.sum,Fiscal_Year)%>%
            filter(Fiscal_Year>=2000 & Fiscal_YQ<2023)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
write.csv(file="..//Output//VendorSize_Pricing2021.csv",row.names = FALSE, na = "",
          def_data %>% group_by(Shiny.VendorSize,PricingUCA.1year,Fiscal_Year)%>%
            filter(Fiscal_Year>=2007 & Fiscal_YQ<2023) %>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
            arrange(Fiscal_Year)%>%
...
(
GFEcust<-build_plot(
  data=def_data %>% filter(Fiscal_Year>=2007 & Fiscal_YQ<2023) ,
  chart_geom = "Bar Chart",
  share = FALSE,
  labels_and_colors=def_lc,
  # NA, #VAR.ncol
...
write.csv(file="..//Output//platform_GFE_current.csv",row.names = FALSE, na = "",
          def_data %>% group_by(PlatformPortfolio,gfe_gfp_value,Fiscal_Year)%>%
            filter(Fiscal_Year>=2004 & Fiscal_YQ<2023)%>%
            dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
