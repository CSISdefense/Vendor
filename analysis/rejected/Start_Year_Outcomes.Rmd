...
def_end$LastCurrentCompletionYear<-lubridate::year(def_end$LastCurrentCompletionDate)+ifelse(lubridate::month(def_end$LastCurrentCompletionDate)>=10,1,0)

def_end$StartFiscal_Year<-factor(def_end$StartFiscal_Year,levels=c(2007:2017))

def_end %>% dplyr::filter(IsComplete==1) %>% group_by(StartFiscal_Year,LastCurrentCompletionYear) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation,na.rm=TRUE)) %>%
  ggplot(aes(fill=factor(StartFiscal_Year),x=LastCurrentCompletionYear,y=Action_Obligation))+geom_col()


def_end$StartFiscal_Year<-factor(def_end$StartFiscal_Year,levels=c(2017:2007))

def_end %>% group_by(IsComplete,StartFiscal_Year,LastCurrentCompletionYear) %>%
  dplyr::summarise(Action_Obligation=sum(Action_Obligation,na.rm=TRUE)) %>%
  ggplot(aes(fill=factor(StartFiscal_Year),x=LastCurrentCompletionYear,y=Action_Obligation))+geom_col()+facet_wrap(IsComplete~.,ncol=1)



  Completion<-ddply(def_all,
               .(SumOfisChangeOrder,
                 StartFY,
                 Ceil),
               plyr::summarise,
...
## Initial Duration and Year
```{r StartYearDur}
if(!file.exists("../data/semi_clean/Defense_Contract.SP_UnmodifiedScopeHistory.rda")){
  unmod_scope<-read_delim("../data/semi_clean/Defense_Contract.SP_UnmodifiedScopeHistory.txt",delim="\t", na=c("NA","NULL"))
  
  unmod_scope$UnmodifiedDays[unmod_scope$UnmodifiedDays <0]<-NA
  if(min(unmod_scope$UnmodifiedDays,na.rm=TRUE)==0)
    unmod_scope$UnmodifiedDays<-unmod_scope$UnmodifiedDays+1
  
  unmod_scope<-standardize_variable_names(unmod_scope)
  
  unmod_scope<-deflate(unmod_scope,
                        money_var = "obligatedamount",
                        # deflator_var="OMB.2019",
                        fy_var="fiscal_year"
      )
  unmod_scope$capped_UnmodifiedDays <- ifelse(unmod_scope$UnmodifiedDays > 3650, 3650, unmod_scope$UnmodifiedDays)
  
  unmod_scope$FirstYear<-NA
  unmod_scope$FirstYear[unmod_scope$StartFiscal_Year==unmod_scope$Fiscal.Year]<-"First Year"
  unmod_scope$FirstYear[unmod_scope$StartFiscal_Year<unmod_scope$Fiscal.Year]<-"Prior Year Start"
  unmod_scope$FirstYear<-factor(unmod_scope$FirstYear,c("First Year","Prior Year Start"))
  sumary()
  if (!"Dur" %in% colnames(unmod_scope)){
    
    unmod_scope$Dur<-Hmisc::cut2(unmod_scope$UnmodifiedDays,cuts=c(61,214,366,732))
  }
  
  if (levels(unmod_scope$Dur)[[2]] %in% c("[   61,  214)","[     61,    214)")){
    levels(unmod_scope$Dur)<- list(
      "[0 months,~2 months)"=c("[    0,   61)","[    1,   61)","[      1,     61)"),
      "[~2 months,~7 months)"=c("[   61,  214)","[     61,    214)"),
      "[~7 months-~1 year]"=c("[  214,  366)","[    214,    366)"),
      "(~1 year,~2 years]"=c("[  366,  732)","[    366,    732)"),
      "(~2 years+]"=levels(unmod_scope$Dur)[5])
  }
  
  unmod_scope<-standardize_variable_names(unmod_scope)
  unmod_scope<-rename_dataset(unmod_scope)
  unmod_scope<-replace_nas_with_unlabeled(unmod_scope,"Dur")
  save(unmod_scope,file="../data/semi_clean/Contract.SP_UnmodifiedScopeHistory.rda")
} else load(file="../data/semi_clean/Defense_Contract.SP_UnmodifiedScopeHistory.rda")
  # full_data$StateRegionSwr<-factor(full_data$StateRegion,
  #                                  levels=levels(full_data$StateRegion),
  #                                    swr(levels(full_data$StateRegion),10))


scope_labels_and_colors<-prepare_labels_and_colors(unmod_scope)
scope_column_key<-get_column_key(unmod_scope)
```
### Gradiant
```{r DurGradiant}
(gradiant_duration<-ggplot(unmod_scope %>%
...
(gradiant_duration_share<-ggplot(unmod_scope %>%
                              dplyr::filter(Fiscal.Year>=2007 &
                                              !is.na(UnmodifiedDays)&
                                              !is.na(FirstYear)) %>%  
         group_by(Fiscal.Year,FirstYear,capped_UnmodifiedDays) %>% #,StateRegionSwr,Source
...
(staked_duration <-  build_plot(data=unmod_scope %>% filter(Fiscal.Year>=2007&
                                                                    !is.na(FirstYear)&
                                                                    !is.na(Dur)),
                            chart_geom="Bar Chart",
                            share=FALSE,
                            x_var="Fiscal.Year",
                            y_var="obligatedamount_OMB20_GDP18",
                            color_var="Dur",
                            facet_var="FirstYear",
                            # second_var="dtDelivYear_fac",
                            labels_and_colors=scope_labels_and_colors,
                            column_key=scope_column_key,
                            legend=TRUE,
                            caption=FALSE,
                        format=TRUE)+#facet_grid(Source ~ dtDelivYear_fac, scales="free_y")+ scale_x_continuous("Corruption Perceptions Index Score", c(0, 25, 50, 75, 100)) + 
...
(staked_duration_share <-  build_plot(data=unmod_scope %>% filter(Fiscal.Year>=2007&
                                                                    !is.na(FirstYear)&
                                                                    !is.na(Dur)),
                            chart_geom="Bar Chart",
                            share=TRUE,
