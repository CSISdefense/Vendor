...
fed_data<-deflate(fed_data,money_var="Action_Obligation_Then_Year",
                  deflator_var ="OMB23_OUT21")
platpscintl<-deflate(platpscintl,money_var="Action_Obligation_Then_Year",
                  deflator_var ="OMB23_OUT21")

OTA_data<-deflate(OTA_data,money_var="Action_Obligation_Then_Year",
                  deflator_var ="OMB23_OUT21")
ota_contract<-deflate(ota_contract,money_var="Action_Obligation_Then_Year",
                  deflator_var ="OMB23_OUT21")
...
ota_lc<-prepare_labels_and_colors(OTA_data %>% select(-contractingOffice))#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")
...
fed_lc<-prepare_labels_and_colors(fed_data)#,path="C:\\Users\\gsand\\Repositories\\Lookup-Tables\\style\\")
                                  #)#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")
fed_ck<-get_column_key(fed_data)#,path="C:\\Users\\gsand\\Repositories\\Lookup-Tables\\style\\")
...
# drone %>% group_by(Customer) %>% summarise(Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21,na.rm=TRUE))
...
droneplat<-drone %>%
  ...
  dplyr::summarise(Action_Obligation_2020=sum(ifelse(Fiscal_Year==2020,Action_Obligation_OMB23_OUT21,0)),
                                              Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21)
                   )%>%
  group_by () %>%#PlatformPortfolioUAV
  mutate(rank_total=rank(desc(Action_Obligation_OMB23_OUT21)),
         rank_2020=rank(desc(Action_Obligation_2020)))
droneplat %>% arrange(desc(Action_Obligation_OMB23_OUT21))


droneplat$TopProject<-
  ifelse(droneplat$rank_2020<=7 | droneplat$rank_total<=7,droneplat$ProjectName,NA)

drone<-left_join(drone,droneplat %>% dplyr::select(-Action_Obligation_OMB23_OUT21,-Action_Obligation_2020),
                 by=c("ProjectName"))
...
dronePSC<-drone %>% group_by (ProductOrServiceCode,ProductOrServiceCodeText) %>%
  summarise(
            Action_Obligation_2020=sum(ifelse(Fiscal_Year==2020,Action_Obligation_OMB23_OUT21,0)),
                                       Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21))%>%
  group_by () %>%
  mutate(rank_total=rank(desc(Action_Obligation_OMB23_OUT21)),
         rank_2020=rank(desc(Action_Obligation_2020)))
dronePSC %>% arrange(desc(Action_Obligation_OMB23_OUT21))
...
drone<-left_join(drone,dronePSC %>% select(-Action_Obligation_OMB23_OUT21,
...
drone %>% dplyr::group_by(ProjectName, IsRemotelyOperated)  %>% dplyr::summarise(Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21))%>%
  arrange(-Action_Obligation_OMB23_OUT21)
...
(
  co_pp_simplearea<-build_plot(
    ...
    # NA, #VAR.ncol
    x_var="dFYear", #x_var
    y_var="Action_Obligation_OMB23_OUT21", #VAR.y.variable
...
write.csv(file="..//Output//Remotely_Crewed//uas_ota_subcust.csv",row.names = FALSE,na = "",
          uas_ota_subcust$data %>% mutate(#Fiscal_Year=lubridate::year(dFYear),
            bAction_Obligation_OMB23_OUT21=Action_Obligation_OMB23_OUT21/1000000000)%>%
            arrange(Fiscal_Year)%>%
            pivot_wider(id_cols=c(SubCustomer.DoD),
                        names_from=Fiscal_Year,values_from=bAction_Obligation_OMB23_OUT21)%>%
...
(
  rc_psr_plat<-build_plot(
    data=fed_data %>% filter(Fiscal_Year>=2010,PlatformPortfolioUAV!="Unlabeled"& ContractingCustomer=="Defense"&
...
(
  rc_psr_plat_share<-build_plot(
    ...
    # color="Competition")+
    theme(legend.position = "bottom")+
    labs(y="Share of Obligations",caption="Note: Unlabeled not shown. Source: FPDS; CSIS analysis.")+
...
#   summarise(Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21))
# drone$SubCustomer
(
...
OTA_data %>% filter(SubCustomer.DoD=="Army"& PlatformPortfolioUAV=="Remotely Crewed"& Fiscal_Year<=2014)%>%
  group_by(Description_of_Requirement,LegalBusinessName,NontraditionalGovernmentContractorParticipationDescription)%>%
       summarise(Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21,na.rm=TRUE))%>%
  arrange(-Action_Obligation_OMB23_OUT21)
...
write.csv(file="..//Output//Remotely_Crewed//uas_nocompetition.csv",row.names = FALSE, na = "",
          ToplineNoComp$data%>% mutate(Fiscal_Year=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(SimpleArea,No.Competition.sum),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_OUT21))
...
(rc_contract_identification <-  build_plot(data=drone %>% filter(ContractingCustomer=="Defense"),
                                           ...
                                           share=FALSE,
                                           x_var="dFYear",
                                           y_var="Action_Obligation_OMB23_OUT21",
...
write.csv(file="..//output//Remotely_Crewed//rc_vendor_place_FMS.csv",row.names = FALSE,na = "",
          rc_vendor_place_fms$data%>%# mutate(dFYear=lubridate::year(dFYear))%>%
            pivot_wider(id_cols=c(VendorSize_Intl,IsFMS, PlaceIsForeign),
                        names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_OUT21)%>% arrange(PlaceIsForeign,IsFMS,VendorSize_Intl))
...
defense_platformUAV_vendor$entity
...
```{r drone_count}
  ...
  #   group_by(EntityID) %>% 
  #   dplyr::mutate(TotalSpend=sum(Action_Obligation_OMB23_OUT21,na.rm=TRUE))%>% group_by()%>%
...
topUAVannual<-defense_UAV_vendor %>%
  ...
  dplyr::summarize(Action_Obligation_OMB23_OUT21=sum(Action_Obligation_OMB23_OUT21,na.rm=TRUE)) %>% arrange(Fiscal_Year,Toppos)
...
defense_UAV_vendor<-defense_UAV_vendor %>%
  group_by(EntityID) %>% 
  dplyr::mutate(TotalSpend=sum(Action_Obligation_OMB23_OUT21,na.rm=TRUE),
                TotalPos=rank(-TotalSpend))
...
# (
# vend_spark<-ggplot(defense_UAV_vendor %>% filter(TotalPos<=20),aes(x=Fiscal_Year,y=Action_Obligation_OMB23_OUT21))+
...
contractorUAVwide<-defense_platformUAV_contractor %>%
  ...
  dplyr::summarize(Action_Obligation_OMB23_OUT21 = sum(Action_Obligation_OMB23_OUT21),
...
# 
# (
# vend_spark<-ggplot(defense_platformUAV_contractor %>% filter(TotalPos<=20),aes(x=Fiscal_Year,y=Action_Obligation_OMB23_OUT21))+
#   facet_grid(EntityID~.,scale="free_y")+geom_col()+
#     theme_light()+
#   theme(axis.title=element_blank(),
#         axis.text.y = element_blank(), 
...
defense_UAV_both<-defense_UAV_vendor %>% pivot_longer(cols=c(EntityCount,Action_Obligation_OMB23_OUT21),names_to = "Metric")
