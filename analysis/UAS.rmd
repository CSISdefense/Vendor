...
OTA_data$SubCustomer.sum<-as.character(OTA_data$SubCustomer.sum)


summary(factor(ota_contract$SubCustomer.sum))

OTA_data$SubCustomer.sum[OTA_data$Customer!="Defense"]<-"Civilian"

ota_lc<-prepare_labels_and_colors(OTA_data)#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")
ota_ck<-get_column_key(OTA_data)#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")


fed_lc<-prepare_labels_and_colors(fed_data)#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")
fed_ck<-get_column_key(fed_data)#,path="K:\\Users\\Greg\\Repositories\\Lookup-Tables\\style\\")
...
summary(factor(topcust$TopProject))
...
load(file="../data/semi_clean/Federal_OTA.Rda")
...
write.csv(file="..//Output//Remotely_Crewed//uas_ota_sub.csv",row.names = FALSE,na = "",
          uas_ota_sub$data %>% mutate(Fiscal_Year=lubridate::year(dFYear),
...
### Simple Area Platform

```{r drone_simplearea}

(
  rc_psr_plat<-build_plot(
    data=def_data %>% filter(Fiscal_Year>=2010,PlatformPortfolioUAV!="Unlabeled"& 
...
ggsave600dpi(rc_psr_plat,file="..//output//Remotely_Crewed//def_platUAS_psr.emf",  
             width=6.5, height= 3.5, units="in",size=12, lineheight=1.2
)
...
output_TY<-def_data %>%   group_by(SimpleArea,PlatformPortfolioUAV,Fiscal_Year)%>%
  dplyr::summarize(Action_Obligation_Then_Year=sum(Action_Obligation_Then_Year,na.rm=TRUE))%>%
  arrange(Fiscal_Year) %>%
  pivot_wider(id_cols=c(PlatformPortfolioUAV,SimpleArea),
              names_from=Fiscal_Year,values_from=Action_Obligation_Then_Year) %>%
...
(
  rc_customer<-build_plot(
    data=drone %>% filter(Fiscal_Year>=2010&AnyCommercial=='Y'&Customer=="Defense") ,
...
(
  rc_commercial<-build_plot(
    data=drone %>% filter(Fiscal_Year>=2010&AnyCommercial %in% c('Y',"NonDev")),
    chart_geom = "Line Chart",
    share = FALSE,
    labels_and_colors=fed_lc,
    # NA, #VAR.ncol
...
write.csv(file="..//Output//Remotely_Crewed//uas_ota_contract_comm_sub.csv",row.names = FALSE,na = "",
          uas_ota_contract_sub$data %>% mutate(Fiscal_Year=lubridate::year(dFYear),
            bAction_Obligation_OMB23_GDP21=Action_Obligation_OMB23_GDP21/1000000000)%>%
            arrange(Fiscal_Year)%>%
            pivot_wider(id_cols=c(SubCustomer.sum,IsOTA),
                        names_from=Fiscal_Year,values_from=bAction_Obligation_OMB23_GDP21)%>%
            arrange(SubCustomer.sum,IsOTA))
```
...
CommPlatUAV<-build_plot(
  data=fed_data %>% filter(Fiscal_Year>=2010,AnyCommercial=='Y' & PlatformPortfolioUAV!="Unlabeled"),
  chart_geom = "Line Chart",
  share = FALSE,
  labels_and_colors=fed_lc,
  # NA, #VAR.ncol
...
### FMS
```{r ToplineFMS}
summary(def_data$Competition.effective.only)
def_data <- def_data %>% group_by(Fiscal_Year,PlatformPortfolioUAV)  %>%
  dplyr::mutate(pctFYPlatCustomer=Action_Obligation_Then_Year/sum(Action_Obligation_Then_Year,na.rm=TRUE))
...
(b<-build_plot(data=platformUAS_only %>% filter(Fiscal_Year>=2010 & PlatformPortfolioRemote %in% 
                                                  c(#"Aircraft","Electronics, Comms, & Sensors",
                                                    #,"Ordnance and Missiles"
                                                    "Remotely Operated")&
                                                      IsEntityAbove2018constant10ThousandThreshold==1),
