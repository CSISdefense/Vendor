...
contractorUAVwide<-defense_platformUAV_contractor %>%
  filter(Fiscal_Year >=2005 & Fiscal_Year<=2021) %>%
  ...
  summarize(Action_Obligation_OMB23_GDP21 = sum(Action_Obligation_OMB23_GDP21),
  ...
  pivot_wider(id_cols=c(PlatformPortfolioUAV,IsDefense,UnknownCompany,AllContractor,
           jointventure,TotalSpend),
              names_from=Fiscal_Year,values_from=Action_Obligation_OMB23_GDP21)%>% 
  group_by(IsDefense) %>%
     dplyr::mutate(TotalPos=rank(-TotalSpend)) %>% arrange(-IsDefense,TotalPos)
...
defense_UAV_vendor$IsEntityAbove2018constant10ThousandThreshold
...
ggsave(gridExtra::grid.arrange(a,b,nrow=1),
                          file="../output/Remotely_Crewed//uav_entity_count_size.png",
              height =3.5, width=6.5)#size = 12, lineheight=1,
