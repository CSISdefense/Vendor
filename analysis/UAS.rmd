...
### Wide
```{r WideDisplay}
defense_platformUAV_vendor<-deflate(defense_platformUAV_vendor,money_var="Action_Obligation")  %>%
  group_by(EntityID) %>% 
  dplyr::mutate(TotalSpend=sum(Action_Obligation_OMB23_GDP21,na.rm=TRUE),
                TotalPos=rank(-TotalSpend))

vendorUAVwide<-defense_platformUAV_vendor %>%
...
write.csv(vendorUAVwide,file="../output/Remotely_Crewed//vendorUAVwide.csv",
          row.names = FALSE,na = "")
