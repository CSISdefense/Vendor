...
platpscintl$SubCustomer.DoD[platpscintl$ContractingCustomer!="Defense"]<-"Civilian"
...
OTA_data$SubCustomer.DoD[OTA_data$ContractingCustomer!="Defense"]<-"Civilian"

ota_contract$SubCustomer.DoD<-as.character(ota_contract$SubCustomer.sum)
ota_contract$SubCustomer.DoD<-as.character(ota_contract$SubCustomer.sum)

ota_lc<-prepare_labels_and_colors(OTA_data %>% dplyr::select(-ContractingOfficeName,-MajorCommandName))
...
# drone %>% group_by(ContractingCustomer) %>% summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE))
japan_log_path<-file.path(get_local_sharepoint_path("JAPAN-DIIG Project - Documents"),"General","Data")
...
eo_psc<-c("1220","1240","1280","3635","5850","6008","6031","6033","6650")
  platpscintl$PlatformPortfolioUAVEO<-as.character(platpscintl$PlatformPortfolioUAV)
  platpscintl$PlatformPortfolioUAVEO[platpscintl$ProductOrServiceCode %in% eo_psc]<-"EO Sensors"
  platpscintl$CaseStudy<-NA
  platpscintl$CaseStudy[platpscintl$PlatformPortfolioUAV=="Remotely Crewed"]<-"UAS"
  platpscintl$CaseStudy[platpscintl$ProductOrServiceCode %in% eo_psc]<-"EO Sensors"
  
  fedpsc_ck<-get_column_key(platpscintl)
  fedpsc_lc<-prepare_labels_and_colors(platpscintl)
  
  droneEO<-platpscintl %>% filter(CaseStudy %in% c("UAS","EO Sensors")) 
...
drone$Action_Obligation_OMB25_GDP23

(drone %>%   mutate(ProjectName=ifelse(ProjectName=="Other UAS",NA,ProjectName)) %>%
  group_by(ProjectName) %>%
  dplyr::summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23)) %>% 
  arrange(desc(Action_Obligation_OMB25_GDP23)))

drone<-drone  %>%   label_top(
                    "ProjectName",
                    n=8,
...
(drone %>% group_by (ProductOrServiceCode,ProductOrServiceCodeText) %>%
...
drone %>% dplyr::group_by(ProjectName, IsRemotelyOperated)  %>% dplyr::summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23))%>%
  arrange(-Action_Obligation_OMB25_GDP23)
                                                         
#write.csv(drone, file="..//Output//Remotely_Crewed//dronesdata.csv")
...
colnames(rdte_uas)<-gsub(" ",".",colnames(rdte_uas))
...
p_long<-procurement_uas %>% pivot_longer(cols=as.character(c(1999:2025)),names_to="Fiscal_Year",values_to="Funding_Const")
r_long<-rdte_uas %>% pivot_longer(cols=as.character(c(1999:2025)),names_to="Fiscal_Year",values_to="Funding_Const")
...
inv_long<-rbind(p_long %>% dplyr::select(Subcustomer,Fiscal_Year,Funding_Const,ColorOfMoney,Domain),
      r_long %>% dplyr::select(Subcustomer,Fiscal_Year,Funding_Const,ColorOfMoney,Domain))
...
```{r}
  ...
  (
    uas_ota_sub<-build_plot(
      data=OTA_data %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2023.2) ,
      chart_geom = "Bar Chart",
      share = FALSE,
      labels_and_colors=ota_lc,
      # NA, #VAR.ncol
...
ggsave600dpi(uas_ota_subcust,file="..//output//Remotely_Crewed//uas_ota_subcust.svg",  
             width=3.25, height= 3, units="in",size=12, lineheight=1.2
)
...
log_plot(plot=rceo_psr, df=droneEO %>% dplyr::filter(  lubridate::year(dFYear)>=1990 & 
           ...
           filename="UAS_EO_PSR",xlsx="Japan_Def_Coop.xlsx",
            #suppress_text = FALSE,
...
(
  rc_psr_plat<-build_plot(
    data=platpscintl %>% filter(Fiscal_Year>=2010,PlatformPortfolioUAV!="Unlabeled"& ContractingCustomer=="Defense"&
...
(
  eorc_customer<-build_plot(
    ...
    labs(title="U.S. Federal UAS and EO Sensor Contracts by Customer",
         y="Obligations (Constant 2022 $s)",caption="Note: Labeling efforts are ongoing, UAS R&D and services are not identified by code and must be manually labeled. Unlabeled not shown.\nSource: FPDS and CSIS analysis.")#+
  # facet_wrap(~ContractingAgencyTextSwr,ncol=4)
)
...
(
  rc_custproj<-build_plot(
    ...
    labs(title="US Federal Contracting for UAS\nby Customer and Project",
         y="Obligations (Constant 2022 $s)",caption="Note: Labeling efforts are ongoing, UAS R&D and services are not identified by code and must be manually labeled.\nSource: FPDS and CSIS analysis.")#+
  # facet_wrap(~ContractingAgencyTextSwr,ncol=4)
)
...
droneEO %>% filter(CaseStudy=="EO Sensors") %>% group_by(ProductOrServiceCodeText) %>%
...
levels(droneEO$PSCsum)<-list(
  ...
  "Optical Sighting &\nRanging Equipment"="OPTICAL SIGHTING & RANGING EQUIPMEN",
  "Light Communication\nEquipment"="VISIBLE AND INVISIBLE LIGHT COMMUNICATION EQUIPMENT",
  "Other EO Sensors &\nRelated"=c("INTEGRATED OPTICAL CIRCUITS","CRYSTAL & GLASS INDUSTRIES MACHINE",
...
(
  eo_custproj<-build_plot(
    data=droneEO %>% filter(Fiscal_Year>=2010 & CaseStudy=="EO Sensors") ,
    chart_geom = "Bar Chart",
    share = FALSE,
    labels_and_colors=fedpsc_lc,
    column_key=fedpsc_ck,
    ...
    # color="Competition")+
    theme(legend.position = "right")+
    labs(title="US Federal Contracting for EO Sensors\nby Customer and Product Code",
         y="Obligations (Constant 2022 $s)",caption="Note: Products only, EO sensor specific R&D and servies codes were not found.\nSource: FPDS and CSIS analysis.")#+
  # facet_wrap(~ContractingAgencyTextSwr,ncol=4)
)


log_plot(plot=eo_custproj, df=droneEO %>% filter(CaseStudy=="EO Sensors"),
...
droneEO %>% filter(CaseStudy=="EO Sensors") %>% group_by(Project.Name) %>%
  summarise(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE)) %>%
  arrange(-Action_Obligation_OMB25_GDP23)
...
#           output_TY)
  ...
  (
    uas_ota_sub<-build_plot(
      data=OTA_data %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022) ,
      chart_geom = "Bar Chart",
      share = FALSE,
      labels_and_colors=ota_lc,
      # NA, #VAR.ncol
  ...
  (
    uas_ota_contract_comm_sub<-build_plot(
      data=ota_contract %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022& 
      ...
      # color="Competition")+
      theme(legend.position = "bottom")+
      labs(y="Obligations (Constant 2022 $s)",title="Commercial Contracts & Other Transaction Authority\n(OTA) Arrangements for UAS by Customer",caption="Source: FPDS and CSIS analysis.")+
     facet_grid(.~SubCustomer.DoD)
  )

log_plot(plot=uas_ota_contract_comm_sub, df=ota_contract %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022& 
                                     AnyCommercial %in% c('Y',"NonDev","Other OTA")),
           filename="UAS_Commercial_OTA",xlsx="UAS_Commercial_OTA.xlsx",suppress_text = FALSE,
           sheet="UAS_CommerOTA",path=japan_repo_path,second_path=japan_log_path, height=3.5,
           format=TRUE,#var_list=c("FlowText","StateRegionJapan"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
...
(
    uas_contract_comm_sub<-build_plot(
      data=fed_data %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022& 
                                     AnyCommercial %in% c('Y',"NonDev","Other OTA")&
                                     Fiscal_Year>=2011) ,
      chart_geom = "Bar Chart",
      share = FALSE,
...
### EO  Commercial Subcustomer
```{r drone_customer}

  (
    eo_ota_sub<-build_plot(
      data=OTA_data %>% filter(ProductOrServiceCode %in% eo_psc & Fiscal_Year<=2022) ,
      chart_geom = "Bar Chart",
      share = FALSE,
      labels_and_colors=ota_lc,
      # NA, #VAR.ncol
...
# 
#   (
#     eo_ota_contract_comm_sub<-build_plot(
#       data=ota_contract %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022& 
#                                      AnyCommercial %in% c('Y',"NonDev","Other OTA")&
#                                      Fiscal_Year>=2011) ,
#       chart_geom = "Bar Chart",
...
#       # color="Competition")+
#       theme(legend.position = "bottom")+
#       labs(y="Obligations (Constant 2022 $s)",title="Commercial Contracts & Other Transaction Authority (OTA)\nArrangements for EO by Customer",caption="Source: FPDS and CSIS analysis.")+
#      facet_grid(.~SubCustomer.DoD)
#   )
...
# 
# (
#     eo_contract_comm_sub<-build_plot(
#       data=fed_data %>% filter(PlatformPortfolioUAV=="Remotely Crewed" & Fiscal_Year<=2022& 
#                                      AnyCommercial %in% c('Y',"NonDev","Other OTA")&
#                                      Fiscal_Year>=2011) ,
#       chart_geom = "Bar Chart",
...
(eorc_contract_identification <-  build_plot(data=droneEO %>% filter(ContractingCustomer=="Defense" &Fiscal_Year>=2012),
                                           chart_geom="Bar Chart",
                                           share=FALSE,
                                           x_var="dFYear",
...
log_plot(plot=eorc_contract_identification, df=droneEO %>% dplyr::filter( 
...
log_plot(plot=rceo_psr, df=droneEO %>% dplyr::filter(  lubridate::year(dFYear)>=2010 & 
                                                                            lubridate::year(dFYear)<=2023),
...
### EO Top PSC Vendor Size 
```{r drone_customer}

droneEO %>% filter(CaseStudy=="EO Sensors") %>% filter(VendorSize_Intl=="International") %>%
  group_by(VendorISOalpha3,OriginISOalpha3) %>%
...
(
  eo_vendpsc<-build_plot(
    ...
    # color="Competition")+
    theme(legend.position = "right")+
    labs(title="US Federal Contracting for EO Sensors\nby Product Code and Vendor Size",
         y="Obligations (Constant 2022 $s)",caption="Note: Labeling efforts are ongoing, EO sensor-specific R&D and services codes were not found.\nSource: FPDS and CSIS analysis.")#+
  # facet_wrap(~ContractingAgencyTextSwr,ncol=4)
)


log_plot(plot=eo_vendpsc, df=droneEO %>% filter(CaseStudy=="EO Sensors"),
           filename="EO_PSC_Vendor",xlsx="Japan_Def_Coop.xlsx",suppress_text = FALSE,
           sheet="EOpscVend",path=japan_repo_path,second_path=japan_log_path, height=3.5,
           format=TRUE,#var_list=c("FlowText","StateRegionJapan"),
           output_doc_png=TRUE,excel_y_var=TRUE
  )
...
```{r drone_count}
  ...
  #   group_by(EntityID) %>% 
  #   dplyr::mutate(TotalSpend=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE))%>% group_by()%>%
...
topUAVannual<-defense_UAV_vendor %>%
  ...
  dplyr::summarize(Action_Obligation_OMB25_GDP23=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE)) %>% arrange(Fiscal_Year,Toppos)
...
defense_UAV_vendor<-defense_UAV_vendor %>%
  group_by(EntityID) %>% 
  dplyr::mutate(TotalSpend=sum(Action_Obligation_OMB25_GDP23,na.rm=TRUE),
                TotalPos=rank(-TotalSpend))

vendorUAVwide<-defense_UAV_vendor %>%
  filter(Fiscal_Year >=2010 & Fiscal_Year<=2023.2) %>%
  dplyr::mutate(TopEntityText=ifelse(pos<=10,EntityText,"All Other"),
         Toppos=ifelse(pos<=10,pos,0)) %>%
...
# (
# vend_spark<-ggplot(defense_UAV_vendor %>% filter(TotalPos<=20),aes(x=Fiscal_Year,y=Action_Obligation_OMB25_GDP23))+
...
contractorUAVwide<-defense_platformUAV_contractor %>%
  ...
  dplyr::summarize(Action_Obligation_OMB25_GDP23 = sum(Action_Obligation_OMB25_GDP23),
...
# 
# (
# vend_spark<-ggplot(defense_platformUAV_contractor %>% filter(TotalPos<=20),aes(x=Fiscal_Year,y=Action_Obligation_OMB25_GDP23))+
#   facet_grid(EntityID~.,scale="free_y")+geom_col()+
#     theme_light()+
#   theme(axis.title=element_blank(),
#         axis.text.y = element_blank(), 
...
defense_UAV_both<-defense_UAV_vendor %>% pivot_longer(cols=c(EntityCount,Action_Obligation_OMB25_GDP23),names_to = "Metric")
