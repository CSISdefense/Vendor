...
fed_data$SubCustomer.sum[fed_data$ContractingCustomer!="Defense"]<-"Civilian"
...
drone$TopProject<-factor(drone$TopProject,
                         levels=c("Other Labeled Project","TACTICAL UAV" ,"MQ-1C Gray Eagle",
                                  "BAMS","VTUAV",
                                  "GLOBAL HAWK" ,"MQ-9 Reaper"  ,"Other UAS"
                                  ))
...
drone<-fed_data%>% filter(PlatformPortfolioUAV=="Remotely Crewed")
...
(
  rc_customer<-build_plot(
    data=drone %>% filter(Fiscal_Year>=2010&AnyCommercial=='Y'&ContractingCustomer=="Defense") ,
...
## Vendor Sizeand Count via Entity
```{r drone_count}
