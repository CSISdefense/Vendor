...
Term_01D_line<-ggplot(data=subset(Term_smp,!is.na(l_Offer)) %>% 
  group_by(l_Offer) %>% 
...
Term_01D_bar<-ggplot(data=subset(Term_smp,b_Term==1),
       aes(x=l_Offer))+geom_bar(stat="count")
Term_01D_bar

#First a quick frequency plot for terminations
Term_01D_bar<-ggplot(data=Term_smp,
       aes(x=cl_Offer))+geom_histogram(stat="count")+facet_wrap(~Term,scales="free_y")
Term_01D_bar
...
Term15A <- glmer (data=Term_smp,b_Term ~ l_Ceil + l_Days + 
...
###Model 15B: Number of offers for Competed Contracts

```{r Model15B}
Term15B <- glmer (data=subset(Term_smp,cb_Comp==1),
...
Term11A <- glmer (data=Term_smp,
                 b_Term ~ l_Ceil +
               (1 | Who) + (1 | PSR_What) + (1 | StartFY), family=binomial(link="logit"))
display(Term11A)


Term11A_s <- glm (data=Term_smp,
...
Term_plot_01A+ stat_function(fun = fit_curve, args=list(a=coef(Term11A_s)[1],
                                                        b=coef(Term11A_s)[2]),
                             color="blue")
...
###Model 11B: Maximum Duration

```{r Model11B}
Term11B <- glmer (data=Term_smp,
                 b_Term ~ l_Ceil + 
               l_Days  +
               (1 | Who) + (1 | PSR_What) + (1 | StartFY), family=binomial(link="logit"))
display(Term11B)
