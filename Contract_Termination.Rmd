...
###Model 05A: Fixed Price and Any International
...
Term_05A <- glm (data=Term_smp,
                 b_Term ~n_Comp + 
                   ...
                   SIDV:n_Comp+MIDV:n_Comp+OIDV:n_Comp+
                   n_UCA:n_Comp+
                   SIDV:n_Intl+MIDV:n_Intl+OIDV:n_Intl
                    , family=binomial(link="logit"))
display(Term_05A)
...
# Plot the model and Vehicle 
Term_06A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
...
###Model 06B: Product/Service/R&D and Platform
```{r Model06B}
...
# Plot the model and Vehicle 
Term_06B_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(Term_06C)


#Plot residuals versus fittedO
residuals_term_plot(Term_06C)+
  labs(x="Estimated  Pr (Termination)")


# Term_06E <- glmer(data=Term_smp,
#                  b_Term ~ n_Comp + 
#                    cl_Ceil + cl_Days+ 
#                    SIDV + MIDV + OIDV +
#                    n_Comp:cl_Ceil  +
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(Term_06D)


#Plot residuals versus fittedO
residuals_term_plot(Term_06D)+
  labs(x="Estimated  Pr (Termination)")
...
# #Plot the fitted values vs actual results
# binned_fitted_versus_residuals(Term_07A)
# 
# 
# #Plot residuals versus fittedO
# residuals_term_plot(Term_07A)+
#   labs(x="Estimated  Pr (Termination)")


# Term_07B <- glmer(data=Term_smp,
#                  b_Term ~n_Comp + 
#                    cl_Ceil + cl_Days+ 
#                    SIDV + MIDV + OIDV +
#                    n_Comp:cl_Ceil  +
...
# Model failed to converge with max|grad| = 0.00505365 (tol = 0.001, component 1)


Term_07E <- glmer(data=Term_smp,
                 b_Term ~n_Comp +
                   cl_Ceil + cl_Days+
                   SIDV + MIDV + OIDV +
                   # n_Comp:cl_Ceil  +
...
# Plot the model and Vehicle 
Term_07A_curve<-function(x, Comp,Ceil,Days,SIDV,MIDV,OIDV,Fixed,UCA){invlogit(
  cbind(1,Comp,Ceil,Days,SIDV,MIDV,OIDV,
        Comp*Ceil,Days*Ceil,Days*Comp,
        Fixed,UCA,x,
        Days*x,Days*MIDV,Days*OIDV,
        UCA*Comp) %*%  coef(Term_07A))}

#Days curves
discrete_fitted_term_model(Term_smp,"SIDV") +
  stat_function(fun = Term_07A_curve,
                             args=list(Comp=0,Ceil=0,Days=0,
                                       MIDV=0,SIDV=0,OIDV=0,
                                       Fixed=1,UCA=0),color="blue")+
  stat_function(fun = Term_07A_curve,
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(Term_07F)

#Plot residuals versus fittedO
residuals_term_plot(Term_07F)+
  labs(x="Estimated  Pr (Termination)")
...
##When
###Model 08A: Sub Customer
```{r Model08A}
#Frequency Plot
freq_discrete_term_plot(Term_smp,"StartFY")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))

#Percent Terminated Plot 
discrete_percent_term_plot(Term_smp,"StartFY")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
#Plot the fitted values vs actual results
binned_fitted_versus_residuals(Term_08F)

#Plot residuals versus fittedO
residuals_term_plot(Term_08F)+
  labs(x="Estimated  Pr (Termination)")
