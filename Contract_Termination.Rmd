...
#Intl
DefenseModelAndDetail$nIntl[DefenseModelAndDetail$Intl=="Unlabeled"]<-NA
...
```{r Model02}
Term02 <- glmer (data=Term_smp,
                 nTerm ~ lCeil + 
               lDays  +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term02)
...
Term05 <- glmer (data=Term_smp,
                 nTerm ~ lCeil + 
               lDays + nFixed +  lDays:nFixed +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term05)
...
###Model 09: Competition


```{r Model09}

Term109 <- glmer (data=Term_smp,nTerm ~ lCeil + 
               lDays + nFixed  + nUCA + Comp +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term09)
```
...
```{r Model10}
Term10 <- glmer (data=subset(Term_smp,Comp==1),
              nTerm ~ lCeil + 
               lDays + nFixed  + nUCA +  lOffer +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term10)
```
...
###Model 11: Competition and Fixed Price 

```{r Model11}
Term11 <- glmer (data=Term_smp,
                 nTerm ~ lCeil + 
               lDays + nFixed  + nUCA +  Comp + Comp:nFixed +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term11)
...
```{r Model12}
Term13 <- glmer (data=subset(Term_smp,Comp==1),
              nTerm ~ lCeil + 
               lDays + nFixed  + nUCA +  lOffer + lOffer:nFixed +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term12)
...
###Model 13: Competition and Contract Ceiling
```{r Model13}
Term14 <- glmer (data=Term_smp,
                 nTerm ~ lCeil + 
               lDays + nFixed  + nUCA +  Comp + Comp:lCeil +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=binomial(link="logit"))
display(Term13)
...
The model fails to converge when combining competition and ceiling. In addition, splitting the two results in a ceiling that no longer exceeds the standard error. Leaving this out of the model.



##Selection: Model 9 

While the direction of competition is still puzzling, none of the attempts to explain it added much to the model. So the study team will use model 09 going forward.
