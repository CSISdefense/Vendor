...
head(DefenseModelAndDetail)
 summary(DefenseModelAndDetail$b_Term)
...
Term01A_s <- glm (data=Term_smp,
                 b_Term ~ l_Ceil, family=binomial(link="logit"))
...
Term_plot_01A+ stat_function(fun = fit_curve, args=list(a=coef(Term01A_s)[1],
                                                        b=coef(Term01A_s)[2]),
                             color="blue")


ggplot(data=Term_smp,
       aes(y=b_Term,x=l_Ceil,color=Comp))+geom_point()+scale_color_continuous(low="red",high="blue")
...
Term05A <- glmer (data=Term_smp,b_Term ~ l_Ceil + l_Days + 
                   SIDV + MIDV + OIDV + SIDV:l_Days + 
                   n_Fixed  + n_UCA +
                   Comp +
               (1 | Who) + (1 | PSR_What) + (1 | StartFY), family=binomial(link="logit"))
...
```{r Model5C}
Term05C <- glmer (data=Term_smp,b_Term ~ l_Ceil + l_Days + 
                   SIDV + MIDV + OIDV + SIDV:l_Days + 
                   n_Fixed  + n_UCA +
                   Comp + Comp:n_Fixed +
               (1 | Who) + (1 | PSR_What) + (1 | StartFY), family=binomial(link="logit"))
