...
ggplot(annual_naics6_summary,aes(x=Fiscal.Year,weight=Obligation.2016))+
  geom_bar()+facet_wrap(~NAICS2)
...
NAICS2digit<-ggplot(subset(annual_naics2_summary,Fiscal.Year>=2007 & Fiscal.Year<=2015),
       aes(x=NAICS_label,weight=Obligation.2016))+
  geom_bar()+coord_flip()+
...
NAICS_join$StartFY<-NAICS_join$Fiscal.Year
NAICS_join$StartFY<-NAICS_join$StartFY+1
NAICS_join$HHI_lag1<-NAICS_join$hh_index
NAICS_join$NAICS_Code<-as.integer(NAICS_join$NAICS_Code)
NAICS_join<-subset(NAICS_join, select=c(StartFY,NAICS_Code,HHI_lag1))
