...
NAICS_join$StartFY<-as.numeric(format(NAICS_join$Fiscal.Year,'%Y'))
NAICS_join$StartFY<-NAICS_join$StartFY+1
NAICS_join$hh_index_lag1<-NAICS_join$hh_index
NAICS_join<-subset(NAICS_join, select=c(StartFY,NAICS_Code,hh_index_lag1))
colnames(NAICS_join)[colnames(NAICS_join)=="NAICS_Code"]<-"NAICS"
