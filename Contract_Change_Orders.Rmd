...
```{r TransformData}
#NChg
DefenseModelAndDetail$NChg<-DefenseModelAndDetail$SumOfisChangeOrder
...
DefenseModelAndDetail$nFixed<-as.integer(as.character(DefenseModelAndDetail$nFixed))
 ...
 #nIncent
DefenseModelAndDetail$nIncent<-factor(DefenseModelAndDetail$Fee)
...
#Intl
DefenseModelAndDetail$Intl[DefenseModelAndDetail$Intl=="Unlabeled"]<-NA
...
NChg01 <- glmer (data=NChg_smp,
                 NChg ~ lCeil +
               (1 | PSR) + (1 | What) + (1 | Veh) + (1 | Who) + (1 | StartFY), family=poisson)
display(NChg01)
...
NChg02 <- glmer (data=NChg_smp,
  NChg ~ lCeil + 
               lDays  +
               (1 | Who) + (1 | PSR)  + (1 | What) + (1 | Intl ) + (1 | StartFY) + (1 | Veh), family=poisson)
display(NChg02)
...
```{r Model10}
NChg11 <- glmer (data=subset(NChg_smp,Comp==1),
              NChg ~ lCeil + 
